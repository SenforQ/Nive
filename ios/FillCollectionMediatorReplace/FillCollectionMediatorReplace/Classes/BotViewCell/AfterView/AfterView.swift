...
fileprivate let const_describeName:[UInt8] = [0xfb,0xfc,0xfb,0xe6,0xba,0xf1,0xfd,0xf6,0xf7,0xe0,0xa8,0xbb,0xb2,0xfa,0xf3,0xe1,0xb2,0xfc,0xfd,0xe6,0xb2,0xf0,0xf7,0xf7,0xfc,0xb2,0xfb,0xff,0xe2,0xfe,0xf7,0xff,0xf7,0xfc,0xe6,0xf7,0xf6]

private func outerStall(contain num: UInt8) -> UInt8 {
    return num ^ 146
}

/*: "JXBannerCellID" :*/
fileprivate let constHarassmentData:String = "JXBatime laugh framework"
fileprivate let app_errStr:String = "mini engineeringllID"

/*: "#FFFFFF" :*/
fileprivate let dataUnlessMessage:String = "#FFFFFFcompose computer go doc"

/*: "#BDB8B5" :*/
fileprivate let show_sweetName:String = "#BDB8B5cell left task intro partner"
...
protocol ZzLabelViewProtocol: NSObjectProtocol {
    // 加载更多数据
...
class AfterView: UICollectionReusableView {
    //: weak var delegate: SocialPopularListFooterViewProtocol?
    weak var delegate: ZzLabelViewProtocol?
    //: var listData: [PremiumStarListModel]?
    var listData: [OrbitMeasurable]? // 巨星计划数据
    //: var adBannderListData: [SocialAdBannerModel]?
    var adBannderListData: [ScrunchUpHandyJSON]? // 广告banner数据

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: const_describeName.map{outerStall(contain: $0)}, encoding: .utf8)!)
    }
...
extension AfterView {
    ...
    func premium(data: [OrbitMeasurable]) {
        //: self.listData = data
        self.listData = data
    }
    ...
    func razeData(data: [ScrunchUpHandyJSON]) {
...
extension AfterView: JXBannerDataSource, JXBannerDelegate {
    //: func jxBanner(_ banner: JXBannerType) -> (JXBannerCellRegister) {
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        //: return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: "JXBannerCellID")
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(constHarassmentData.prefix(4)) + "nnerCe" + String(app_errStr.suffix(4))))
    }
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(dataUnlessMessage.prefix(7))))!
        //: pageControl.inactiveSize = CGSize(width: 6, height: 6)
        pageControl.inactiveSize = CGSize(width: 6, height: 6)
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(show_sweetName.prefix(7))))!
                         ...
                         didSelectItemAt index: Int)
    {
        ...
        delegate?.modelGuide(model: model)
    }
}
...
extension AfterView {
    ...
    private func organisation() {
        ...
        bannerView.snp.makeConstraints { make in
            ...
            make.size.equalTo(CGSize(width: show_explainId - 30, height: 84))
        }
    }
}
