...
fileprivate let app_bearEventMessage:[UInt8] = [0xf6,0xf1,0xf6,0xeb,0xb7,0xfc,0xf0,0xfb,0xfa,0xed,0xa5,0xb6,0xbf,0xf7,0xfe,0xec,0xbf,0xf1,0xf0,0xeb,0xbf,0xfd,0xfa,0xfa,0xf1,0xbf,0xf6,0xf2,0xef,0xf3,0xfa,0xf2,0xfa,0xf1,0xeb,0xfa,0xfb]

private func handPick(following num: UInt8) -> UInt8 {
    return num ^ 159
}
...
class BringHomeView: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_bearEventMessage.map{handPick(following: $0)}, encoding: .utf8)!)
    }

    //: func customUI() {
    func rewrite() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(imageViewClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(publishBy))
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(evaluateTap))
...
extension BringHomeView: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func publishBy(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func evaluateTap(tap: UIGestureRecognizer) {
    ...
    func resignAbout(url: String) {
        //: imageView.setUrlImage(urlStr: url)
        imageView.scrubFinish(urlStr: url)
    }

    //: func configImage(image: UIImage) {
    func relapsing(image: UIImage) {
