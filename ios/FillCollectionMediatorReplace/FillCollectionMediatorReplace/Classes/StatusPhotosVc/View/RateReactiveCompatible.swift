...
fileprivate let app_speakerMessage:[UInt8] = [0x2,0x5,0x2,0x1f,0x43,0x8,0x4,0xf,0xe,0x19,0x51,0x42,0x4b,0x3,0xa,0x18,0x4b,0x5,0x4,0x1f,0x4b,0x9,0xe,0xe,0x5,0x4b,0x2,0x6,0x1b,0x7,0xe,0x6,0xe,0x5,0x1f,0xe,0xf]

private func performSibling(formation num: UInt8) -> UInt8 {
    return num ^ 107
}
...
class RateReactiveCompatible: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_speakerMessage.map{performSibling(formation: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var progressView: PersonalView = {
        ...
        let v = PersonalView(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(postGesture(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(exist(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension RateReactiveCompatible {
    //: @objc func panGesture(gesture: UIPanGestureRecognizer) {
    @objc func postGesture(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func exist(gesture: UIPanGestureRecognizer) {
