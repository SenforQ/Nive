...
fileprivate let show_dataUrl:[UInt8] = [0x98,0x9f,0x98,0x85,0xd9,0x92,0x9e,0x95,0x94,0x83,0xcb,0xd8,0xd1,0x99,0x90,0x82,0xd1,0x9f,0x9e,0x85,0xd1,0x93,0x94,0x94,0x9f,0xd1,0x98,0x9c,0x81,0x9d,0x94,0x9c,0x94,0x9f,0x85,0x94,0x95]

private func cheeseMeanYear(sheer num: UInt8) -> UInt8 {
    return num ^ 241
}

/*: "icon_home_v" :*/
fileprivate let app_balanceSectionKey:String = "icolarge"

/*: "icon_lounge" :*/
fileprivate let data_combinedTitle:String = "he legitimate freshicon_"

/*: "#AAAAAA" :*/
fileprivate let main_warnData:[Character] = ["#"]
fileprivate let showLateData:[Character] = ["A","A","A","A","A","A"]

/*: "icon_moment_nor_report" :*/
fileprivate let data_minuteValue:[Character] = ["i","c","o","n","_","m","o","m","e","n","t","_","n","o","r","_","r"]
fileprivate let appMakerUrl:String = "EPORT"

/*: "img_me_edit_photo_default" :*/
fileprivate let main_poseFormat:[Character] = ["i","m","g","_","m","e","_","e","d","i","t","_","p","h","o","t","o","_","d","e"]
fileprivate let appEstimatedMsg:String = "worldault"

/*: "icon_home_boy" :*/
fileprivate let appFlushPath:String = "icdirectionn"
fileprivate let dataIncidentName:String = "his interaction stretch mine forthe_boy"

/*: "icon_home_girl" :*/
fileprivate let showHairValue:String = "icselectedn"
fileprivate let app_cancelOpStr:[Character] = ["g","i","r","l"]

/*: "level_" :*/
fileprivate let kWayFormat:String = "level_suspend warning"
...
class LanguageReactiveCompatible: UITableViewCell {
    //: var cid = ""
    var cid = ""
    //: var uid = ""
    var uid = ""
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_dataUrl.map{cheeseMeanYear(sheer: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var iconBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(cost), for: .touchUpInside)
        //: self.addSubview(btn)
        self.addSubview(btn)
        //: return btn
        return btn
    ...
    lazy var cardImgView: UIImageView = {
        ...
        img.image = UIImage.scheme(name: (app_balanceSectionKey.replacingOccurrences(of: "large", with: "n") + "_home_v"))
    ...
    lazy var sexBtn: UIButton = {
        ...
        btn.titleLabel?.font = .shareOff(type: .Medium, fontSize: 11)
        //: self.addSubview(btn)
        self.addSubview(btn)
        //: return btn
        return btn
    ...
    lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.scheme(name: (String(data_combinedTitle.suffix(5)) + "lounge"))
        //: self.addSubview(img)
        self.addSubview(img)
        //: return img
        return img
    ...
    lazy var timeLB: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(main_warnData) + String(showLateData)))
        //: self.addSubview(label)
        self.addSubview(label)
        //: return label
        return label
    ...
    lazy var BlockBtn: UIButton = {
        ...
        btn.setImage(UIImage.scheme(name: (String(data_minuteValue) + appMakerUrl.lowercased())), for: .normal)
        //: btn.imageView?.contentMode = .scaleAspectFill
        btn.imageView?.contentMode = .scaleAspectFill
        //: btn.addTarget(self, action: #selector(BlockBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(foretellingClick), for: .touchUpInside)
        //: self.addSubview(btn)
        self.addSubview(btn)
        //: return btn
        return btn
...
extension LanguageReactiveCompatible {
    ...
    @objc func down(gestureRecongnizer: UIGestureRecognizer) {
    ...
    @objc func foretellingClick() {
        //: let reportView = TalkingReportAlertView.init(frame: UIScreen.main.bounds, type: .reportCommentType, rId: cid)
        let reportView = ExtraView(frame: UIScreen.main.bounds, type: .reportCommentType, rId: cid)
        //: reportView.showReportViewIn(view: DelineateThen.getWindow())
        reportView.showThroughView(view: DelineateThen.premiumOperation())
    }
}
...
extension LanguageReactiveCompatible {
    ...
    func screenCell(model: RealmModelTransformable) {
        //: cid = model.cid ?? ""
        cid = model.cid ?? ""
        //: uid = model.uid ?? ""
        uid = model.uid ?? ""
        //: iconBtn.setUrlImage(urlStr: model.headPic ?? "", placeImg: UIImage.BundleImageNamed(name: "img_me_edit_photo_default"))
        iconBtn.mpCharacter(urlStr: model.headPic ?? "", placeImg: UIImage.scheme(name: (String(main_poseFormat) + appEstimatedMsg.replacingOccurrences(of: "world", with: "f"))))
        ...
        nameLabel.textColor = model.loungePlus ? .cancer() : .capVidNonsolidColorConfirm()
        ...
        case SharedMirrorKingCustomReflectable.male.rawValue:
            //: sexBtn.setBackgroundImage(UIImage.BundleImageNamed(name: "icon_home_boy"), for: .normal)
            sexBtn.setBackgroundImage(UIImage.scheme(name: (appFlushPath.replacingOccurrences(of: "direction", with: "o") + "_hom" + String(dataIncidentName.suffix(5)))), for: .normal)
        //: break
        //: case Gender.female.rawValue:
        case SharedMirrorKingCustomReflectable.female.rawValue:
            //: sexBtn.setBackgroundImage(UIImage.BundleImageNamed(name: "icon_home_girl"), for: .normal)
            sexBtn.setBackgroundImage(UIImage.scheme(name: (showHairValue.replacingOccurrences(of: "selected", with: "o") + "_home_" + String(app_cancelOpStr))), for: .normal)
        //: break
        //: default:
        default:
            //: break
        ...
        levelBtn.setBackgroundImage(UIImage.scheme(name: (String(kWayFormat.prefix(6))) + String(model.level ?? 1)), for: .normal)
        ...
        contentLB.text = model.content
        //: BlockBtn.isHidden = AppManager.share.loginUserMode.userID == model.uid
        BlockBtn.isHidden = AppManager.share.loginUserMode.userID == model.uid
    }
}
...
extension LanguageReactiveCompatible {
    ...
    private func startScript() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(nor))
        //: self.addGestureRecognizer(tap)
        self.addGestureRecognizer(tap)

        //: let longPress = UILongPressGestureRecognizer(target: self, action: #selector(onLongTapCell(gestureRecongnizer:)))
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(down(gestureRecongnizer:)))
