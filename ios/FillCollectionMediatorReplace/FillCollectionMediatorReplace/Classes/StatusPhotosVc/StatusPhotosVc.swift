...
fileprivate let k_zzPrincipalText:[UInt8] = [0x6c,0x6b,0x6c,0x71,0x2d,0x66,0x6a,0x61,0x60,0x77,0x3f,0x2c,0x25,0x6d,0x64,0x76,0x25,0x6b,0x6a,0x71,0x25,0x67,0x60,0x60,0x6b,0x25,0x6c,0x68,0x75,0x69,0x60,0x68,0x60,0x6b,0x71,0x60,0x61]

private func controlGray(main num: UInt8) -> UInt8 {
    return num ^ 5
}

/*: "Save successfully" :*/
fileprivate let k_asideStr:String = "player oval medication conclude fileSave"
fileprivate let showResponseFormat:String = "popularss"
...
class StatusPhotosVc: FullViewController {
    ...
    init(momentModel: OphidiaMomentModel, index: Int, type: LabelCreasePhotoType) {
        ...
        if CombinedLanguageManager.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(show_explainId), y: 0)
    ...
    init(images: [UIImage], index: Int, type: LabelCreasePhotoType) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_zzPrincipalText.map{controlGray(main: $0)}, encoding: .utf8)!)
    }
...
//        }
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: show_explainId, height: userSystemKey)
        ...
        view.frame = CGRect(x: 0, y: 0, width: show_explainId, height: userSystemKey)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(show_explainId), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(show_explainId), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(show_explainId), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosPreviewCell.className())
        view.register(BringHomeView.self, forCellWithReuseIdentifier: BringHomeView.className())
...
//    lazy var downloadBtn: ExtendedView = {
//        let btn = ExtendedView.init()
...
//        EditThen.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: BalloonWhiteTransformable = self.currenModel.pic[self.pageIndex]
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.sirenSong(showMsg: (String(k_asideStr.suffix(4)) + " suc" + showResponseFormat.replacingOccurrences(of: "popular", with: "ce") + "fully").localized)
                }
                //: } else {
            } else {
...
extension StatusPhotosVc: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func preparedness() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: BringHomeView = collectionView.dequeueReusableCell(withReuseIdentifier: BringHomeView.className(), for: indexPath) as! BringHomeView
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: BalloonWhiteTransformable = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.resignAbout(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.relapsing(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.relapsing(image: photos[indexPath.row])
        }
    ...
    func collectionView(_: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt _: IndexPath) {
        ...
        let tcell: BringHomeView = cell as! BringHomeView
        //: tcell.recoverSubviews()
        tcell.focusEffectYear()
    }
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / show_explainId)
        //: if myType == .normal {
        if myType == .normal {
