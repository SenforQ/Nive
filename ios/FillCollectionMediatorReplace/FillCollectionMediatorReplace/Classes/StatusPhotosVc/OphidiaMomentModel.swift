...
struct OphidiaMomentModel: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
    ...
    var pic = [BalloonWhiteTransformable]()
    //: var momentType: String?
    var momentType: String?
    //: var videoInfo: TalkingMomentVideoModel?
    var videoInfo: BagHandyJSON?
...
struct BalloonWhiteTransformable: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
enum TypeDefaultsSerializable: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct BagHandyJSON: HandyJSON {
    ...
    var status: PulseTextConvertible?
}

///
//: enum MomentVideoStatus: String {
enum PulseTextConvertible: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension OphidiaMomentModel {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func fromEachOneCurrent(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(HouseView.alongOld(text: content!, size: CGSize(width: show_explainId - 54, height: CGFloat(MAXFLOAT)), font: UIFont.shareOff(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == TypeDefaultsSerializable.Photo.rawValue || momentType == TypeDefaultsSerializable.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
