...
/*: "/TXCache" :*/
fileprivate let k_arcRawBoardKey:String = "/TXCachethan enough peer feedback"
...
protocol GamePlanPlayerDelegate: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func full(player: SalesReactiveCompatible, status: MaskerPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func identityAppearance(player: SalesReactiveCompatible, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func starBright(player: SalesReactiveCompatible, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class SalesReactiveCompatible: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: GamePlanPlayerDelegate?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: MaskerPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = SalesReactiveCompatible()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(k_arcRawBoardKey.prefix(8)))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(hang), userInfo: nil, repeats: true)
...
extension SalesReactiveCompatible {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func occur(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: constDailyTitle.thanBasic(), with: data_lineUrl)
    ...
    @objc func hang() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.starBright(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func spread(renderMode: KeepRenderMode) {
    ...
    func skipSuite(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isMutual == true {
        if MigrationSocketDelegate.shared.isMutual == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func attention() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.full(player: self, status: self.playStatus!)
        }
    }
...
extension SalesReactiveCompatible: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.identityAppearance(player: self, duration: Int(duration), currentTime: Int(currentTime))
