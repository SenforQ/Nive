...
/*: "reply" :*/
fileprivate let dataParentCleanStillId:[Character] = ["r","e","p","l","y"]
...
struct RealmModelTransformable: HandyJSON {
    //: var cid: String?
...
extension RealmModelTransformable {
    //: mutating func caculateCommentItemHeight() {
    mutating func charmOf() {
        //: var tempContent = ""
        var tempContent = ""
        //: if replyUser?.count ?? 0>0 {
        if replyUser?.count ?? 0 > 0 {
            //: if LanguageManager.shared.direction == .leftToRight {
            if CombinedLanguageManager.shared.direction == .leftToRight {
                //: tempContent = "reply".localized + "\(replyUser!): \(content!)"
                tempContent = (String(dataParentCleanStillId)).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (String(dataParentCleanStillId)).localized
            }
        ...
        let sizeheight = Int(HouseView.alongOld(text: tempContent, size: CGSize(width: show_explainId - 70, height: CGFloat(MAXFLOAT)), font: UIFont.shareOff(type: .Regular, fontSize: 16), lineNumber: 0).height)
