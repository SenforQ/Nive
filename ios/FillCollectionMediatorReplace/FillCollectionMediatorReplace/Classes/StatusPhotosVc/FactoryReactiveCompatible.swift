...
/*: "News Feed" :*/
fileprivate let main_primaryData:String = "News Feedopportunity ready"

/*: "There's no feed yet." :*/
fileprivate let showDevoteId:[Character] = ["T","h","e","r","e","\'","s"]
fileprivate let show_errorId:String = " no lab correct hide success full"
fileprivate let userBondKey:String = "until contend yet."

/*: "The post has been deleted ~" :*/
fileprivate let showMissingUrl:[Character] = ["T","h","e"," ","p","o","s"]
fileprivate let show_eyeFormat:String = "t behind pure action"
fileprivate let showThemeData:[Character] = ["h","a","s"," ","b","e","e","n"," ","d","e","l","e","t","e","d"," ","~"]
...
class FactoryReactiveCompatible: MirrorRealmReactiveCompatible {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var dataArr: [TalkingRemindListModel] = []
    private var dataArr: [SnakeDomainTransformable] = []
    ...
    override func viewDidLoad() {
        ...
        title = (String(main_primaryData.prefix(9))).localized
        //: setupSubviews()
        programSetup()
        //: setupSubViewsConstraint()
        setupWriteConstraint()
    ...
    lazy var emptyView: EmptyView = {
        ...
        style.TipsTitle = (String(showDevoteId) + String(show_errorId.prefix(4)) + "feed" + String(userBondKey.suffix(5))).localized
    ...
    lazy var tableView: UITableView = {
        ...
        table.register(CarminativeView.self, forCellReuseIdentifier: CarminativeView.className())
        //: return table
        return table
        //: }()
    }()
...
extension FactoryReactiveCompatible {
    ...
    private func listTheoretical() {
        //: TalkingMomentRequestTool.req_getRemindList(page: pageIndex) { data in
        PriceBlueReactiveCompatible.quantityerrupt(page: pageIndex) { data in
...
extension FactoryReactiveCompatible: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: CarminativeView.className(), for: indexPath) as! CarminativeView
        //: cell.refreshCell(dataArr[indexPath.row])
        cell.refreshVertical(dataArr[indexPath.row])
        //: return cell
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if model.status == 0 {
            ...
            diskTerms(showMsg: (String(showMissingUrl) + String(show_eyeFormat.prefix(2)) + String(showThemeData)).localized)
            //: return
            return
        }
        ...
        let detailVC = MoonlightThen(model: detailModel)
...
extension FactoryReactiveCompatible {
    ...
    private func setupWriteConstraint() {
