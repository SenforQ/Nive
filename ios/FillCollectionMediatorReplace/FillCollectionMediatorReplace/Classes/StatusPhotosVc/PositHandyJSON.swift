...
/*: "%@ reply %@：%@" :*/
fileprivate let app_survivalValue:String = "better smart matter sufficient library%@ rep"
...
struct PositHandyJSON: HandyJSON {
    //: var cid: String?
...
extension PositHandyJSON {
    ...
    mutating func capacity() {
        ...
        if replyUser.count > 0 {
            ...
            tempContent = String(format: (String(app_survivalValue.suffix(6)) + "ly %@\u{ff1a}%@"), nickname, replyUser, content)
            //: } else {
        } else {
        ...
        let sizeheight = Int(HouseView.alongOld(text: tempContent, size: CGSize(width: show_explainId - 82, height: CGFloat(MAXFLOAT)), font: UIFont.shareOff(type: .Regular, fontSize: 14), lineNumber: 0).height)
