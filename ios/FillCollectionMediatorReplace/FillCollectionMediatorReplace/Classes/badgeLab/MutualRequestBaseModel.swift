...
/*: "http://stat. :*/
fileprivate let userLiteProtectionMsg:String = "http:sphere uniform"

/*: .com" :*/
fileprivate let mainPositionContent:String = ".comchannel night"

/*: "http://" :*/
fileprivate let dataSendRequestMessage:[Character] = ["h"]
fileprivate let dataTheoryId:String = "cling unable record required yardttp://"

/*: "https://" :*/
fileprivate let user_yetMsg:String = "HTTPS"
fileprivate let show_mouthData:[Character] = [":","/","/"]

/*: "http://m. :*/
fileprivate let constPersonalPath:String = "premium differencehttp://"
fileprivate let notiMakeData:String = "m.prison focus"

/*: "http:// :*/
fileprivate let data_crushUrl:String = "http://letter input"

/*: "3,90" :*/
fileprivate let data_everyoneValue:String = "3,90"

/*: "1990-01-01" :*/
fileprivate let show_primaValue:String = "19"
fileprivate let noti_ingredientKey:String = "90-statement-statement"

/*: "MM-dd-yyyy" :*/
fileprivate let user_femaleStr:String = "MM-ddbeyond m reason clean"
fileprivate let userRootId:String = "awake from-yyyy"
...
class ExplosiveTraceDetectionRequestModel: NSObject {
    ...
    var requestType: StartRequestType?
    ...
    override init() {
        ...
        self.requestServer = YesterdayAnotherAddrTool.share.serverUrlStr
        //: self.showErrorStatusBar = true
        self.showErrorStatusBar = true
    }

    //: func paramsContainBinaryObject() -> Bool {
    func containBinaryParamsNeighbor() -> Bool {
...
struct PulseHandyJSON: HandyJSON {
    //: var errno: Int!
...
public struct CombinedIncursionErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct QuantityensityMeasurable: HandyJSON {
    ...
    var CSConfig = BoundedModelType() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(userLiteProtectionMsg.prefix(5)) + "//stat.") + "\(data_lineUrl)" + (String(mainPositionContent.prefix(4)))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(dataSendRequestMessage) + String(dataTheoryId.suffix(6)))) && !statDomain.hasPrefix((user_yetMsg.lowercased() + String(show_mouthData))) {
                //: return "http://".appending(statDomain)
                return (String(dataSendRequestMessage) + String(dataTheoryId.suffix(6))).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(constPersonalPath.suffix(7)) + String(notiMakeData.prefix(2))) + "\(data_lineUrl)" + (String(mainPositionContent.prefix(4)))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(data_crushUrl.prefix(7))) + "\(urlDomain)"
        }
    }
...
public class ComputerSimulationThen: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [LapPriceModel] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [LapPriceModel] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [LapPriceModel] = [] // 语音收费设置
    ...
    var vipSetting = [ModelHandyJSON]() // vip皮肤
    //: var gameShowBit = 0
    ...
    var tabPageBanner = [ScrunchUpHandyJSON]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = AnotherTransformable() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class SalesSizeTransformable: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [OriginalTagModel]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [OriginalTagModel]?
    ...
    var gallery: [FactoryHouseModelType]?
    //: var status: Int = -1
    ...
    var callPopupSetting = (data_everyoneValue.capitalized) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.thanFormat(self.birthday ?? (show_primaValue.capitalized + noti_ingredientKey.replacingOccurrences(of: "statement", with: "01")), dateFormat: (String(user_femaleStr.prefix(5)) + String(userRootId.suffix(5))))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.pickUp().year else { return "0" }
...
struct FactoryHouseModelType: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct OriginalTagModel: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func quadrate(compareTo: OriginalTagModel) -> Bool {
        //: return
        return
...
struct ReplyRateTagModel: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [BindTypeModel]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [BindTypeModel]!
}

//: struct UserTagTypeModel: HandyJSON {
struct BindTypeModel: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [OriginalTagModel]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct BoundedModelType: HandyJSON {
...
struct AnotherTransformable: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
