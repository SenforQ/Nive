...
fileprivate let userPortionKey:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "FFD855" :*/
fileprivate let noti_adjustmentData:String = "FFDnatural5"
...
class StartReactiveCompatible: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - showHundredKey
                //: self.rightSlider.frame = CGRect(x: minX,
                self.rightSlider.frame = CGRect(x: minX,
                                                //: y: self.rightSlider.frame.origin.y,
                                                y: self.rightSlider.frame.origin.y,
                                                //: width: kSliderWidth,
                                                width: showHundredKey,
                ...
                self.upgradeSuiteInstallApproximately(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userPortionKey.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (noti_adjustmentData.replacingOccurrences(of: "natural", with: "85")))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: notiListData, y: 0, width: showHundredKey, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - showHundredKey, y: 0, width: showHundredKey, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.itInstall(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: notiListData, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.itInstall(0, 0, 0, 0.6)
...
extension StartReactiveCompatible {
    ...
    func cameraState() {
        ...
        self.leftSilder.frame = CGRect(x: notiListData, y: 0, width: showHundredKey, height: self.frame.size.height)
        //: self.rightSlider.frame = CGRect(x: self.frame.size.width - kSliderWidth, y: 0, width: kSliderWidth, height: self.frame.size.height)
        self.rightSlider.frame = CGRect(x: self.frame.size.width - showHundredKey, y: 0, width: showHundredKey, height: self.frame.size.height)
        //: resetIndicator()
        headIn()
    }

    //: func needShowIndicator(isShow: Bool) {
    func viewFact(isShow: Bool) {
        //: self.indicatorView.isHidden = !isShow
        self.indicatorView.isHidden = !isShow
    }

    //: func refreshSliderValidScope(startX: CGFloat) {
    func upgradeSuiteInstallApproximately(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - showHundredKey,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - showHundredKey)
    }
}
...
extension StartReactiveCompatible {
    ...
    @objc func leftmost(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.upgradeSuiteInstallApproximately(startX: videoStartX)
    ...
    @objc func labGesture(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension StartReactiveCompatible {
    ...
    private func editInclude() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(leftmost(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(labGesture(gesture:)))
    ...
    func removeXposition(_ xposition: CGFloat) {
