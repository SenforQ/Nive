...
/*: "cellID" :*/
fileprivate let show_benchPath:String = "statute"
fileprivate let app_brakeUrl:String = "case pretty on visibleellID"

/*: "init(coder:) has not been implemented" :*/
fileprivate let show_survivalMsg:[UInt8] = [0x4e,0x49,0x4e,0x53,0xf,0x44,0x48,0x43,0x42,0x55,0x1d,0xe,0x7,0x4f,0x46,0x54,0x7,0x49,0x48,0x53,0x7,0x45,0x42,0x42,0x49,0x7,0x4e,0x4a,0x57,0x4b,0x42,0x4a,0x42,0x49,0x53,0x42,0x43]

private func litePaper(meet num: UInt8) -> UInt8 {
    return num ^ 39
}

/*: "Video Call" :*/
fileprivate let mainEmptyUrl:String = "others ownVideo"

/*: "Video Call (%@ Points/Min)" :*/
fileprivate let dataStatChaseValue:String = "Videorequire past habit"
fileprivate let appPanAllRatingTitle:String = "(%@ Pvacation property"
fileprivate let dataWaveExploreName:String = "oiagos"

/*: "Voice Call" :*/
fileprivate let data_ratingName:String = "rational chin still replacement extraVoice Ca"
fileprivate let const_beforeMessage:String = "fatalfatal"

/*: "Voice Call (%@ Points/Min)" :*/
fileprivate let app_specStr:String = "Voice Calpause wait relieve combined sense"
fileprivate let dataConsequenceMistNorMsg:[Character] = ["l"," ","(","%","@"," ","P","o","i","n","t","s","/","M","i","n",")"]

/*: "#F8F8F8" :*/
fileprivate let main_discoverMsg:String = "#F8F8F8define assessment medication"

/*: "Cancel" :*/
fileprivate let showAppropriateFailureMessage:String = "Cancelproposal official distance regain"

/*: "#F5F5F5" :*/
fileprivate let const_accuracyTitle:[Character] = ["#","F","5","F","5"]
fileprivate let notiParticleData:String = "f5"

/*: "icon_lounge" :*/
fileprivate let show_willUrl:String = "imast"
fileprivate let constPictureId:String = "n_loungedetermination its document invite player"
...
class OppositePopView: UIView {
    //: var munuBlock: MunuBlock?
    var munuBlock: MunuBlock?

    //: private var type: MunuPopType = .Unknown
    private var type: IncomePopType = .Unknown // 当前展示类型
    ...
    var cellTitleColor = UIColor.aircraft()
    /// cell字体大小
    //: var cellTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cellTitleFont = UIFont.shareOff(type: .Regular, fontSize: 16)
    /// 是否有"Cancel"菜单
    //: var hasCancel = true
    var hasCancel = true
    /// cancel字体颜色
    //: var cancelColor = UIColor.appValueDetailColor()
    var cancelColor = UIColor.eigenvalueOfAMatrix()
    /// cancel字体大小
    //: var cancelTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cancelTitleFont = UIFont.shareOff(type: .Regular, fontSize: 16)
    /// 标题
    //: var title: String?
    var title: String?
    /// 标题颜色
    //: var titleColor = UIColor.appThemeColor()
    var titleColor = UIColor.retirementProgram()
    /// 标题字体大小
    //: var titleFont = UIFont.pingfangMediumFont(fontSize: 16)
    var titleFont = UIFont.pingfangBusy(fontSize: 16)

    //: var tableView = UITableView()
    var tableView = UITableView()
    //: let cellID = "cellID"
    let cellID = (show_benchPath.replacingOccurrences(of: "statute", with: "c") + String(app_brakeUrl.suffix(5)))
    ...
    fileprivate var footerViewHight: CGFloat {
        ...
        return hasCancel ? (footerViewLineHight + 50 + user_videoMsg) : user_videoMsg
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_survivalMsg.map{litePaper(meet: $0)}, encoding: .utf8)!)
    }

    //: func initwithList(cellTitleList: [String]!) {
    func divideList(cellTitleList: [String]!) {
        // 初始化
    ...
    func utilised(type: IncomePopType, videoPrice: String?, voicePrice: String?) {
        //: self.type = type
        self.type = type
        //: if self.type == .Call {
        if self.type == .Call { // 音视频通话
            ...
            if AppManager.share.appStatus == EarnLetterCustomReflectable.normal.rawValue {
                ...
                if AppManager.share.loginUserMode.sex == SharedMirrorKingCustomReflectable.male.rawValue {
                    ...
                    let videoAttrStr = String.establishPrice(VideoPrice: Int(videoPrice ?? "0") ?? 0)
                    //: let voiceAttrStr = String.setVoicePriceStr(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    let voiceAttrStr = String.voiceRemote(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    ...
                    let attributes = [.foregroundColor: UIColor.retirementProgram(), .font: UIFont.shareOff(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
                    //: let VideoCall = "Video Call".localized
                    let VideoCall = (String(mainEmptyUrl.suffix(5)) + " Call").localized
                    //: let videoStr = "Video Call (%@ Points/Min)".localizedArguments(videoPrice ?? "0")
                    let videoStr = (String(dataStatChaseValue.prefix(5)) + " Call " + String(appPanAllRatingTitle.prefix(5)) + dataWaveExploreName.replacingOccurrences(of: "ago", with: "nt") + "/Min)").queryionLocalized(videoPrice ?? "0")
                    ...
                    let VoiceCall = (String(data_ratingName.suffix(8)) + const_beforeMessage.replacingOccurrences(of: "fatal", with: "l")).localized
                    //: let voiceStr = "Voice Call (%@ Points/Min)".localizedArguments(voicePrice ?? "0")
                    let voiceStr = (String(app_specStr.prefix(9)) + String(dataConsequenceMistNorMsg)).queryionLocalized(voicePrice ?? "0")
                ...
                self.cellTitleList = [(String(mainEmptyUrl.suffix(5)) + " Call").localized, (String(data_ratingName.suffix(8)) + const_beforeMessage.replacingOccurrences(of: "fatal", with: "l")).localized]
            }
        }
    ...
    public func partnerTag() {
        ...
        tableView = UITableView(frame: CGRect(x: 0, y: userSystemKey - tableViewHight, width: show_explainId, height: tableViewHight), style: .plain)
        ...
        tableView.register(StillSpendView.self, forCellReuseIdentifier: cellID)
        ...
        if title == nil {
            ...
            tableView.glide(width: show_explainId,
...
extension OppositePopView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var cell = tableView.dequeueReusableCell(withIdentifier: cellID) as? StillSpendView
        //: if cell == nil {
        if cell == nil {
            //: cell = TitleCell(style: .default, reuseIdentifier: cellID)
            cell = StillSpendView(style: .default, reuseIdentifier: cellID)
        }
        ...
        if self.type == .Call && AppManager.share.appStatus == EarnLetterCustomReflectable.normal.rawValue {
    ...
    func tableView(_: UITableView, viewForFooterInSection _: Int) -> UIView? {
        ...
        let footerView = UIView(frame: CGRect(x: 0, y: 0, width: show_explainId, height: footerViewHight))
        //: let lineView = UIView(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: footerViewLineHight))
        let lineView = UIView(frame: CGRect(x: 0, y: 0, width: show_explainId, height: footerViewLineHight))
        //: lineView.backgroundColor = UIColor(hex: "#F8F8F8")
        lineView.backgroundColor = UIColor(hex: (String(main_discoverMsg.prefix(7))))
        ...
        btn.frame = CGRect(x: 0, y: footerViewLineHight, width: show_explainId, height: 50)
        //: footerView.addSubview(btn)
        footerView.addSubview(btn)
        //: btn.setTitle("Cancel".localized, for: .normal)
        btn.setTitle((String(showAppropriateFailureMessage.prefix(6))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(exhibit), for: .touchUpInside)
        //: return footerView
        return footerView
    }
    ...
    func tableView(_: UITableView, viewForHeaderInSection _: Int) -> UIView? {
        ...
        let headerView = UIView(frame: CGRect(x: 0, y: 0, width: show_explainId, height: headerViewHight))

        //: let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: ScreenWidth-30, height: headerViewHight))
        let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: show_explainId - 30, height: headerViewHight))
        ...
        let lineView = UIView(frame: CGRect(x: 0, y: headerViewHight - 1, width: show_explainId, height: 1))
        //: lineView.backgroundColor = UIColor.init(hex: "#F5F5F5")
        lineView.backgroundColor = UIColor(hex: (String(const_accuracyTitle) + notiParticleData.uppercased()))
...
class StillSpendView: UITableViewCell {
    ...
    lazy var vipIcon: UIImageView = {
        ...
        img.image = UIImage.scheme(name: (show_willUrl.replacingOccurrences(of: "mast", with: "co") + String(constPictureId.prefix(8))))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var lineLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(const_accuracyTitle) + notiParticleData.uppercased()))
        //: return lab
        return lab
        //: }()
    }()
