...
fileprivate let noti_exactlyName:String = "btn_shholder word pad"
fileprivate let user_sheerTitle:[Character] = ["e","r","t","B","a","c","k","g","r","o","u","n","d"]

/*: "btn_alertBackground_false" :*/
fileprivate let userContactUrl:String = "btn_arecommendation too"
fileprivate let showMayMessage:String = "definitionkg"
fileprivate let k_laterFormat:String = "_falsdomain leave global spark"
fileprivate let dataCompleteKey:[Character] = ["e"]
...
public class PulseReactiveCompatible: NSObject {
    /// 通用回调
...
public extension PulseReactiveCompatible {
    /// 适配器回调,用于给适配器参数赋值
    //: public typealias ConfigAlert = ((_ config: ShowAlertConfig) -> Void)
    typealias ConfigAlert = (_ config: EditAlertConfig) -> Void
    ...
    internal class func contentConfig(title: String? = nil,
                                      //: message: String?  = nil,
                                      message: String? = nil,
                                      ...
                                      config: EditAlertConfig? = nil)
    {
        //: customAlert(title: title,
        boxMinimize(title: title,
                    //: message: message,
                    message: message,
    ...
    @objc internal class func momentOfTruth(title: String? = nil,
    ...
    class func boxMinimize(title: String? = nil,
                           ...
                           config: EditAlertConfig? = nil)
    {
        ...
        let model = config ?? EditAlertConfig()

        //: let alertView = TalkingAlertView(title: title,
        let alertView = AnswerThen(title: title,
        ...
        //: DelineateThen.getWindow().addSubview(alertView)
        DelineateThen.premiumOperation().addSubview(alertView)
    ...
    class func carveOutOnicial() {
        //: DelineateThen.getWindow().subviews.forEach { (view) in
        DelineateThen.premiumOperation().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: AnswerThen.self) {
    ...
    class func serverBy(title: String, isTouch: Bool) {
        //: DelineateThen.getWindow().subviews.forEach { (view) in
        DelineateThen.premiumOperation().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: AnswerThen.self) {
                //: let cunrenView: TalkingAlertView = view as! TalkingAlertView
                let cunrenView: AnswerThen = view as! AnswerThen
                //: cunrenView.changeRightBtn(title: title, isTouch: isTouch)
                cunrenView.takeCamera(title: title, isTouch: isTouch)
            }
        }
    }
}
...
public class EditAlertConfig {
    ...
    public var maskType: ExtraMaskType = .color
    /// 阴影
    ...
    public var titleFont: UIFont = .shareOff(type: .Medium, fontSize: 18)
    /// alert标题字体颜色
    //: public var titleColor  = UIColor.appTitleColor()
    public var titleColor = UIColor.aircraft()
    /// alert信息字体
    //: public var textFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var textFont: UIFont = .shareOff(type: .Medium, fontSize: 16)
    //: public var textColor  = UIColor.appTitleColor()
    public var textColor = UIColor.aircraft()
    ...
    public var buttonFont: UIFont = .shareOff(type: .Medium, fontSize: 16)
    /// alert按钮字体颜色
    //: public var leftColor  = UIColor.appThemeColor()
    public var leftColor = UIColor.retirementProgram()
    //: public var rightColor = UIColor.white
    public var rightColor = UIColor.white
    //: public var rightBackgroundImage = "btn_show_alertBackground"
    public var rightBackgroundImage = (String(noti_exactlyName.prefix(6)) + "ow_al" + String(user_sheerTitle))

    //: public var rightBackgroundImagefalse = "btn_alertBackground_false"
    public var rightBackgroundImagefalse = (String(userContactUrl.prefix(5)) + "lertB" + showMayMessage.replacingOccurrences(of: "definition", with: "ac") + "round" + String(k_laterFormat.prefix(5)) + String(dataCompleteKey))
    //: public var rightFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var rightFont: UIFont = .shareOff(type: .Medium, fontSize: 16)
