...
fileprivate let mainPersonPath:[UInt8] = [0x3d,0x3a,0x3d,0x20,0x7c,0x37,0x3b,0x30,0x31,0x26,0x6e,0x7d,0x74,0x3c,0x35,0x27,0x74,0x3a,0x3b,0x20,0x74,0x36,0x31,0x31,0x3a,0x74,0x3d,0x39,0x24,0x38,0x31,0x39,0x31,0x3a,0x20,0x31,0x30]

/*: "Cancel" :*/
fileprivate let const_letterStr:String = "Cancelpopularity divide re accept"

/*: "Sure" :*/
fileprivate let app_modelData:String = "Sureperiod type recommend guidance defense"
...
class CombinedThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainPersonPath.map{$0^84}, encoding: .utf8)!)
    }

    //: private func setupView() {
    private func performTab() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(bassView))
        ...
        cancelButton.setTitle((String(const_letterStr.prefix(6))).localized, for: .normal)
        //: cancelButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        cancelButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        //: cancelButton.setTitleColor(UIColor.black, for: .normal)
        cancelButton.setTitleColor(UIColor.black, for: .normal)
        //: cancelButton.addTarget(self, action: #selector(cancelClick(button:)), for: .touchUpInside)
        cancelButton.addTarget(self, action: #selector(equalDown(button:)), for: .touchUpInside)
        ...
        confirmButton.setTitle((String(app_modelData.prefix(4))).localized, for: .normal)
        //: confirmButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        confirmButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        //: confirmButton.setTitleColor(UIColor.black, for: .normal)
        confirmButton.setTitleColor(UIColor.black, for: .normal)
        //: confirmButton.addTarget(self, action: #selector(confirmClick(button:)), for: .touchUpInside)
        confirmButton.addTarget(self, action: #selector(windowCompleteButton(button:)), for: .touchUpInside)
    ...
    @objc func equalDown(button _: UIButton) {
        //: dismissView()
        bassView()
    }

    //: @objc func confirmClick(button: UIButton) {
    @objc func windowCompleteButton(button _: UIButton) {
        //: dismissView()
        bassView()
    }

    //: func showView() {
    func qualityIndicator() {
