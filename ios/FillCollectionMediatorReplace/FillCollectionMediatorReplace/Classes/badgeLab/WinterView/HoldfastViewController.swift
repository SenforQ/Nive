...
protocol GuidanceNavigationDelegate: AnyObject {
...
class HoldfastViewController: UIViewController {
    ...
    weak var delegate: GuidanceNavigationDelegate?
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        let count = min(self.controllerItems.count, self.titleItems.count)
        //: var height = DelineateThen.getScreenHeight() - (topDistance + kTitle_H)
        var height = DelineateThen.width() - (topDistance + kTitle_H)
        ...
        //: let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: DelineateThen.getScreenWidth(), height: height)
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: DelineateThen.analysisDit(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(DelineateThen.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(DelineateThen.analysisDit()), height: 0)
        ...
        scrollView.semanticContentAttribute = CombinedLanguageManager.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        //: var rect = CGRect(x: 15, y: topDistance, width: DelineateThen.getScreenWidth()-rightSpace, height: kTitle_H)
        var rect = CGRect(x: 15, y: topDistance, width: DelineateThen.analysisDit()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: DelineateThen.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: DelineateThen.analysisDit()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = CombinedLanguageManager.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            titleBtn.addTarget(self, action: #selector(embolism), for: .touchDown)
            //: var titleWidth = 0.0
            var titleWidth = 0.0
            //: if isBisect {
            if isBisect {
                //: titleWidth = DelineateThen.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = DelineateThen.analysisDit() / Double(self.titleItems.count)
            //: } else {
            } else {
                //: titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
                titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
            }
            //: let x = LanguageManager.shared.direction == .leftToRight ? originX:(DelineateThen.getScreenWidth()-originX-titleWidth)
            let x = CombinedLanguageManager.shared.direction == .leftToRight ? originX:(DelineateThen.analysisDit()-originX-titleWidth)
...
extension HoldfastViewController {
    //: @objc private func titleButtonClick(_ sender: UIButton) {
    @objc private func embolism(_ sender: UIButton) {
        //: currentSelectedTitleButton(sender)
        tillPurchase(sender)
        //: let index = sender.tag - 1000
        let index = sender.tag - 1000
        //: let offset_x = CGFloat(index) * DelineateThen.getScreenWidth()
        let offset_x = CGFloat(index) * DelineateThen.analysisDit()
        //: controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        //: addControllerView(withIndex: index)
        streetSmartPanel(withIndex: index)
        //: seletedViewController(withIndex: index)
        shape(withIndex: index)
    }
    
    //: private func addControllerView(withIndex index: Int) {
    private func streetSmartPanel(withIndex index: Int) {
        //: if index >= children.count {
        if index >= children.count {
            //: return
            return
        }
        //: let origin_x = CGFloat(index) * DelineateThen.getScreenWidth()
        let origin_x = CGFloat(index) * DelineateThen.analysisDit()
        ...
        //: vc.view.frame = CGRect(x: origin_x, y: 0, width: DelineateThen.getScreenWidth(), height: controllerScrollView.frame.size.height)
        vc.view.frame = CGRect(x: origin_x, y: 0, width: DelineateThen.analysisDit(), height: controllerScrollView.frame.size.height)
        //: controllerScrollView.addSubview(vc.view)
        controllerScrollView.addSubview(vc.view)
    }
    
    //: private func currentSelectedTitleButton(_ button: UIButton) {
    private func tillPurchase(_ button: UIButton) {
    ...
    func third(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= DelineateThen.getScreenWidth() {
        if titleScrollView.contentSize.width <= DelineateThen.analysisDit() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - DelineateThen.getScreenWidth() * 0.5
        var offset_x = sender.center.x - DelineateThen.analysisDit() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        }
        //: let maxOffset = titleScrollView.contentSize.width - DelineateThen.getScreenWidth()
        let maxOffset = titleScrollView.contentSize.width - DelineateThen.analysisDit()
...
extension HoldfastViewController {
    ...
    func shape(withIndex index: Int) {
        //: guard index < children.count else { return }
        guard index < children.count else { return }
        //: delegate?.selectedViewController(children[index], index: index)
        delegate?.area(children[index], index: index)
    }
}

// MARK: - UIScrollViewDelegate
//: extension QYSlideNavigationViewController: UIScrollViewDelegate {
extension HoldfastViewController: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: let index = Int(controllerScrollView.contentOffset.x / DelineateThen.getScreenWidth())
        let index = Int(controllerScrollView.contentOffset.x / DelineateThen.analysisDit())
        //: if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
        if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
            //: currentSelectedTitleButton(selectedBtn)
            tillPurchase(selectedBtn)
        }
        //: addControllerView(withIndex: index)
        streetSmartPanel(withIndex: index)
        //: seletedViewController(withIndex: index)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //: let index = Int(scrollView.contentOffset.x / DelineateThen.getScreenWidth())
        let index = Int(scrollView.contentOffset.x / DelineateThen.analysisDit())
        ...
        //: let scaleR = scrollView.contentOffset.x / DelineateThen.getScreenWidth() - CGFloat(index)
        let scaleR = scrollView.contentOffset.x / DelineateThen.analysisDit() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
