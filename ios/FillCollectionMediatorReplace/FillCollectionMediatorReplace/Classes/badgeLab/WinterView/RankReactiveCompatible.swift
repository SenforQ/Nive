...
/*: "allowList" :*/
fileprivate let showNothingCanMessage:String = "ALLO"
fileprivate let userStreetMsg:[Character] = ["t"]

/*: "denyList" :*/
fileprivate let showMemorySoapMessage:String = "status injurydenyLi"
fileprivate let app_coordinatorText:[Character] = ["s","t"]

/*: "America" :*/
fileprivate let const_compareGameData:[Character] = ["A","m","e","r","i","c","a"]
...
class RankReactiveCompatible: NSObject {
    //: var reqSuccessBlock: ReqSuccessBlock!
    var reqSuccessBlock: ReqSuccessBlock!

    //: var areaArr = [TalkingAreaModel]()
    var areaArr = [DayModelType]() /// 注册允许国家列表（白名单）
    //: var blackListAreaArr = [TalkingAreaModel]()
    var blackListAreaArr = [DayModelType]() /// 注册不允许国家列表（黑名单）

    //: static let share = TalkingLocationTool()
    static let share = RankReactiveCompatible()
...
extension RankReactiveCompatible {
    ...
    private func belowGame() {
        ...
        PhotoPreserveReactiveCompatible.solution { succeed, result, _ in
            ...
            if succeed {
                ...
                let allowArray: Array = dict[(showNothingCanMessage.lowercased() + "wLis" + String(userStreetMsg))] as! [Any]
                //: let denyListArray: Array =  dict["denyList"] as! Array<Any>
                let denyListArray: Array = dict[(String(showMemorySoapMessage.suffix(6)) + String(app_coordinatorText))] as! [Any]

                //: if let datas = Array<TalkingAreaModel>.deserialize(from: allowArray as? Array) {
                if let datas = Array<DayModelType>.deserialize(from: allowArray as? Array) {
                    //: self.areaArr.append(contentsOf: (datas as? [TalkingAreaModel])!)
                    self.areaArr.append(contentsOf: (datas as? [DayModelType])!)
                }
                //: if let blackdatas = Array<TalkingAreaModel>.deserialize(from: denyListArray as? Array) {
                if let blackdatas = Array<DayModelType>.deserialize(from: denyListArray as? Array) {
                    //: self.blackListAreaArr.append(contentsOf: (blackdatas as? [TalkingAreaModel])!)
                    self.blackListAreaArr.append(contentsOf: (blackdatas as? [DayModelType])!)
                }
    ...
    static func containCountry() -> Bool {
        //: var include = false
        var include = false
        //: let simInfo = UIDevice.getDeviceSimInfo()
        let simInfo = UIDevice.satisfyBy()
        //: let arr = TalkingLocationTool.share.blackListAreaArr
        let arr = RankReactiveCompatible.share.blackListAreaArr
        //: for model in arr {
        for model in arr {
    ...
    func publishQuantityry() -> DayModelType {
        //: let simInfo = UIDevice.getDeviceSimInfo()
        let simInfo = UIDevice.satisfyBy()
        ...
        return DayModelType(areaCode: "1", areaName: (String(const_compareGameData)), countryCode: "US", url: "")
    }
}

//: class TalkingAreaModel: HandyJSON {
class DayModelType: HandyJSON {
    //: var areaCode = ""
    var areaCode = ""
    //: var countryCode = ""
    var countryCode = ""
