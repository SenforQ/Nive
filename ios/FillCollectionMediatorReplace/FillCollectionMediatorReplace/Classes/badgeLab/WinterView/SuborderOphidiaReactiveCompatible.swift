...
/*: "Select a date" :*/
fileprivate let mainMaximumColumnName:[Character] = ["S","e","l","e","c","t"," ","a"," "]
fileprivate let noti_funMessage:String = "drewarde"
...
// __CLOSE_PRINT__
//
//  SuborderOphidiaReactiveCompatible.swift
//  AbroadTalking
//
...
protocol FiniteObjectProtocol: NSObjectProtocol {
...
class SuborderOphidiaReactiveCompatible: CombinedThen, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: FiniteObjectProtocol?

    //: override func initPickView() {
    override func positionFor() {
        //: super.initPickView()
        super.positionFor()

        //: titleString = "Select a date".localized
        titleString = (String(mainMaximumColumnName) + noti_funMessage.replacingOccurrences(of: "reward", with: "at")).localized
        ...
        untilRoute(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func untilRoute(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func windowCompleteButton(button: UIButton) {
        //: super.confirmClick(button: button)
        super.windowCompleteButton(button: button)
        //: refreshPickViewData()
        pathView()
        ...
        if let day = selectDay {
            ...
            delegate?.privacyNearby(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.privacyNearby(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return distanceFamily(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func distanceFamily(year: Int, month: Int) -> Int {
