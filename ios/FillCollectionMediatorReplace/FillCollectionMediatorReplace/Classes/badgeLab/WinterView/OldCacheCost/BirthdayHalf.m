...
//: 请打开相机权限
@property (nonatomic, copy) NSString *show_inmateMsg;

//: Width
@property (nonatomic, copy) NSString *main_spanName;

//: Height
@property (nonatomic, copy) NSString *k_attentionInsideMsg;
...
+ (instancetype)sharedInstance {
    static MeetingData *instance = nil;
...
- (Byte *)MeetingDataToCache:(Byte *)data {
    ...
    Byte attach = data[1];
    int estimated = data[2];
    for (int i = estimated; i < estimated + available; i++) {
    ...
    data[estimated + available] = 0;
    return data + estimated;
}

- (NSString *)StringFromMeetingData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self MeetingDataToCache:data]];
}

//: Width
- (NSString *)main_spanName {
    if (!_main_spanName) {
        Byte value[] = {5, 1, 11, 42, 213, 97, 112, 212, 195, 20, 54, 88, 106, 101, 117, 105, 239};
        _main_spanName = [self StringFromMeetingData:value];
    }
    return _main_spanName;
}

//: 请打开相机权限
- (NSString *)show_inmateMsg {
    if (!_show_inmateMsg) {
        Byte value[] = {21, 75, 6, 227, 33, 146, 51, 250, 2, 49, 212, 222, 48, 7, 203, 50, 230, 3, 49, 231, 5, 49, 232, 206, 52, 228, 219, 130};
        _show_inmateMsg = [self StringFromMeetingData:value];
    }
    return _show_inmateMsg;
}

//: Height
- (NSString *)k_attentionInsideMsg {
    if (!_k_attentionInsideMsg) {
        Byte value[] = {6, 58, 11, 164, 42, 62, 16, 197, 96, 210, 247, 130, 159, 163, 161, 162, 174, 194};
        _k_attentionInsideMsg = [self StringFromMeetingData:value];
    }
    return _k_attentionInsideMsg;
}
...
@interface BirthdayHalf () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
                         ...
                         mediaOn:(BOOL)needYuvOutput
{
    ...
    if (self) {
        ...
        self.videoDevice = [self cameraStreetwisePositionDevice:iDevicePosition];
...
- (void)beautyLayer:(CGPoint)point late:(CGRect)frame
{
    ...
    [self strength:AVCaptureFocusModeAutoFocus
           //: exposureMode:AVCaptureExposureModeContinuousAutoExposure
           angle:AVCaptureExposureModeContinuousAutoExposure
                //: atPoint:CGPointMake(fX, fY)];
                inexpensivenessPoint:CGPointMake(fX, fY)];

}

//: - (void)focusWithMode:(AVCaptureFocusMode)focusMode exposureMode:(AVCaptureExposureMode)exposureMode atPoint:(CGPoint)point
- (void)strength:(AVCaptureFocusMode)focusMode angle:(AVCaptureExposureMode)exposureMode inexpensivenessPoint:(CGPoint)point
{

    //: [self changeDeviceProperty:^(AVCaptureDevice *captureDevice) {
    [self focusPost:^(AVCaptureDevice *captureDevice) {
...
- (void)focusPost:(void(^)(AVCaptureDevice *))propertyChange
{
...
- (void)setDevicePosition:(AVCaptureDevicePosition)devicePosition
{
    ...
    if (_devicePosition != devicePosition && devicePosition != AVCaptureDevicePositionUnspecified) {
        ...
        if (_session) {
            ...
            AVCaptureDevice *targetDevice = [self cameraStreetwisePositionDevice:devicePosition];

            //: if (targetDevice && [self judgeCameraAuthorization]) {
            if (targetDevice && [self sinceScale]) {
...
- (void)setIExpectedFPS:(int)iExpectedFPS
{
    ...
    CGFloat fWidth = [[_dataOutput.videoSettings objectForKey:[MeetingData sharedInstance].main_spanName] floatValue];
    //: CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:@"Height"] floatValue];
    CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:[MeetingData sharedInstance].k_attentionInsideMsg] floatValue];
...
- (CGRect)failure:(CGRect)rect scale:(BOOL)bScaleToFit
{
    ...
    if (self.dataOutput.videoSettings) {
        ...
        CGFloat fWidth = [[self.dataOutput.videoSettings objectForKey:[MeetingData sharedInstance].main_spanName] floatValue];
        //: CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:@"Height"] floatValue];
        CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:[MeetingData sharedInstance].k_attentionInsideMsg] floatValue];
...
- (BOOL)sinceScale
{
    ...
    if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            UIAlertController *alertView = [UIAlertController alertControllerWithTitle:@""
                                                                               //: message:@"请打开相机权限"
                                                                               message:[MeetingData sharedInstance].show_inmateMsg
...
- (AVCaptureDevice *)cameraStreetwisePositionDevice:(AVCaptureDevicePosition)position
{
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (!self.bSessionPause) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(line_strong:all:relatedness:)]) {
            //: [self.delegate captureOutput:captureOutput didOutputSampleBuffer:sampleBuffer fromConnection:connection];
            [self.delegate line_strong:captureOutput all:sampleBuffer relatedness:connection];
        }
    }
