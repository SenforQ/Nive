...
@property (nonatomic, copy) NSString *user_attachName;

//: model
@property (nonatomic, copy) NSString *appBrightUrl;

//: FaceActionEvent_Interval_12_frame
@property (nonatomic, copy) NSString *showChaseMsg;

//: key.Notification.Tap.To.Modify.Exposure.Point
@property (nonatomic, copy) NSString *user_grantNearbyTrackName;

//: hasFace
@property (nonatomic, copy) NSString *kFlipTakeText;

//: camera_focus_red
@property (nonatomic, copy) NSString *userWeightPressTitle;

//: FaceActionEvent_Interval_1_second
@property (nonatomic, copy) NSString *notiWarningId;
...
+ (NSData *)SkipDataToData:(NSString *)value {
...
- (Byte *)SkipDataToCache:(Byte *)data {
    int transfer = data[0];
    Byte daily = data[1];
    int directContrast = data[2];
    for (int i = directContrast; i < directContrast + transfer; i++) {
        int value = data[i] + daily;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    }
    data[directContrast + transfer] = 0;
    return data + directContrast;
}
...
//: hasFace
- (NSString *)kFlipTakeText {
    if (!_kFlipTakeText) {
		NSString *origin = @"073904F62F283A0D282A2C4F";
		NSData *data = [SkipData SkipDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _kFlipTakeText = [self StringFromSkipData:value];
    }
    return _kFlipTakeText;
}

//: model
- (NSString *)appBrightUrl {
    if (!_appBrightUrl) {
		NSString *origin = @"05370BA2CA56D9ADEC794336382D2E35C2";
		NSData *data = [SkipData SkipDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _appBrightUrl = [self StringFromSkipData:value];
    }
    return _appBrightUrl;
}

//: FaceActionEvent_Interval_12_frame
- (NSString *)showChaseMsg {
    if (!_showChaseMsg) {
		NSString *origin = @"211C0BFC511B7CB3136C8E2A4547492547584D5352295A495258432D525849565A4550431516434A5645514997";
		NSData *data = [SkipData SkipDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _showChaseMsg = [self StringFromSkipData:value];
    }
    return _showChaseMsg;
}

//: camera_focus_red
- (NSString *)userWeightPressTitle {
    if (!_userWeightPressTitle) {
		NSString *origin = @"10560D03AB4F2BC9126AC779C70D0B170F1C0B0910190D1F1D091C0F0EB9";
		NSData *data = [SkipData SkipDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _userWeightPressTitle = [self StringFromSkipData:value];
    }
    return _userWeightPressTitle;
}

//: FaceActionEvent_Interval_1_second
- (NSString *)notiWarningId {
    if (!_notiWarningId) {
		NSString *origin = @"212D0C3597D1C3BDE5162047193436381436473C42411849384147321C4147384549343F32043246383642413747";
		NSData *data = [SkipData SkipDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _notiWarningId = [self StringFromSkipData:value];
    }
    return _notiWarningId;
}

//: M_SenseME_Face_Video_5.3.4
- (NSString *)user_attachName {
    if (!_user_attachName) {
		NSString *origin = @"1A2A097CFBBD94BA0C2335293B44493B231B351C37393B352C3F3A3B45350B0409040AF6";
		NSData *data = [SkipData SkipDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _user_attachName = [self StringFromSkipData:value];
    }
    return _user_attachName;
}

//: key.Notification.Tap.To.Modify.Exposure.Point
- (NSString *)user_grantNearbyTrackName {
    if (!_user_grantNearbyTrackName) {
		NSString *origin = @"2D1B0AEFB84039AC0E35504A5E133354594E4B4E4846594E545313394655133954133254494E4B5E132A5D5554585A574A1335544E53594B";
		NSData *data = [SkipData SkipDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _user_grantNearbyTrackName = [self StringFromSkipData:value];
    }
    return _user_grantNearbyTrackName;
}
...
//
//#import "OldCacheCost.h"
//#import "LetterReactiveCompatible.h"
//ST_MOBILE
...
@interface FailView ()<ComeSomeone,UIGestureRecognizerDelegate>
{
...
//: @property (nonatomic, strong) MedicationLicHelper * lichelper;
@property (nonatomic, strong) MedicationLicHelper * lichelper;
//: @property (nonatomic, strong) ReplyThen * ParamUtil;
@property (nonatomic, strong) ReplyThen * ParamUtil;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        //: self.lichelper = [MedicationLicHelper share];
        self.lichelper = [MedicationLicHelper share];
        //: self.ParamUtil = [[ReplyThen alloc]init];
        self.ParamUtil = [[ReplyThen alloc]init];
        //: _currentDropCount = 5;
        ...
        if ([self.lichelper meetBackground]) {
            ...
            //: [[OldCacheCost share] getLocalModel];
            [[OldCacheCost share] occurModel];
            ...
            [NSNotificationCenter.defaultCenter addObserver:self
                                                   ...
                                                   selector:@selector(sinceRowNoti:)
                                                       //: name:@"key.Notification.Tap.To.Modify.Exposure.Point"
                                                       name:[SkipData sharedInstance].user_grantNearbyTrackName
                                                     //: object:nil];
                                                     object:nil];
            ...
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(brightEqualNotice:) name:self.ParamUtil.kNotificationSelectBeautyFilterStr object:nil];
            ...
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(postses:) name:self.ParamUtil.kNotificationBeautyFilterValueChangeStr object:nil];
...
- (instancetype)initStatusUponCrossfire:(NSString*)sessionPreset {
    ...
    if (self = [super init]) {
        ...
        if ([self.lichelper meetBackground]) {
            ...
            [self initLinguisticContext:sessionPreset];

        //: } else {
        } else {
            //: [MedicationLicHelper.share checkRemoteLicInfoWithCompletion:^(BOOL x) {
            [MedicationLicHelper.share birthSpaceWithCompletion:^(BOOL x) {
                //: if (x) {
                if (x) {
                    //: [self initFilterView:sessionPreset];
                    [self initLinguisticContext:sessionPreset];
...
- (void)initLinguisticContext:(NSString*)sessionPreset {
    //: self.layer.masksToBounds = YES;
    self.layer.masksToBounds = YES;
    //: self.clipsToBounds = YES;
    self.clipsToBounds = YES;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(brightEqualNotice:)
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(postses:)
...
- (void)cach {
    ...
    //: if ([OldCacheCost share].filterSuite.filterModelPath.length > 0) {
    if ([OldCacheCost share].filterSuite.filterModelPath.length > 0) {
        //: LetterReactiveCompatible* model = [[OldCacheCost share] getFilterDataModel];
        LetterReactiveCompatible* model = [[OldCacheCost share] filtrateHost];
        //: if (model) {
        if (model) {
...
- (void)setCurrentSessionPreset:(NSString *)currentSessionPreset {
    ...
    [self.stCamera beautyLayer:self.previewCenter late:self.previewFrame];
}
...
- (void)range:(BOOL)isFullScreen {
    ...
    [self level:width strengthenerHandle:height];
}
/// 设置视频通话预览的采集大小
...
- (void)to {
    ...
    _stCamera = [[BirthdayHalf alloc] initWithPrey:AVCaptureDevicePositionFront
                                          //: sessionPresset:self.currentSessionPreset
                                          collectionManager:self.currentSessionPreset
                                                     //: fps:fps
                                                     picture:fps
                                           //: needYuvOutput:NO];
                                           mediaOn:NO];
...
- (void)level:(CGFloat)width strengthenerHandle:(CGFloat)height {
    //: CGRect previewRect = [self.stCamera getScaleRect:CGRectMake(0 , 0 , width, height)
    CGRect previewRect = [self.stCamera failure:CGRectMake(0 , 0 , width, height)
                                                   //: scaleToFit:NO];
                                                   scale:NO];
    ...
    if (self.glPreview.superview == nil) {
        ...
        @synchronized (self) {
           ...
           _glPreview = [[PrimitivePersonView alloc] initWithAfterIncome:previewRect shared:previewContext];
        }
...
- (void)textStud {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(profileUser) name:UIApplicationWillResignActiveNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appDidEnterBackground) name:UIApplicationDidEnterBackgroundNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(maximumMain) name:UIApplicationDidEnterBackgroundNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appWillEnterForeground) name:UIApplicationWillEnterForegroundNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(listWithout) name:UIApplicationWillEnterForegroundNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appDidBecomeActive) name:UIApplicationDidBecomeActiveNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(changeStateChemicalAgentFile) name:UIApplicationDidBecomeActiveNotification object:nil];
}
...
- (void)changeStateChemicalAgentFile {
...
- (void)emplacement:(CMAccelerometerData *)accelerometerData {
...
- (st_rotate_type)press {
    ...
    [self emplacement:self.motionManager.accelerometerData];
...
- (void)bright {
    ...
    //初始化检测模块句柄
    //: NSString *strModelPath = [DelineateThen.getModelFileName stringByAppendingPathComponent:@"M_SenseME_Face_Video_5.3.4"];
    NSString *strModelPath = [DelineateThen.publishName stringByAppendingPathComponent:[SkipData sharedInstance].user_attachName];
    //: strModelPath = [strModelPath stringByAppendingPathExtension:@"model"];
    strModelPath = [strModelPath stringByAppendingPathExtension:[SkipData sharedInstance].appBrightUrl];
...
- (void)initWindow {
    ...
    [self quote:&_cvBeautifyBuffer
                                    //: w:self.imageWidth
                                    animalInSmoothness:self.imageWidth
                                    //: h:self.imageHeight
                                    running:self.imageHeight
                            //: glTexture:&_textureBeautifyOutput
                            advantageContenture:&_textureBeautifyOutput
                            //: cvTexture:&_cvTextureBeautify];
                            picture:&_cvTextureBeautify];

    //: [self setupTextureWithPixelBuffer:&_cvStickerBuffer
    [self quote:&_cvStickerBuffer
                                    //: w:self.imageWidth
                                    animalInSmoothness:self.imageWidth
                                    //: h:self.imageHeight
                                    running:self.imageHeight
                            //: glTexture:&_textureStickerOutput
                            advantageContenture:&_textureStickerOutput
                            //: cvTexture:&_cvTextureSticker];
                            picture:&_cvTextureSticker];

    //: [self setupTextureWithPixelBuffer:&_cvFilterBuffer
    [self quote:&_cvFilterBuffer
                                    //: w:self.imageWidth
                                    animalInSmoothness:self.imageWidth
                                    //: h:self.imageHeight
                                    running:self.imageHeight
                            //: glTexture:&_textureFilterOutput
                            advantageContenture:&_textureFilterOutput
                            //: cvTexture:&_cvTextureFilter];
                            picture:&_cvTextureFilter];

    //: [self setupTextureWithPixelBuffer:&_cvMakeUpBuffer
    [self quote:&_cvMakeUpBuffer
                                    //: w:self.imageWidth
                                    animalInSmoothness:self.imageWidth
                                    //: h:self.imageHeight
                                    running:self.imageHeight
                            //: glTexture:&_textureMakeUpOutput
                            advantageContenture:&_textureMakeUpOutput
                            //: cvTexture:&_cvTextureMakeup];
                            picture:&_cvTextureMakeup];

}

//: - (BOOL)setupTextureWithPixelBuffer:(CVPixelBufferRef *)pixelBufferOut
- (BOOL)quote:(CVPixelBufferRef *)pixelBufferOut
                                  //: w:(int)iWidth
                                  animalInSmoothness:(int)iWidth
                                  //: h:(int)iHeight
                                  running:(int)iHeight
                          //: glTexture:(GLuint *)glTexture
                          advantageContenture:(GLuint *)glTexture
                          //: cvTexture:(CVOpenGLESTextureRef *)cvTexture {
                          picture:(CVOpenGLESTextureRef *)cvTexture {
...
- (BOOL)rangeRound:(CVPixelBufferRef)pixelBuffer {
...
- (void)sinceRowNoti:(NSNotification *)sender {
    ...
    if ([tapGesture isKindOfClass:UITapGestureRecognizer.class]) {
        ...
        [self face:tapGesture];
    }

}

//: - (void)tapScreen:(UITapGestureRecognizer *)tapGesture {
- (void)face:(UITapGestureRecognizer *)tapGesture {
    ...
    [self.stCamera beautyLayer:point late:self.frame];
...
- (UIImageView *)focusImageView {
    ...
    if (!_focusImageView) {
        ...
        _focusImageView.image = [UIImage imageNamed:[SkipData sharedInstance].userWeightPressTitle];
...
- (void)brightEqualNotice:(NSNotification*) notice {
    //: if ([notice.object isKindOfClass:[LetterReactiveCompatible class]]) {
    if ([notice.object isKindOfClass:[LetterReactiveCompatible class]]) {
        //: LetterReactiveCompatible* model = (LetterReactiveCompatible*)notice.object;
        LetterReactiveCompatible* model = (LetterReactiveCompatible*)notice.object;
        //: [self handleFilterChanged:model];
        [self changed:model];
    }
...
//: - (void)handleFilterChanged:(LetterReactiveCompatible *)model {
- (void)changed:(LetterReactiveCompatible *)model {
...
- (void)postses:(NSNotification*) notice {
    //: LetterReactiveCompatible* model = (LetterReactiveCompatible*)notice.object;
    LetterReactiveCompatible* model = (LetterReactiveCompatible*)notice.object;
    //: if (_hFilter) {
    if (_hFilter) {
...
- (void)strip {
    ...
    //: CombinedModel *beautyValueCache = OldCacheCost.share.currentBeautySuite;
    CombinedModel *beautyValueCache = OldCacheCost.share.isShade;

    // 设置美白参数
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_WHITEN_STRENGTH value:beautyValueCache.fWhitenStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_WHITEN_STRENGTH value:beautyValueCache.fWhitenStrength];
    // 设置默认红润参数
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_REDDEN_STRENGTH value:beautyValueCache.fReddenStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_REDDEN_STRENGTH value:beautyValueCache.fReddenStrength];
    // 设置默认磨皮参数
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SMOOTH_STRENGTH value:beautyValueCache.fSmoothStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SMOOTH_STRENGTH value:beautyValueCache.fSmoothStrength];
    // 设置默认大眼参数
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ENLARGE_EYE_RATIO value:beautyValueCache.fEnlargeEyeStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ENLARGE_EYE_RATIO value:beautyValueCache.fEnlargeEyeStrength];
    // 设置默认瘦脸参数
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_FACE_RATIO value:beautyValueCache.fShrinkFaceStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_FACE_RATIO value:beautyValueCache.fShrinkFaceStrength];
    // 设置小脸参数
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_JAW_RATIO value:beautyValueCache.fShrinkJawStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_JAW_RATIO value:beautyValueCache.fShrinkJawStrength];

    //瘦脸型
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_THIN_FACE_SHAPE_RATIO value:beautyValueCache.fThinFaceShapeStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_THIN_FACE_SHAPE_RATIO value:beautyValueCache.fThinFaceShapeStrength];
    //窄脸
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_NARROW_FACE_STRENGTH value:beautyValueCache.fNarrowFaceStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_NARROW_FACE_STRENGTH value:beautyValueCache.fNarrowFaceStrength];
    //圆眼
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ROUND_EYE_RATIO value:beautyValueCache.fRoundEyeStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ROUND_EYE_RATIO value:beautyValueCache.fRoundEyeStrength];
    //下巴
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_CHIN_LENGTH_RATIO value:beautyValueCache.fChinStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_CHIN_LENGTH_RATIO value:beautyValueCache.fChinStrength];
    //额头
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_HAIRLINE_HEIGHT_RATIO value:beautyValueCache.fHairLineStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_HAIRLINE_HEIGHT_RATIO value:beautyValueCache.fHairLineStrength];
    //瘦鼻翼
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NARROW_NOSE_RATIO value:beautyValueCache.fNarrowNoseStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NARROW_NOSE_RATIO value:beautyValueCache.fNarrowNoseStrength];
    //长鼻
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NOSE_LENGTH_RATIO value:beautyValueCache.fLongNoseStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NOSE_LENGTH_RATIO value:beautyValueCache.fLongNoseStrength];
    //嘴形
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_MOUTH_SIZE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fMouthStrength]];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_MOUTH_SIZE_RATIO value:[self still:beautyValueCache.fMouthStrength]];
    //缩人中
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PHILTRUM_LENGTH_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fPhiltrumStrength]];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PHILTRUM_LENGTH_RATIO value:[self still:beautyValueCache.fPhiltrumStrength]];

    //设置对比度参数
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_CONTRAST_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fContrastStrength]];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_CONTRAST_STRENGTH value:[self still:beautyValueCache.fContrastStrength]];

    //设置饱和度参数
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SATURATION_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSaturationStrength]];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SATURATION_STRENGTH value:[self still:beautyValueCache.fSaturationStrength]];

    //锐化参数
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHARPEN_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSharpenStrength]];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHARPEN_STRENGTH value:[self still:beautyValueCache.fSharpenStrength]];

    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_APPLE_MUSLE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fAppleMusleStrength]];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_APPLE_MUSLE_RATIO value:[self still:beautyValueCache.fAppleMusleStrength]];

    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PROFILE_RHINOPLASTY_RATIO value:beautyValueCache.fProfileRhinoplastyStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PROFILE_RHINOPLASTY_RATIO value:beautyValueCache.fProfileRhinoplastyStrength];

    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_DISTANCE_RATIO value:beautyValueCache.fEyeDistanceStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_DISTANCE_RATIO value:beautyValueCache.fEyeDistanceStrength];

    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_ANGLE_RATIO value:beautyValueCache.fEyeAngleStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_ANGLE_RATIO value:beautyValueCache.fEyeAngleStrength];

    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_OPEN_CANTHUS_RATIO value:beautyValueCache.fOpenCanthusStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_OPEN_CANTHUS_RATIO value:beautyValueCache.fOpenCanthusStrength];

    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_BRIGHT_EYE_RATIO value:beautyValueCache.fBrightEyeStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_BRIGHT_EYE_RATIO value:beautyValueCache.fBrightEyeStrength];
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_DARK_CIRCLES_RATIO value:beautyValueCache.fRemoveDarkCirclesStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_DARK_CIRCLES_RATIO value:beautyValueCache.fRemoveDarkCirclesStrength];
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_NASOLABIAL_FOLDS_RATIO value:beautyValueCache.fRemoveNasolabialFoldsStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_NASOLABIAL_FOLDS_RATIO value:beautyValueCache.fRemoveNasolabialFoldsStrength];
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_WHITE_TEETH_RATIO value:beautyValueCache.fWhiteTeethStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_WHITE_TEETH_RATIO value:beautyValueCache.fWhiteTeethStrength];
    //: [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_CHEEKBONE_RATIO value:beautyValueCache.fShrinkCheekboneStrength];
    [self.ParamUtil handleRefreshValueParamBeautifyTypeWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_CHEEKBONE_RATIO value:beautyValueCache.fShrinkCheekboneStrength];

}

//: - (float)rangeMappedValueFrom:(float) rawValue {
- (float)still:(float) rawValue {
...
void writingOrigin(st_mobile_animal_face_t *src, int faceCount, st_mobile_animal_face_t *dst) {
...
void orientationOrigin(st_mobile_animal_face_t *src, int faceCount) {
    //: if (faceCount > 0) {
    if (faceCount > 0) {
        //: for (int i = 0; i < faceCount; ++i) {
        for (int i = 0; i < faceCount; ++i) {
...
- (void)hide:(CMSampleBufferRef)originalSampleBuffer
                       //: resultPixelBuffer:(CVPixelBufferRef)resultPixelBufffer {
                       localFactor:(CVPixelBufferRef)resultPixelBufffer {
    ...
    if ( originalSampleBuffer && resultPixelBufffer ) {
        ...
        if (tmpDelegate && [tmpDelegate respondsToSelector:@selector(camera:listEnd:)]) {
            ...
            if (sampleBuffer) {
                ...
                [tmpDelegate camera:self listEnd:sampleBuffer];
            }
...
- (CVPixelBufferRef)available:(CVPixelBufferRef)pixelBuffer {
...
- (void)line_strong:(AVCaptureOutput *)captureOutput all:(CMSampleBufferRef)sampleBuffer relatedness:(AVCaptureConnection *)connection {
    ...
    st_rotate_type stMobileRotate = [self press];
    ...
    if (_hDetector) {
        ...
        if (self.iCurrentAction > 0) {
            ...
            if (_faceFrameCount == 12) { // 第12帧时
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:[SkipData sharedInstance].showChaseMsg object:nil userInfo: @{[SkipData sharedInstance].kFlipTakeText: @(hasFace)}];
            }
            ...
            if (_faceFrameCount%18 == 0) { // 每秒发送一次
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:[SkipData sharedInstance].notiWarningId object:nil userInfo: @{[SkipData sharedInstance].kFlipTakeText: @(hasFace)}];
            }
        }
    }
    ...
    if (faceCount > 0) {
        ...
        writingOrigin(_detectResult1, faceCount, newDetectResult1);
    }
    ...
    dispatch_async(self.renderQueue, ^{
       ...
       BOOL isTextureOriginReady = [self rangeRound:pixelBuffer];
       ...
       // ST_MOBILE 以下为滤镜部分
        ...
        if (callBackFlag) [self hide:sampleBuffer localFactor:resultPixelBufffer];
        ...
        if (faceCount > 0) {
            ...
            orientationOrigin(newDetectResult1, faceCount);
        }
        //: if (callBackFlag) {
        if (callBackFlag) {
            //: @synchronized (self) {
            @synchronized (self) {
                ...
                [self.glPreview tap:textureResult];
            }
        }
