...
/*: "filterSuite" :*/
fileprivate let app_hungValue:String = "project boy priority signfilterSui"
fileprivate let showScriptMessage:String = "phone"

/*: "customSuite" :*/
fileprivate let constFillIconStr:String = "workust"

/*: "App/OldCacheCost" :*/
fileprivate let mainIsolateStr:String = "App/Sfoundation multi two required"
fileprivate let main_instructionData:[Character] = ["t","e","r","C","a","c","h","e","C","o","s","t"]
...
//: @objcMembers public class OldCacheCost: NSObject, NSSecureCoding {
@objcMembers public class OldCacheCost: NSObject, NSSecureCoding {
    //: var curBeautySuite: BeautyItemType?
    var curBeautySuite: CoordinateBotSendable?
    //: var curFeature: STEffectsType?
    var curFeature: CoordinateCustomReflectable?
    /// 用户的滤镜设置
    //: public var filterSuite: CombinedModel?
    public var filterSuite: CombinedModel?
    /// 滤镜列表模型数组
    //: var filterModelArray = Array<LetterReactiveCompatible>.init()
    var filterModelArray = Array<LetterReactiveCompatible>.init()

    //: var customSuite =  CombinedModel.init()
    var customSuite = CombinedModel() // 用户自定义的美颜

    //: static public let share = OldCacheCost()
    public static let share = OldCacheCost()
    ...
    public required init?(coder: NSCoder) {
        ...
        let filDict = coder.decodeObject(forKey: (String(app_hungValue.suffix(9)) + showScriptMessage.replacingOccurrences(of: "phone", with: "te")))
        //: let custDict =  coder.decodeObject(forKey: "customSuite")
        let custDict = coder.decodeObject(forKey: (constFillIconStr.replacingOccurrences(of: "work", with: "c") + "omSuite"))
        //: if filDict != nil {
        if filDict != nil {
            //: filterSuite = try! CombinedModel.init(dictionary: filDict as? [AnyHashable: Any])
            filterSuite = try! CombinedModel(dictionary: filDict as? [AnyHashable: Any])
        }
        //: if custDict != nil {
        if custDict != nil {
            //: customSuite = try! CombinedModel.init(dictionary: custDict as? [AnyHashable: Any])
            customSuite = try! CombinedModel(dictionary: custDict as? [AnyHashable: Any])
        }
    }
    ...
    public func encode(with coder: NSCoder) {
        ...
        coder.encode(filDict, forKey: (String(app_hungValue.suffix(9)) + showScriptMessage.replacingOccurrences(of: "phone", with: "te")))
        //: coder.encode(custDict, forKey: "customSuite")
        coder.encode(custDict, forKey: (constFillIconStr.replacingOccurrences(of: "work", with: "c") + "omSuite"))
    }
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(shareSender(sender:)), name: show_sceneValue, object: nil)
    }
...
//: @objc extension OldCacheCost {
@objc extension OldCacheCost {
    /// 获取缓存
    //: public func getLocalModel() {
    public func occurModel() {
        //: let path = self.cachePath()
        let path = self.pathDraft()
        //: if FileManager.default.fileExists(atPath: path) {
        if FileManager.default.fileExists(atPath: path) {
            //: var obj: OldCacheCost?
            var obj: OldCacheCost?
            ...
            if #available(iOS 11.0, *) {
                ...
                do {
                    ...
                    let data = try Data(contentsOf: URL(fileURLWithPath: path))
                    //: if let model: OldCacheCost =  try NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(data) as? OldCacheCost {
                    if let model: OldCacheCost = try NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(data) as? OldCacheCost {
                        //: obj  = model
                        obj = model
                    }
                ...
                //: let model: OldCacheCost? = NSKeyedUnarchiver.unarchiveObject(withFile: path) as? OldCacheCost
                let model: OldCacheCost? = NSKeyedUnarchiver.unarchiveObject(withFile: path) as? OldCacheCost
                //: if  model != nil {
                if model != nil {
                    //: obj  = model
    ...
    //: public func currentBeautySuite() -> CombinedModel {
    public func isShade() -> CombinedModel {
    ...
    //: func checkAllSuite() {
    func underMobile() {
        //: customSuite  = CombinedModel.init().initWith(suiteType: .Face)
        customSuite = CombinedModel().doingInitReward(suiteType: .Face)

        //: if filterSuite == nil {
        if filterSuite == nil {
            //: filterSuite  = CombinedModel.init()
            filterSuite = CombinedModel()
        }
    }
    ...
    func pathDraft() -> String {
        //: let userId = 0
        let userId = 0

        //: let documentDictPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        let documentDictPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        //: let appDirectoryPath = (documentDictPath as NSString).appendingPathComponent("App/OldCacheCost")
        let appDirectoryPath = (documentDictPath as NSString).appendingPathComponent((String(mainIsolateStr.prefix(5)) + "TFil" + String(main_instructionData)))
    ...
    func redClub(curBeautySuite: CoordinateBotSendable) {
    ...
    func announcementEvent() {
        ...
        self.customSuite.viaFor()
        //: self.curBeautySuite = .Face
        self.curBeautySuite = .Face
        //: self.filterSuite = CombinedModel.init()
        self.filterSuite = CombinedModel()
    }

    // notification
    //: func didReceiveFeatureValueChange(sender: NSNotification) {
    func shareSender(sender: NSNotification) {
        //: var cache: CombinedModel?
        var cache: CombinedModel?
        //: cache = self.customSuite
        cache = self.customSuite
        //: let featureModel: LetterReactiveCompatible = sender.object as! LetterReactiveCompatible
        let featureModel: LetterReactiveCompatible = sender.object as! LetterReactiveCompatible
        //: handleFeatureValueChangeWith(beautySuite: cache ?? CombinedModel.init(), featureModel: featureModel)
        justModel(beautySuite: cache ?? CombinedModel(), featureModel: featureModel)
    }

    //: func handleFeatureValueChangeWith(beautySuite: CombinedModel, featureModel: LetterReactiveCompatible) {
    func justModel(beautySuite: CombinedModel, featureModel: LetterReactiveCompatible) {
        //: if (self.curBeautySuite == .Face) {
        if self.curBeautySuite == .Face {
            //: if featureModel.isKind(of: LetterReactiveCompatible.self) == true {
            if featureModel.isKind(of: LetterReactiveCompatible.self) == true {
                // [0,1]
    ...
    /// -------初始化滤镜资源
    //: func reloadFilterData(model: LetterReactiveCompatible) {
    func reloadNeedModel(model: LetterReactiveCompatible) {
    ...
    //: public func getFilterDataModel() -> LetterReactiveCompatible? {
    public func filtrateHost() -> LetterReactiveCompatible? {
    ...
    func fullCustomer(rawValue: Float) -> Float {
    ...
    //: func initFilterData() {
    func placeError() {
        //: let orignArray  = LetterReactiveCompatible.init().arrayOfFeatureModelWith(categoryType: .Filter)
        let orignArray = LetterReactiveCompatible().clew(categoryType: .Filter)

        //: if (orignArray.count > 0) {
        if orignArray.count > 0 {
            //: let model = orignArray.first
            let model = orignArray.first
            //: self.filterModelArray = NSMutableArray.init(array: model?.subFilterItemsArr ?? [LetterReactiveCompatible].init()) as! [LetterReactiveCompatible]
            self.filterModelArray = NSMutableArray(array: model?.subFilterItemsArr ?? [LetterReactiveCompatible].init()) as! [LetterReactiveCompatible]
        }
    }
}
