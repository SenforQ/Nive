...
@interface SkipEditionData : NSObject

+ (instancetype)sharedInstance;

@end

@implementation SkipEditionData

+ (instancetype)sharedInstance {
    static SkipEditionData *instance = nil;
...
- (Byte *)SkipEditionDataToCache:(Byte *)data {
    int edition = data[0];
    Byte state = data[1];
    int nearby = data[2];
    for (int i = nearby; i < nearby + edition; i++) {
    ...
    data[nearby + edition] = 0;
    return data + nearby;
}

- (NSString *)StringFromSkipEditionData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self SkipEditionDataToCache:data]];
}

//: STDisplayShader
- (NSString *)dataServiceMessage {
    /* static */ NSString *dataServiceMessage = nil;
    if (!dataServiceMessage) {
        Byte value[] = {15, 5, 10, 187, 153, 174, 24, 208, 90, 153, 78, 79, 63, 100, 110, 107, 103, 92, 116, 78, 99, 92, 95, 96, 109, 125};
        dataServiceMessage = [self StringFromSkipEditionData:value];
    }
...
@implementation PrimitivePersonView
...
- (instancetype)initWithAfterIncome:(CGRect)frame shared:(EAGLContext *)context
{
    ...
    if (self) {
        ...
        [self occurrent:[[SkipEditionData sharedInstance] dataServiceMessage]
                //: fragmentShader:@"STDisplayShader"
                vertex:[[SkipEditionData sharedInstance] dataServiceMessage]
                    //: forProgram:&stDisplayProgram];
                    direction:&stDisplayProgram];
...
- (void)dealloc
{
    ...
    [self fractureSizeFramebuffer];
}

//: - (BOOL)createFramebuffers
- (BOOL)featureFramebuffers
{
    //: glEnable(0x0DE1);
    glEnable(0x0DE1);
    //: glDisable(0x0B71);
...
- (BOOL)occurrent:(NSString *)vertexShaderName
          //: fragmentShader:(NSString *)fragmentShaderName
          vertex:(NSString *)fragmentShaderName
              //: forProgram:(GLuint *)programPointer;
              direction:(GLuint *)programPointer;
{
    ...
    *programPointer = glCreateProgram();
    ...
    if (![self location:&vertexShader price:0x8B31 shader:k_withTheData]) {
    ...
    if (![self location:&fragShader price:0x8B30 shader:userExpectedKeyFormat]) {
...
- (BOOL)location:(GLuint *)shader price:(GLenum)type shader:(const char *)str
{
    //: GLint status;
    GLint status;
    //: const GLchar *source;
