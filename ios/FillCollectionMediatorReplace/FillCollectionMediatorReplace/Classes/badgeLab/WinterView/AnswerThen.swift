...
fileprivate let kEveryoneUrl:String = "btn_metender numb dramatic"
fileprivate let user_historyJumpPath:String = "oincidentinue"

/*: "init(coder:) has not been implemented" :*/
fileprivate let main_betterConFormat:[UInt8] = [0xd4,0xd9,0xd4,0xdf,0x93,0xce,0xda,0xcf,0xd0,0xdd,0xa5,0x94,0x8b,0xd3,0xcc,0xde,0x8b,0xd9,0xda,0xdf,0x8b,0xcd,0xd0,0xd0,0xd9,0x8b,0xd4,0xd8,0xdb,0xd7,0xd0,0xd8,0xd0,0xd9,0xdf,0xd0,0xcf]

fileprivate func temporaryPresent(group num: UInt8) -> UInt8 {
...
class AnswerThen: UIView {
    //: private var alertConfig: ShowAlertConfig
    private var alertConfig: EditAlertConfig
    ...
    private lazy var leftBtn: UIButton = {
        ...
        leftBtn.layer.borderColor = UIColor.retirementProgram().cgColor
        //: leftBtn.layer.borderWidth = 1
        leftBtn.layer.borderWidth = 1
        //: leftBtn.addTarget(self, action: #selector(leftClick), for: .touchUpInside)
        leftBtn.addTarget(self, action: #selector(clickEye), for: .touchUpInside)
        //: return leftBtn
        return leftBtn
        //: }()
    }()
    ...
    private lazy var rightBtn: UIButton = {
        ...
        let image = UIImage.scheme(name: alertConfig.rightBackgroundImage).withTintColor(.retirementProgram())
        //: rightBtn.setBackgroundImage(image, for: .normal)
        rightBtn.setBackgroundImage(image, for: .normal)
        //: rightBtn.addTarget(self, action: #selector(rightClick), for: .touchUpInside)
        rightBtn.addTarget(self, action: #selector(clickTo), for: .touchUpInside)
        //: return rightBtn
        return rightBtn
        //: }()
    }()
         ...
         config: EditAlertConfig)
    {
        ...
        if leftBtnTitle == nil && leftBtnAttributedTitle == nil {
            ...
            alertConfig.rightBackgroundImage = (String(kEveryoneUrl.prefix(6)) + "_back_c" + user_historyJumpPath.replacingOccurrences(of: "incident", with: "nt"))
        }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_betterConFormat.map{temporaryPresent(group: $0)}, encoding: .utf8)!)
    }

    //: func changeRightBtn(title: String, isTouch: Bool) {
    func takeCamera(title: String, isTouch: Bool) {
        ...
        if !isTouch {
            ...
            rightBtn.setBackgroundImage(UIImage.scheme(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            //: } else {
        } else {
            //: let image = UIImage.BundleImageNamed(name: alertConfig.rightBackgroundImage).withTintColor(.appThemeColor())
            let image = UIImage.scheme(name: alertConfig.rightBackgroundImage).withTintColor(.retirementProgram())
