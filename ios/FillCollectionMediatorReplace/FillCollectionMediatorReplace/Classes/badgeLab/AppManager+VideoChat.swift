...
/*: "type" :*/
fileprivate let dataBucketMsg:String = "typstand"

/*: "You are currently on a call, please try again later" :*/
fileprivate let showRedMsg:[UInt8] = [0xa5,0x93,0x89,0xdc,0x9d,0x8e,0x99,0xdc,0x9f,0x89,0x8e,0x8e,0x99,0x92,0x88,0x90,0x85,0xdc,0x93,0x92,0xdc,0x9d,0xdc,0x9f,0x9d,0x90,0x90,0xd0,0xdc,0x8c,0x90,0x99,0x9d,0x8f,0x99,0xdc,0x88,0x8e,0x85,0xdc,0x9d,0x9b,0x9d,0x95,0x92,0xdc,0x90,0x9d,0x88,0x99,0x8e]

/*: "onCallResume" :*/
fileprivate let mainEndName:String = "engage far layonCal"
fileprivate let k_entityOuterName:String = "hi selection flexiblelResume"
...
extension AppManager {
    ...
    func coverSituation(info: [String: Any], situation: YesterdayChatSituation = .Start) {
        //: let json = JSON(info)
        let json = JSON(info)
        //: let type = json["type"].intValue
        let type = json[(dataBucketMsg.replacingOccurrences(of: "stand", with: "e"))].intValue
...
extension AppManager {
    ...
    private func topTalk(info: [String: Any], resumeTalk: Bool) {
        //: if !resumeTalk && TalkingSocketManager.shared.isMutual {
        if !resumeTalk && MigrationSocketDelegate.shared.isMutual {
            //: self.func__showStatusBarErrorMsg(showMsg: "You are currently on a call, please try again later".localized)
            self.diskTerms(showMsg: String(bytes: showRedMsg.map{$0^252}, encoding: .utf8)!.localized)
            //: return
            return
        }
        ...
        if reasonWindow()!.isKind(of: TelecastingStartErrorDelegate.self) ||
            //: currentViewController()!.isKind(of: TalkingVoiceChatViewController.self) ||
            reasonWindow()!.isKind(of: StillViewController.self) ||
            //: currentViewController()!.isKind(of: TalkingVideoInitivCallVC.self) {
            reasonWindow()!.isKind(of: BotThen.self)
        {
            //: return
            return
        }
        ...
        if (WhiteThen.shared as! WhiteThen).inviteCallView != nil {
            //: (TalkingApplication.shared as! TalkingApplication).inviteCallView?.closeView()
            (WhiteThen.shared as! WhiteThen).inviteCallView?.sinceCuttingEdgeProperty()
        }
        ...
        if type == 0 { // 视频通话
            ...
            (vc as! TelecastingStartErrorDelegate).chatModel = SimulationChatModel.videoSituation(info: info, situation: resumeTalk ? .Resume : .Recieved)
            //: } else {
        } else { // 语音通话
            //: vc = TalkingVoiceChatViewController()
            vc = StillViewController()
            ...
            (vc as! StillViewController).chatModel = SimulationChatModel.videoSituation(info: info, situation: resumeTalk ? .Resume : .Recieved)
        }

        //: let callResume = json["onCallResume"].stringValue
        let callResume = json[(String(mainEndName.suffix(5)) + String(k_entityOuterName.suffix(7)))].stringValue
        ...
        if resumeTalk, callResume.count > 0, callResume == "1" { // 恢复通话
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
                ...
                self.thinkOf(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: resumeTalk, resumeData: info)
            }
            //: } else {
        } else { // 构建音视频通话
            //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: resumeTalk, resumeData: info)
    ...
    private func thinkOf(roomVC: UIViewController, isVideoTalk: Bool, resumeTalk: Bool, resumeData: [String: Any]) {
        //: guard var currentVc = currentViewController() else { return }
        guard var currentVc = reasonWindow() else { return }
        ...
        if currentVc.isKind(of: UINavigationController.self) {
            ...
            //: let rootVc = DelineateThen.getWindow().rootViewController as! UITabBarController
            let rootVc = DelineateThen.premiumOperation().rootViewController as! UITabBarController
        ...
        if !isVideoTalk {
            ...
            talkClass = NSStringFromClass(StillViewController.self)
            //: } else {
        } else {
            //: talkClass = NSStringFromClass(TalkingVideoChatViewController.self)
            talkClass = NSStringFromClass(TelecastingStartErrorDelegate.self)
        }
        //: var chatIndex = -1
        var chatIndex = -1
        //: var index = -1
        ...
        if isVideoTalk {
            ...
            var currentChatVc: TelecastingStartErrorDelegate?
            //: let pushChatVc = roomVC as! TalkingVideoChatViewController
            let pushChatVc = roomVC as! TelecastingStartErrorDelegate

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVideoChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? TelecastingStartErrorDelegate
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    ...
                    currentChatVc?.data(resumeData: resumeData)
                }
                //: return
                return
            }
            ...
            if navVc!.isKind(of: LetterClingReactiveCompatible.self) {
                //: let nav = navVc as! TalkingNavigationController
                let nav = navVc as! LetterClingReactiveCompatible
            ...
            var currentChatVc: StillViewController?
            //: let pushChatVc = roomVC as! TalkingVoiceChatViewController
            let pushChatVc = roomVC as! StillViewController

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVoiceChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? StillViewController
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    //: currentChatVc?.updateInfoViaResumeTalkData(resumeData: resumeData)
                    currentChatVc?.modifyWithDataKey(resumeData: resumeData)
                }
                //: return
                return
            }
