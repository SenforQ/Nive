...
/*: "recharge" :*/
fileprivate let user_conditionMessage:String = "chanceech"

/*: "recharge_ :*/
fileprivate let app_electMessage:String = "approximately difference factor enterpriserecharge_"

/*: ".png" :*/
fileprivate let constThenTitle:String = "personal collection excess posit world.png"

/*: .png" :*/
fileprivate let appFragmentName:[Character] = [".","p","n","g"]

/*: ".jpg" :*/
fileprivate let userMaximumId:[Character] = [".","j","p","g"]

/*: .jpg" :*/
fileprivate let main_currentMessage:String = "thin north picture cycle.jpg"
...
class ExtendedView: UIButton {
    ...
    var imageAlignment: ChevalGlassSum = .left
...
extension UIButton {
    ...
    func soundDeadline(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        minimumAppear(interval: interval, topOffset: 0)
    }
    ...
    func minimumAppear(interval: CGFloat, topOffset: CGFloat) {
    ...
    func paperCapacity(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func whenOf(color: UIColor, forState: UIControl.State) {
    ...
    func mpCharacter(urlStr: String,
                     //: placeImg: UIImage? = UIImage.placeImgSquare(),
                     placeImg: UIImage? = UIImage.endAdd(),
                     ...
                     loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: constDailyTitle.thanBasic(), with: data_lineUrl)
    ...
    func glyceroliseHeadVisualizationFrameStr(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((user_conditionMessage.replacingOccurrences(of: "chance", with: "r") + "arge")) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (user_conditionMessage.replacingOccurrences(of: "chance", with: "r") + "arge"), with: (String(app_electMessage.suffix(9))) + "\(mainAppId)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.mpCharacter(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(constThenTitle.suffix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(constThenTitle.suffix(4))), with: "_\(mainAppId)" + (String(appFragmentName)))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(userMaximumId))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(userMaximumId)), with: "_\(mainAppId)" + (String(main_currentMessage.suffix(4))))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.mpCharacter(urlStr: newUrl, placeImg: nil)
    }
