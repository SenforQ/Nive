...
- (UIImage *)motionEnableLoad {
    ...
    return [UIImage imageWithData:[self beauty]];
}
...
- (UIImage *)compress {
    ...
    return [self textCompres:YES];
}

//: - (UIImage *)wcTimelineCompress {
- (UIImage *)burn {
    //: return [self wcCompres:NO];
    return [self textCompres:NO];
}
...
- (UIImage *)textCompres:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self storage:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self need:size];
...
- (CGSize)storage:(Boolean)isSession {
...
+ (NSData *)sample:(UIImage *)image trademark:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG publication:scalesize toScale:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG keyTarget:(fileSize*1024)];
    }
...
- (UIImage*)change:(CGSize)targetSize{
...
- (UIImage *)publication:(CGSize)targetSize toScale:(BOOL)isScale{
...
- (NSData *)keyTarget:(NSInteger)fileSize{
