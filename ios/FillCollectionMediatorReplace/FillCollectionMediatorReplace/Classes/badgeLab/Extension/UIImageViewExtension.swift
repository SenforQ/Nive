...
/*: "add_header_cookies" :*/
fileprivate let data_directionThinPlaceContent:String = "add_heato split border day"
fileprivate let user_legitimateValue:[Character] = ["d"]
fileprivate let kUnlessId:[Character] = ["e","r","_","c","o","o","k","i","e","s"]
...
public extension UIImageView {
    ...
    @objc func scrubFinish(urlStr: String,
    ...
    internal func commonClose(urlStr: String,
                              //: placeImg: UIImage = UIImage.placeImgSquare(),
                              placeImg: UIImage = UIImage.endAdd(),
    ...
    @objc func orientation(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: constDailyTitle.thanBasic(), with: data_lineUrl)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.holderImage())
    }
    ...
    @objc func linemen(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: YesterdayAnotherAddrTool.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(data_directionThinPlaceContent.prefix(7)) + String(user_legitimateValue) + String(kUnlessId)))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.endAdd(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
