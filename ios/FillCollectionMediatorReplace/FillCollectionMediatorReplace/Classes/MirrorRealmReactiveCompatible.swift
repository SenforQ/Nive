...
/*: "🌛 🌛 enter  :*/
fileprivate let notiOperationChamberMsg:[Character] = ["🌛"," ","\u{0001f31b}"," "]
fileprivate let data_inputFormat:[Character] = ["e","n","t","e","r"," "]

/*: "🌛 🌛 deinit  :*/
fileprivate let appFindMsg:[Character] = ["\u{0001f31b}"," ","\u{0001f31b}"," ","d","e","i","n","i","t"," "]

/*: "nav_back_black_nor" :*/
fileprivate let userPingBitListenTitle:String = "come gap withnav_b"
fileprivate let userTicketMediumData:String = "ack_norforehead mutual we"
...
// __CLOSE_PRINT__
//
//  MirrorRealmReactiveCompatible.swift
//  AbroadTalking
//
...
class MirrorRealmReactiveCompatible: UIViewController {
    //: var hideNavi = false
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        printLog(message: (String(notiOperationChamberMsg) + String(data_inputFormat)) + "\(self)")
    }
    ...
    deinit {
        ...
        printLog(message: (String(appFindMsg)) + "\(self)")
    }
}

//: extension TalkingBaseViewController: UIGestureRecognizerDelegate {
extension MirrorRealmReactiveCompatible: UIGestureRecognizerDelegate {
    //: func addTapGestureRecognizer() {
    func decideRecognizer() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(func__TapGestureRecognizer))
        let tap = UITapGestureRecognizer(target: self, action: #selector(lateRecognizer))
        //: tap.delegate = self
        tap.delegate = self
    ...
    @objc func lateRecognizer(_: UITapGestureRecognizer) {
...
extension MirrorRealmReactiveCompatible {
    /// 侧滑返回手势开关【直播、音视频通话及后续页面不支持返回手势】
    /// - Parameter isOpen: 是否开启
    //: func popGesture(isOpen: Bool) {
    func successPick(isOpen: Bool) {
        //: var isOpen = isOpen
        var isOpen = isOpen
        ...
        if let navArr = navigationController?.viewControllers {
            ...
            if navArr.contains(where: {
                ...
                $0.isKind(of: EmigrationViewController.self) ||
                    //: $0.isKind(of: TalkingLivePullStreamsViewController.self) ||
                    $0.isKind(of: CreaseThen.self) ||
                    //: $0.isKind(of: TalkingVoiceChatViewController.self) ||
                    $0.isKind(of: StillViewController.self) ||
                    //: $0.isKind(of: TalkingVideoChatViewController.self)
                    $0.isKind(of: TelecastingStartErrorDelegate.self)
                //: }) {
            }) {
                //: isOpen = false
                isOpen = false
...
extension UIViewController {
    ...
    func featureTo() -> UIBarButtonItem {
        ...
        let item = UIBarButtonItem(image: UIImage.scheme(name: (String(userPingBitListenTitle.suffix(5)) + "ack_bl" + String(userTicketMediumData.prefix(7)))), style: .plain, target: self, action: #selector(snapshotDrag))
