...
fileprivate let constProgramPath:[UInt8] = [0xd8,0xdf,0xd8,0xc5,0x99,0xd2,0xde,0xd5,0xd4,0xc3,0x8b,0x98,0x91,0xd9,0xd0,0xc2,0x91,0xdf,0xde,0xc5,0x91,0xd3,0xd4,0xd4,0xdf,0x91,0xd8,0xdc,0xc1,0xdd,0xd4,0xdc,0xd4,0xdf,0xc5,0xd4,0xd5]

private func legalInstrument(find num: UInt8) -> UInt8 {
    return num ^ 177
}

/*: "#30D00B" :*/
fileprivate let data_homeValue:String = "approximate"
fileprivate let const_fadeKey:[Character] = ["3","0","D","0","0","B"]

/*: "icon_receivedcalls" :*/
fileprivate let constJoinFloorName:String = "pause association bound mystery hangicon_rec"
fileprivate let noti_beatUrl:String = "EIVE"

/*: "Received calls" :*/
fileprivate let k_offStr:String = "technology"
fileprivate let notiDotUrl:[Character] = ["e","c","e","i","v","e","d"," ","c","a","l","l","s"]

/*: "icon_dialedcalls" :*/
fileprivate let kOffeRegionId:[Character] = ["i","c","o","n","_","d","i","a","l","e","d","c","a"]
fileprivate let const_tarAccountPositData:[Character] = ["l","l","s"]

/*: "Dialed calls" :*/
fileprivate let data_foodValue:String = "passage cotton my appropriateDi"
fileprivate let notiLabMsg:String = "decideled"
...
class InsideViewCell: UITableViewCell {
    //: var currenModel = TalkingVideoCallRecordModel()
    var currenModel = SupportModelType()
    //: var seleteIndex: IndexPath?
    var seleteIndex: IndexPath?
    //: open weak var delegate: WhoLikeDelegate?
    open weak var delegate: AfterThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constProgramPath.map{legalInstrument(find: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var iconBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(but), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var durationLB: UILabel = {
        ...
        label.font = .shareOff(type: .Medium, fontSize: 10)
        //: label.textColor = .appValueDetailColor()
        label.textColor = .eigenvalueOfAMatrix()
    ...
    lazy var onlineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (data_homeValue.replacingOccurrences(of: "approximate", with: "#") + String(const_fadeKey)))
...
extension InsideViewCell {
    //: func setVideoCallCell(model: TalkingVideoCallRecordModel) {
    func placeModel(model: SupportModelType) {
        //: currenModel = model
        currenModel = model
        ...
        if !model.headPicFrame.isEmptyString {
            ...
            iconBorderBtn.glyceroliseHeadVisualizationFrameStr(urlStr: model.headPicFrame)
            //: } else {
        } else {
        ...
        if model.callStatus == 0 {
            ...
            rcordImg.image = UIImage.scheme(name: (String(constJoinFloorName.suffix(8)) + noti_beatUrl.lowercased() + "dcalls"))
            //: rcordLB.text = "Received calls".localized
            rcordLB.text = (k_offStr.replacingOccurrences(of: "technology", with: "R") + String(notiDotUrl)).localized
            //: } else {
        } else {
            //: rcordImg.image = UIImage.BundleImageNamed(name: "icon_dialedcalls")
            rcordImg.image = UIImage.scheme(name: (String(kOffeRegionId) + String(const_tarAccountPositData)))
            //: rcordLB.text = "Dialed calls".localized
            rcordLB.text = (String(data_foodValue.suffix(2)) + notiLabMsg.replacingOccurrences(of: "decide", with: "a") + " calls").localized
        }
