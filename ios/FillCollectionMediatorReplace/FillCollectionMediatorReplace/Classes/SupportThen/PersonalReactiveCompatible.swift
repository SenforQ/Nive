...
fileprivate let const_extendContent:[UInt8] = [0x4a,0x4d,0x4a,0x57,0xb,0x40,0x4c,0x47,0x46,0x51,0x19,0xa,0x3,0x4b,0x42,0x50,0x3,0x4d,0x4c,0x57,0x3,0x41,0x46,0x46,0x4d,0x3,0x4a,0x4e,0x53,0x4f,0x46,0x4e,0x46,0x4d,0x57,0x46,0x47]

private func whiteSlide(formal num: UInt8) -> UInt8 {
    return num ^ 35
}

/*: "Reply" :*/
fileprivate let data_prisonMsg:[Character] = ["R","e","p","l","y"]

/*: "#F95151" :*/
fileprivate let notiSurgeryMatterKey:String = "fine car connection#F95151"
...
// __CLOSE_PRINT__
//
//  PersonalReactiveCompatible.swift
//  AbroadTalking
//
...
let dataMinUrl = (dataInfoTrustName + 80.0)
//: class TalkingMessageNotificationView: UIView {
class PersonalReactiveCompatible: UIView {
    //: var startDismissBlock: ((_ model: TalkingConversationModel) -> Void)?
    var startDismissBlock: ((_ model: MiddleThen) -> Void)? // 开始移除视图Block
    //: var finishDismissBlock: ((_ view: TalkingMessageNotificationView) -> Void)?
    var finishDismissBlock: ((_ view: PersonalReactiveCompatible) -> Void)? // 完成移除视图Block
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_extendContent.map{whiteSlide(formal: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var nickNameLabel: UILabel = {
        ...
        label.font = .shareOff(type: .Medium, fontSize: 15)
    ...
    private lazy var replyBtn: UIButton = {
        ...
        btn.titleLabel?.font = .shareOff(type: .Medium, fontSize: 15)
        //: btn.setTitle("Reply".localized, for: .normal)
        btn.setTitle((String(data_prisonMsg)).localized, for: .normal)
        //: btn.setTitleColor(UIColor(hex: "#F95151"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(notiSurgeryMatterKey.suffix(7)))), for: .normal)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
...
extension PersonalReactiveCompatible {
    /// 展示视图，5s后自动移除
    //: func show(_ model: TalkingConversationModel?) {
    func executeIncome(_ model: MiddleThen?) {
    ...
    func margin(needAlpha: Bool = true) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                if needAlpha {
                    ...
                    self.frame.origin.y = -dataMinUrl
                }
                //: } completion: { _ in
            } completion: { _ in
    ...
    @objc private func outcome() {
        ...
        if currModel.chatType == .service {
            ...
            //: ElementEarnReactiveCompatible.share.func__pushToPriveteChatVC(chatID: currModel.targetId)
            ElementEarnReactiveCompatible.share.totalAgain(chatID: currModel.targetId)
        }
    }

    /// 滑动手势
    //: @objc private func swipeGestureEvent(gesture: UISwipeGestureRecognizer) {
    @objc private func canPokeGesture(gesture: UISwipeGestureRecognizer) {
...
extension PersonalReactiveCompatible {
    ...
    func noneMouth() {
        ...
        if let userInfo = currModel.gj_userInfo {
            ...
            headImgView.commonClose(urlStr: userInfo.headPic, placeImg: UIImage.backgroundDown(sex: userInfo.sex))
            //: if !userInfo.headPicFrame.isEmptyString {
            if !userInfo.headPicFrame.isEmptyString {
                //: iconBorder.isHidden = false
                iconBorder.isHidden = false
                //: iconBorder.setHeadFrameUrlImage(urlStr: userInfo.headPicFrame)
                iconBorder.glyceroliseHeadVisualizationFrameStr(urlStr: userInfo.headPicFrame)
                //: } else {
            } else {
    ...
    private func recognise() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(outcome))
        //: self.addGestureRecognizer(tap)
        self.addGestureRecognizer(tap)

        //: let swipe = UISwipeGestureRecognizer(target: self, action: #selector(swipeGestureEvent))
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(canPokeGesture))
        //: swipe.direction = .up
        swipe.direction = .up
