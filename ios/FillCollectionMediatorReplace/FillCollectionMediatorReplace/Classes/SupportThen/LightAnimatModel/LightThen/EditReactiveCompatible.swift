...
fileprivate let user_asideData:String = "cheek his changeCacheG"
fileprivate let user_tensionId:String = "generalect"
fileprivate let userHiddenId:[Character] = ["e","y"]

/*: "下载动画文件" :*/
fileprivate let data_examineUrl:String = "\u{4e0b}载\u{52a8}画\u{6587}件"

/*: "加载本地动画文件" :*/
fileprivate let appPaperMessage:String = "\u{52a0}载\u{672c}\u{5730}动"
fileprivate let notiPadMessage:String = "画\u{6587}\u{4ef6}"

/*: "Cache_GiftEffect/" :*/
fileprivate let noti_averageOriginTitle:String = "style composition up spaceCache"
fileprivate let user_approvalName:String = "ect/service extension better sweet lay"

/*: "GJSGVA" :*/
fileprivate let show_opNoKey:String = "GbagGVA"

/*: "GJVap" :*/
fileprivate let mainNameFireMessage:String = "GJVapnarrow outer actual"

/*: "config.json" :*/
fileprivate let notiRetainSheerText:[Character] = ["c","o","n","f","i","g",".","j"]
fileprivate let dataDiskRecordingName:[Character] = ["s","o","n"]

/*: "app. :*/
fileprivate let showFadeMessage:String = "app.tag nature see"

/*: .com" :*/
fileprivate let data_askDownMsg:String = "share deal.com"

/*: "key" :*/
fileprivate let appNorBlastKey:String = "kzzy"

/*: "zip" :*/
fileprivate let mainSustainLimitedUrl:[Character] = ["z","i","p"]

/*: " fail to unzip file" :*/
fileprivate let show_campSortMysteryFormat:[Character] = [" ","f","a","i","l"," ","t","o"," ","u"]
fileprivate let dataLiveValue:[Character] = ["n","z","i","p"," ","f","i","l","e"]

/*: "RSA 解密失败" :*/
fileprivate let const_cuteEssayFormat:[Character] = ["R","S","A"]
fileprivate let mainFunPath:String = " 解密失败"

/*: "fail to unzip script file" :*/
fileprivate let main_titleName:String = "fail extend fit"
fileprivate let appInformationKey:String = "remind clingzip "
fileprivate let constGuidancePath:String = "stockcr"
fileprivate let show_directionKey:String = "ipt fileking time time"

/*: "gift" :*/
fileprivate let noti_lastPath:[Character] = ["g","i","f","t"]

/*: "%@%@effect_%ld" :*/
fileprivate let notiAffectMsg:String = "quantity admin disabled message%@%@eff"
fileprivate let dataJawValue:String = "car"
fileprivate let mainWhatStayStr:[Character] = ["c","t","_","%","l","d"]

/*: "%@%@effect_%ld_unzipTest/" :*/
fileprivate let showAfterEndKey:[Character] = ["%","@","%","@","e"]
fileprivate let k_goData:[Character] = ["f","f","e","c","t","_","%","l","d","_","u","n","z","i","p","T","e","s","t","/"]

/*: "%@%@effect_%ld_unzip/" :*/
fileprivate let appPermitReplaceMsg:[Character] = ["%","@","%","@","e","f","f","e","c"]
fileprivate let mainDitName:String = "technical aircraft black distributiont_%ld"
...
typealias CompletionBlock = (_ success: Bool, _ effectModel: WheezeHandyJSON?) -> Void

//: typealias ErrorBlock = (_ error: NSError?) -> Void
typealias ErrorBlock = (_ error: NSError?) -> Void

//: class TalkingGiftAnimatTool: NSObject {
class EditReactiveCompatible: NSObject {
    //: let TalkingGiftAnimatCacheKey  = "CacheGiftEffectCacheKey"
    let TalkingGiftAnimatCacheKey = (String(user_asideData.suffix(6)) + "iftEf" + user_tensionId.replacingOccurrences(of: "general", with: "f") + "CacheK" + String(userHiddenId))

    //: @objc static let shared = TalkingGiftAnimatTool()
    @objc static let shared = EditReactiveCompatible()
    ...
    func underbelly(effectMsgModel: IndoorsTransformable, completion: @escaping CompletionBlock) {
        //: if checkNewVersionWithAnimatMsgModel(model: effectMsgModel) {
        if computeCell(model: effectMsgModel) {
            //: printLog(message: "下载动画文件")
            printLog(message: (data_examineUrl))
            //: downloadAnimat(effectMsgModel: effectMsgModel) { [weak self] error in
            rewardBlock(effectMsgModel: effectMsgModel) { [weak self] error in
                ...
                self.fullComplete(effectMsgModel: effectMsgModel, complete: completion)
            }
            //: } else {
        } else {
            //: printLog(message: "加载本地动画文件")
            printLog(message: (appPaperMessage + notiPadMessage))
            //: self.loadLocalAnimatConfig(effectMsgModel: effectMsgModel, complete: completion)
            self.fullComplete(effectMsgModel: effectMsgModel, complete: completion)
        }
    }
    ...
    func modelBean(fileName: String, model: IndoorsTransformable) -> String {
        //: let scriptDirectory = fetchScriptDirectory(animatMsgModel: model)
        let scriptDirectory = whiteModel(animatMsgModel: model)
    ...
    func assetOnly() {
        ...
        edge(path: imageDesign())
    }

    // 动效资源本地的缓存目录路径
    //: func fetchScriptDirectory(animatMsgModel: TalkingAnimatMsgModel) -> String {
    func whiteModel(animatMsgModel: IndoorsTransformable) -> String {
        //: if animatMsgModel.eid == 0 {
        if animatMsgModel.eid == 0 {
            //: return String.init(format: "%@/%@/", giftEffectDirectory(), animatMsgModel.effectUrl.md5()!)
            return String(format: "%@/%@/", imageDesign(), animatMsgModel.effectUrl.md5()!)
            //: } else {
        } else {
            //: return String.init(format: "%@/%zd/", giftEffectDirectory(), animatMsgModel.eid)
            return String(format: "%@/%zd/", imageDesign(), animatMsgModel.eid)
        }
    }

    //: func giftEffectDirectory() -> String {
    func imageDesign() -> String {
        //: let subPath = "Cache_GiftEffect/"
        let subPath = (String(noti_averageOriginTitle.suffix(5)) + "_GiftEff" + String(user_approvalName.prefix(4)))
    ...
    func photoSunnaPath(filePath: String) -> String {
    ...
    func appraise(model: IndoorsTransformable) -> Bool {
        //: return !checkNewVersionWithAnimatMsgModel(model: model)
        return !computeCell(model: model)
    }

    //: func getNewAnimatClassName(oldName: String) -> String {
    func quoteScale(oldName: String) -> String {
        //: var newName = oldName
        var newName = oldName
        //: if newName.hasPrefix("GJSGVA") {
        if newName.hasPrefix((show_opNoKey.replacingOccurrences(of: "bag", with: "JS"))) {
            //: newName = String(describing: TalkingSGVAAnimationView.self)
            newName = String(describing: LightThen.self)
            //: } else if newName.hasPrefix("GJVap") {
        } else if newName.hasPrefix((String(mainNameFireMessage.prefix(5)))) {
            //: newName = String(describing: TalkingVapAnimationView.self)
            newName = String(describing: FinishReactiveCompatible.self)
        }
        //: return newName
        return newName
    }
...
extension EditReactiveCompatible {
    ...
    func fullComplete(effectMsgModel: IndoorsTransformable, complete: @escaping CompletionBlock) {
        //: let scriptDirectory = fetchScriptDirectory(animatMsgModel: effectMsgModel)
        let scriptDirectory = whiteModel(animatMsgModel: effectMsgModel)
        //: let scriptPath = (scriptDirectory as NSString).appendingPathComponent("config.json")
        let scriptPath = (scriptDirectory as NSString).appendingPathComponent((String(notiRetainSheerText) + String(dataDiskRecordingName)))
        ...
        let dic = NSDictionary.beforeJson(withJsonString: configString!) ?? NSDictionary()
        //: let giftAmodel = TalkingGiftAnimatModel.init()
        let giftAmodel = WheezeHandyJSON()
        //: let effectModel = giftAmodel.setAnimatModek(dic: dic)
        let effectModel = giftAmodel.animat(dic: dic)
        ...
        className = quoteScale(oldName: className)
        //: let clsName = PodspecName + "." + className
        let clsName = data_resolutionTitle + "." + className
    ...
    func computeCell(model: IndoorsTransformable) -> Bool {
    ...
    func rewardBlock(effectMsgModel: IndoorsTransformable, errorBlock: @escaping ErrorBlock) {
        ...
        HouseThen.boardStream(giftUrl: effectMsgModel.effectUrl, path: sunnaBy(model: effectMsgModel)) { [weak self] t in
            //: guard let self = self else {return}
            guard let self = self else { return }
            //: DispatchQueue.global().async {
            DispatchQueue.global().async {
                //: self.afterDownloadGiftAnimat(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
                self.shade(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
            }
        }
    }

    //: func afterDownloadGiftAnimat(effectMsgModel: TalkingAnimatMsgModel, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
    func shade(effectMsgModel: IndoorsTransformable, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
        //: if !isSucess {
        if !isSucess {
            //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.DownLoadFailed.rawValue))
            errorBlock(NSError(domain: (String(showFadeMessage.prefix(4))) + "\(data_lineUrl)" + (String(data_askDownMsg.suffix(4))), code: EarnStrideable.DownLoadFailed.rawValue))
            //: return
            return
            ...
            let downloadPath = sunnaBy(model: effectMsgModel)
            //: let unzipVerifyDir = unzipVerifyDirectory(model: effectMsgModel)
            let unzipVerifyDir = effect(model: effectMsgModel)
            ...
            if verifyUnzipSucc {
                ...
                for filename in unzippedFiles {
                    //: if filename == "key" {
                    if filename == (appNorBlastKey.replacingOccurrences(of: "zz", with: "e")) {
                        // encrypted md5 file
                        //: keyFilePath = (unzipVerifyDir as NSString).appendingPathComponent(filename)
                        keyFilePath = (unzipVerifyDir as NSString).appendingPathComponent(filename)
                        //: } else if (filename as NSString).pathExtension == "zip" {
                    } else if (filename as NSString).pathExtension == (String(mainSustainLimitedUrl)) {
                        // script file
                ...
                printLog(message: (String(show_campSortMysteryFormat) + String(dataLiveValue)))
                //: isFailed = true
                isFailed = true
                //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                errorBlock(NSError(domain: (String(showFadeMessage.prefix(4))) + "\(data_lineUrl)" + (String(data_askDownMsg.suffix(4))), code: EarnStrideable.UnzipFailed.rawValue))
            }
            ...
            if !isFailed {
                ...
                do {
                    ...
                    let md5Data: Data = RuleFzrsa.last(NSData(contentsOfFile: keyFilePath) as Data?, windowProfile: userBusyUrl)
                    ...
                    let md5 = photoSunnaPath(filePath: scriptZipFilePath)
                    ...
                    if decryptMD5 != md5, FileManager.default.fileExists(atPath: keyFilePath) {
                        ...
                        errorBlock(NSError(domain: (String(showFadeMessage.prefix(4))) + "\(data_lineUrl)" + (String(data_askDownMsg.suffix(4))), code: EarnStrideable.VerifyFailed.rawValue))
                    }
                    ...
                    printLog(message: (String(const_cuteEssayFormat) + mainFunPath.capitalized))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.VerifyFailed.rawValue))
                    errorBlock(NSError(domain: (String(showFadeMessage.prefix(4))) + "\(data_lineUrl)" + (String(data_askDownMsg.suffix(4))), code: EarnStrideable.VerifyFailed.rawValue))
            ...
            if !isFailed {
                ...
                let unzipTmpDir = thenModel(model: effectMsgModel)
                ...
                //: else {
                    ...
                    printLog(message: (String(main_titleName.prefix(5)) + "to un" + String(appInformationKey.suffix(4)) + constGuidancePath.replacingOccurrences(of: "stock", with: "s") + String(show_directionKey.prefix(8))))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                    errorBlock(NSError(domain: (String(showFadeMessage.prefix(4))) + "\(data_lineUrl)" + (String(data_askDownMsg.suffix(4))), code: EarnStrideable.UnzipFailed.rawValue))
    ...
    func asWithout(type: OldBinaryMagnitude) -> String {
        //: var tmpIdentify = ""
        var tmpIdentify = ""
        //: switch type {
        switch type {
        //: case .Gift:     tmpIdentify =  "gift"
        case .Gift: tmpIdentify = (String(noti_lastPath))
        //: break
        //: default:
        default:
            //: break
    ...
    func sunnaBy(model: IndoorsTransformable) -> String {
        ...
        let downloadTmpPath = String(format: (String(notiAffectMsg.suffix(7)) + dataJawValue.replacingOccurrences(of: "car", with: "e") + String(mainWhatStayStr)), NSTemporaryDirectory(), asWithout(type: model.effectType), model.eid)
        //: return downloadTmpPath
        return downloadTmpPath
    }

    //: func unzipVerifyDirectory(model: TalkingAnimatMsgModel)->String {
    func effect(model: IndoorsTransformable) -> String {
        ...
        let unzipVerifyDirectory = String(format: (String(showAfterEndKey) + String(k_goData)), NSTemporaryDirectory(), asWithout(type: model.effectType), model.eid)
        //: return unzipVerifyDirectory
        return unzipVerifyDirectory
    }

    //: func unzipTmpDirectory(model: TalkingAnimatMsgModel)->String {
    func thenModel(model: IndoorsTransformable) -> String {
        ...
        let unzipTmpDirectory = String(format: (String(appPermitReplaceMsg) + String(mainDitName.suffix(5)) + "_unzip/"), NSTemporaryDirectory(), asWithout(type: model.effectType), model.eid)
