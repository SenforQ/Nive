...
fileprivate let main_arrowStretchValue:[UInt8] = [0x3,0x4,0x3,0x1e,0x42,0x9,0x5,0xe,0xf,0x18,0x50,0x43,0x4a,0x2,0xb,0x19,0x4a,0x4,0x5,0x1e,0x4a,0x8,0xf,0xf,0x4,0x4a,0x3,0x7,0x1a,0x6,0xf,0x7,0xf,0x4,0x1e,0xf,0xe]

private func earlySick(toward num: UInt8) -> UInt8 {
    return num ^ 106
}

/*: "iosSvgaEffect" :*/
fileprivate let user_takeDitStr:[Character] = ["i","o","s","S","v","g","a","E","f"]
fileprivate let k_portionTitle:String = "femoviet"

/*: "iosEffect" :*/
fileprivate let showParticularName:String = "key scope caseiosEff"
fileprivate let data_lightPath:String = "mysteryct"

/*: "entityId" :*/
fileprivate let dataResumeValue:[Character] = ["e","n","t","i","t","y","I","d"]
...
class ExtendedViewDelegate: UIView {
    //: var currentEffectMsgModel: TalkingAnimatMsgModel?
    var currentEffectMsgModel: IndoorsTransformable?
    ...
    var animatedView: UniversalItemView?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: ReflectionThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_arrowStretchValue.map{earlySick(toward: $0)}, encoding: .utf8)!)
    }
...
extension ExtendedViewDelegate {
    //: func playAnimation(infoDic: Dictionary<String, Any>) {
    func outAnimation(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(user_takeDitStr) + k_portionTitle.replacingOccurrences(of: "movie", with: "c"))] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(String(showParticularName.suffix(6)) + data_lightPath.replacingOccurrences(of: "mystery", with: "e"))] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(String(dataResumeValue))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = rankWith(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func rankWith(msgModel: Any?) -> Bool {
        ...
        if msgModel is IndoorsTransformable {
            //: effectMsgModel = msgModel as? TalkingAnimatMsgModel
            effectMsgModel = msgModel as? IndoorsTransformable
            //: } else if msgModel is TalkingGiftMsgAnimatModel {
        } else if msgModel is LightAnimatModel {
            //: let giftMsgModel = msgModel as! TalkingGiftMsgAnimatModel
            let giftMsgModel = msgModel as! LightAnimatModel
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.bequestOffd(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = IndoorsTransformable()
            ...
            effectMsgModel = IndoorsTransformable()
            ...
            if giftMsgModel.showType == CreaseMultiplierTarget.myStery.rawValue || giftMsgModel.showType == CreaseMultiplierTarget.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: IndoorsTransformable = self.effectMsgModelArray[i] as! IndoorsTransformable
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? IndoorsTransformable())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? IndoorsTransformable(), at: index)
                }
            ...
            factor(effectMsgModel: effectMsgModel!)
        }
...
extension ExtendedViewDelegate {
    ...
    func lengthWith() {
        ...
        self.animatedView?.minimum()
    }

    //: func resume() {
    func writtenMatterResume() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = firstborn() else { return }
                //: firstShow(effectMsgModel: removeModel)
                factor(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.fieldEnableAnnouncement()
        }
    ...
    func firstborn() -> IndoorsTransformable? {
        //: if (self.effectMsgModelArray.count == 0) {
        if self.effectMsgModelArray.count == 0 {
            //: return TalkingAnimatMsgModel.init()
            return IndoorsTransformable()
        }
        ...
        if effectMsgModel is IndoorsTransformable {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! IndoorsTransformable
        ...
        return effectMsgModel as? IndoorsTransformable
    }
}
...
extension ExtendedViewDelegate {
    ...
    func factor(effectMsgModel: IndoorsTransformable) {
        ...
        self.delegate?.share(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        EditReactiveCompatible.shared.underbelly(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.enableicial(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? WheezeHandyJSON())
            //: })
        })
    }
    ...
    func enableicial(effectMsgModel: IndoorsTransformable, success: Bool, effectModel: WheezeHandyJSON) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.unionize(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    knapAlongReverberation(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func untilParadigm(effectMsgModel: IndoorsTransformable, effectModel: WheezeHandyJSON) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.brake()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.dark()
        ...
        className = EditReactiveCompatible.shared.quoteScale(oldName: className ?? "")
        ...
        let realClass = AnimatedViewClass as! UniversalItemView.Type
...
extension ExtendedViewDelegate: ExhibitThen {
    ...
    func presentPanoramicViewItemBackwashVictoryPointResponsePosition(effectItemView: UniversalItemView, success: Bool) {
        ...
        self.delegate?.rank(effectView: self, obj: effectItemView.effectMsgModel ?? IndoorsTransformable(), success: success)
    }
    ...
    func create(effectItemView: UniversalItemView) {
        ...
        self.delegate?.adminObj(effectView: self, obj: effectMsgModel!)
    }
}
...
extension ExtendedViewDelegate {
    ...
    func effectVia() {
        ...
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.brake() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.dark() // clean animation data
