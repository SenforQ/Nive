...
/*: "num" :*/
fileprivate let k_nameFormat:String = "nuexhibit"
...
@objc public enum CreaseMultiplierTarget: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct LightAnimatModel: HandyJSON {
    //: init() {
    init() {}
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// CreaseMultiplierTarget
    ...
    mutating func bequestOffd(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct StatusTransformable: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (k_nameFormat.replacingOccurrences(of: "exhibit", with: "m"))
    }
}
