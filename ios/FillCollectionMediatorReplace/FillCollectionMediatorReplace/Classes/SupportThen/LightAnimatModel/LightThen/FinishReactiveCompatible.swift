...
fileprivate let mainDecideContent:[UInt8] = [0xbf,0xb8,0xbf,0xa2,0xfe,0xb5,0xb9,0xb2,0xb3,0xa4,0xec,0xff,0xf6,0xbe,0xb7,0xa5,0xf6,0xb8,0xb9,0xa2,0xf6,0xb4,0xb3,0xb3,0xb8,0xf6,0xbf,0xbb,0xa6,0xba,0xb3,0xbb,0xb3,0xb8,0xa2,0xb3,0xb2]

private func cloudScan(yes num: UInt8) -> UInt8 {
    return num ^ 214
}

/*: "%@.mp4" :*/
fileprivate let data_bearUrl:[Character] = ["%","@",".","m","p","4"]

/*: "http" :*/
fileprivate let data_governId:String = "HTTP"
...
class FinishReactiveCompatible: UniversalItemView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainDecideContent.map{cloudScan(yes: $0)}, encoding: .utf8)!)
    }
    ...
    public lazy var vapPlayerView: UIView? = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(doingly))
    ...
    override func drownPull() {
        ...
        let path = EditReactiveCompatible.shared.modelBean(fileName: self.giftEffectModel?.mainFile ?? "", model: self.effectMsgModel ?? IndoorsTransformable())

        //: if  path.count > 0 {
        if path.count > 0 {
            //: let mp4Path = String.init(format: "%@.mp4", path)
            let mp4Path = String(format: (String(data_bearUrl)), path)
            ...
            self.delegate?.create(effectItemView: self)
        }
    }
...
extension FinishReactiveCompatible: HWDMP4PlayDelegate {
    ...
    func loadVapImage(withURL urlStr: String!, context _: [AnyHashable: Any]!, completion completionBlock: VAPImageCompletionBlock!) {
        ...
        DispatchQueue.main.async {
            ...
            if urlStr.hasPrefix((data_governId.lowercased())) {
