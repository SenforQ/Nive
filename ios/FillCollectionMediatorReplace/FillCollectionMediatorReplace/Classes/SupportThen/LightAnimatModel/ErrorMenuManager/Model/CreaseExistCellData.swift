...
/*: "#BAFBFF" :*/
fileprivate let kIslandFormat:String = "#BAFBFFoutput resign flow extra plus"
...
class CreaseExistCellData: NSObject {
    ...
    var model = LanguageHandyJSON()

    //: func caculateMsgHeight(model: TalkingVideoCallDanmuModel) -> TalkingVideoCallDanmuModel {
    func sampleDimension(model: LanguageHandyJSON) -> LanguageHandyJSON {
        //: self.model = model
        self.model = model
        /// 普通文本消息
    ...
    func factorModel(model: LanguageHandyJSON) -> LanguageHandyJSON {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func readyDecide() {
        ...
        let paraStyle = capitalExpenditure()
        ...
        let name = noticeBy(text: firstStr, font: UIFont.pingfangBusy(fontSize: 14), color: UIColor(hex: (String(kIslandFormat.prefix(7))))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.pingfangMediumFont(fontSize: 14), color: UIColor.white)
        let content = noticeBy(text: self.model.content, font: UIFont.pingfangBusy(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        missiveUnder(attribText: self.model.msgAttribText)
    }
    ...
    func recoverBean() {
        ...
        let content = noticeBy(text: self.model.transContent, font: UIFont.pingfangBusy(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func capitalExpenditure() -> NSMutableParagraphStyle {
    ...
    func noticeBy(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = capitalExpenditure()
        //: return attribute
        return attribute
    }
    ...
    func missiveUnder(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = userInfoTitle - 36
