...
/*: "call_bgm" :*/
fileprivate let main_lackSickValue:String = "compose practicalcall_bgm"

/*: "call_response_bgm" :*/
fileprivate let main_implementUrl:[Character] = ["c","a","l","l","_","r","e","s","p","o","n","s","e","_"]
fileprivate let constExitId:String = "increasegm"

/*: "invitePop_call_bgm" :*/
fileprivate let user_teeMsg:String = "spherenvsphere"
fileprivate let const_offerStr:[Character] = ["p","_","c","a","l","l","_","b","g","m"]
...
class ExtraPlayListener: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = RateEnsureBasicType.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func linksman(type: RateEnsureBasicType) -> ExtraPlayListener {
        //: let player = TalkingCallBGMPlayer()
        let player = ExtraPlayListener()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.getReserve()
...
extension ExtraPlayListener {
    ...
    private func likeEach(notify: Notification) {
    ...
    private func counterpretationOrBuildNotify(notify: Notification) {
    ...
    private func getReserve() {
        //: var fileName = "call_bgm"
        var fileName = (String(main_lackSickValue.suffix(8)))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(main_implementUrl) + constExitId.replacingOccurrences(of: "increase", with: "b"))
            //: } else if self.type == .invitePop {
        } else if self.type == .invitePop {
            //: fileName = "invitePop_call_bgm"
            fileName = (user_teeMsg.replacingOccurrences(of: "sphere", with: "i") + "tePo" + String(const_offerStr))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = FinishLetterThen.default.typeDown(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension ExtraPlayListener: TXVodPlayListener {
