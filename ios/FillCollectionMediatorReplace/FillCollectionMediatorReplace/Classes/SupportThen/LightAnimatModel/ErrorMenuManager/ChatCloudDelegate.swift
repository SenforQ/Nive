...
/*: "mf/call/GetUserInfo" :*/
fileprivate let kWhenPath:[Character] = ["m","f","/","c","a","l","l","/","G","e","t","U","s","e","r"]
fileprivate let show_resolutionStr:String = "Infogirl clothes carve dimension"

/*: "uid" :*/
fileprivate let mainMatterMsg:[UInt8] = [0xd2,0xc6,0xc1]

fileprivate func trustThread(publicly num: UInt8) -> UInt8 {
...
/*: "TRTC 随机视频匹配中." :*/
fileprivate let notiCheckMessage:[Character] = ["T","R","T","C"," ","随","机","视","频","匹","配"]
fileprivate let appSkipFormat:String = "hi."

/*: "TRTC  :*/
fileprivate let const_mentallyNoticeUrl:String = "phase strength yellowTRTC "

/*: 视频通话. logId: :*/
fileprivate let show_surePath:String = "\u{89c6}频通话. "
fileprivate let user_bassThenTarPath:String = ":"

/*: "TRTC 视频通话销毁. logId： :*/
fileprivate let const_familyId:String = "TRTC \u{89c6}"
fileprivate let constDelayKey:String = " logId\u{ff1a}"

/*: "TRTC 视频通话错误. errCode: :*/
fileprivate let show_wrapStr:[Character] = ["T","R","T","C"," ","视","频","通","话","错","\u{8bef}","."," ","e","r"]
fileprivate let kThickData:String = "simultaneously replace outcome attributerCode:"

/*: , errMsg: :*/
fileprivate let dataSeriesStr:[Character] = [","," ","e","r","r","M","s","g",":"]

/*: "TRTC 视频通话SDK收到警告. warningCode: :*/
fileprivate let constConstraintKey:String = "streak exactlyTRTC "
fileprivate let kSelectCorrectUrl:String = "SDK收drag gesture closed injury"
fileprivate let showBeamDynamicsText:String = "exception tone present war"
fileprivate let appSameName:[Character] = ["d","e",":"]

/*: , warningMsg: :*/
fileprivate let kRevenueId:String = "vacation superior max path element, wa"
fileprivate let userOthersUrl:[Character] = ["g","M","s","g",":"]

/*: "TRTC 视频通话进入房间. result: :*/
fileprivate let noti_exceptionValue:[Character] = ["T","R","T","C"," ","\u{89c6}","频","通","\u{8bdd}","\u{8fdb}","入","房","间"]
fileprivate let appDismissValue:[Character] = ["."," ","r","e","s","u","l","t",":"]

/*: , logId： :*/
fileprivate let kSizeText:String = ", logId：constraint air valid"

/*:  加入了房间" :*/
fileprivate let user_personEffMsg:[Character] = [" ","加","入","了","房","间"]

/*:  离开了房间" :*/
fileprivate let userHerTitle:String = " \u{79bb}开\u{4e86}房间"
...
enum MigrationSchemeRenderPosition {
    //: case selfOnBg
...
class ClingEarnChatConfig: NSObject {
    ...
    var remoteCameraStatus = StartCameraStatus.Normal // 对方摄像头状态
}

//: class TalkingVideoChatManager: NSObject {
class ChatCloudDelegate: NSObject {
...
extension ChatCloudDelegate {
    /// 开始视频速配（男）
    /// - Parameters:
    ///   - uid: 接收者的id
    ///   - completion: 回调
    //: class func req_callGetUserInfo(uid: String, completion: @escaping FinishBlock) {
    class func notice(uid: String, completion: @escaping FinishBlock) {
        //: let reqModel = TalkingRequestModel.init()
        let reqModel = ExplosiveTraceDetectionRequestModel()
        //: reqModel.requestPath = "mf/call/GetUserInfo"
        reqModel.requestPath = (String(kWhenPath) + String(show_resolutionStr.prefix(4)))
        //: reqModel.requestType = .GET
        reqModel.requestType = .GET
        //: reqModel.params = ["uid": uid]
        reqModel.params = [String(bytes: mainMatterMsg.map{trustThread(publicly: $0)}, encoding: .utf8)!: uid]
...
extension ChatCloudDelegate {
    ...
    func originalUser(_ openCamera: Bool) {
        ...
        self.pastVideo(isOpen: openCamera)
    }
    ...
    func belowAsset() {
        ...
        DispatchQueue.main.async {
            ...
            self.getRolling(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
...
extension ChatCloudDelegate {
    ...
    private func pastVideo(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard notiMaxMsg == true else {
            //: if isOpen {
            if isOpen {
...
extension ChatCloudDelegate {
    ...
    func errorSpend() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(midNotif(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(writeAnimal(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(whoFragment(notif:)),
    ...
    @objc private func midNotif(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func writeAnimal(notif: Notification?) {
        //: if notif == nil {
    ...
    @objc func whoFragment(notif: Notification) {
...
extension ChatCloudDelegate {
    ...
    func getMovingContact(isMatch: Bool = false) {
        //: if isMatch {
        if isMatch {
            //: UploadLogTool.writeLog(msg: "TRTC 随机视频匹配中.")
            NiftyLogTool.paragraphFromRank(msg: (String(notiCheckMessage) + appSkipFormat.replacingOccurrences(of: "hi", with: "中")))
            //: } else {
        } else {
            //: UploadLogTool.writeLog(msg: "TRTC \((infoModel?.videoStage == .Waiting) ? "发起":"收到")视频通话. logId:\(infoModel?.logId ?? 0).")
            NiftyLogTool.paragraphFromRank(msg: (String(const_mentallyNoticeUrl.suffix(5))) + "\((infoModel?.videoStage == .Waiting) ? "发起" : "收到")" + (show_surePath + "logId" + user_bassThenTarPath.capitalized) + "\(infoModel?.logId ?? 0).")
        }
        //: if self.infoModel?.videoStage == .Waiting {
        if self.infoModel?.videoStage == .Waiting {
            //: startTRTCPreview(smallWindow: false)
            getRolling(smallWindow: false)
    ...
    func getRolling(smallWindow: Bool) {
        //: guard SenseTime_Use == true else {
        guard notiMaxMsg == true else {
    ...
    private func noteSight() {
        //: UploadLogTool.writeLog(msg: "TRTC 视频通话销毁. logId：\(infoModel?.logId ?? 0).")
        NiftyLogTool.paragraphFromRank(msg: (const_familyId + "频\u{901a}话销毁." + constDelayKey) + "\(infoModel?.logId ?? 0).")
...
extension ChatCloudDelegate: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        NiftyLogTool.paragraphFromRank(msg: (String(show_wrapStr) + String(kThickData.suffix(6))) + "\(errCode)" + (String(dataSeriesStr)) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        NiftyLogTool.paragraphFromRank(msg: (String(constConstraintKey.suffix(5)) + "视频通话" + String(kSelectCorrectUrl.prefix(4)) + "到警告." + String(showBeamDynamicsText.suffix(4)) + "ningCo" + String(appSameName)) + "\(warningCode)" + (String(kRevenueId.suffix(4)) + "rnin" + String(userOthersUrl)) + "\(String(describing: warningMsg)).")
        ...
        if self.config.remoteCameraStatus == .Normal {
            ...
            NotificationCenter.default.post(name: k_firstValue, object: self)
        }
    }
    ...
    func onEnterRoom(_ result: Int) {
        ...
        NiftyLogTool.paragraphFromRank(msg: (String(noti_exceptionValue) + String(appDismissValue)) + "\(result)" + (String(kSizeText.prefix(8))) + "\(infoModel?.logId ?? 0).")
        //: if result < 0 {
        if result < 0 { // 加入失败
    ...
    func onUserVideoAvailable(_ userId: String, available: Bool) {
        //: guard userId != AppManager.share.loginUserMode.userID else { return }
        guard userId != AppManager.share.loginUserMode.userID else { return }
