...
/*: "Speaker On" :*/
fileprivate let main_hoodMessage:String = "Speakclosed library satisfy"
fileprivate let k_gradeId:String = "make con pass party itser On"

/*: "Speaker Off" :*/
fileprivate let data_calculationKey:String = "Speakeunder enter push sample cost"
fileprivate let dataLaughBlastTicketStr:String = "platform"

/*: "TRTC 音频通话销毁. logId： :*/
fileprivate let user_replacePath:String = "TRTC \u{97f3}"
fileprivate let showProcessKey:String = "bullet meet detect uniform. log"
fileprivate let app_napName:[Character] = ["I","d","："]

/*: "TRTC 音频通话错误. errCode: :*/
fileprivate let mainVictoryValue:String = "priceTC"
fileprivate let notiBirthId:String = "\u{8bdd}错误. err"
fileprivate let dataAmStr:String = "Code:plan policy button"

/*: , errMsg: :*/
fileprivate let userGapUrl:String = ", errpinion equity gesture"

/*: "TRTC 音频通话SDK收到警告. warningCode: :*/
fileprivate let show_sessionStr:[Character] = ["T","R","T","C"," ","音","频","\u{901a}","话","S","D","K"]
fileprivate let constVersionFormat:[Character] = ["收","到","警","告","."," ","w","a","r","n","i","n","g","C","o","d","e",":"]

/*: , warningMsg: :*/
fileprivate let app_principalName:[Character] = [","," ","w","a","r","n","i","n","g","M","s"]
fileprivate let k_redMsg:String = "opportunity bench protocol owng:"

/*: "TRTC 音频通话进入房间. result: :*/
fileprivate let constRandomMessage:[Character] = ["T","R","T","C"," ","音","频","通","话","\u{8fdb}","\u{5165}","房","\u{95f4}","."," ","r"]
fileprivate let const_manualGesturePath:[Character] = ["e","s","u","l","t",":"]

/*: , logId： :*/
fileprivate let constBuilderStr:String = ", logextent wide option cognitive"
fileprivate let showUserMutualStr:[Character] = ["I","d","："]

/*:  加入了房间" :*/
fileprivate let main_personalKey:[Character] = [" ","加","入","了","房","间"]

/*:  离开了房间" :*/
fileprivate let app_perAccountVersionTitle:String = " 离must了房间"
...
class KingLightChatManager: NSObject {
    ...
    private var config = FirstChatConfig() // 音频渲染配置类
    //: private var logId = 0
...
extension KingLightChatManager {
    ...
    func moreFor(roomId: Int) {
        //: self.handsfreeBtn?.isEnabled = !hasHeadSet()
        self.handsfreeBtn?.isEnabled = !placeSet()

        //: self.config.isMutual = true
        self.config.isMutual = true
        //: self.logId = roomId
        self.logId = roomId
        //: enterRoom()
    ...
    func busyDetect(isHandsfree: Bool) {
        //: guard hasHeadSet() == false else { return }
        guard placeSet() == false else { return }
        ...
        self.handsfreeDesLab?.text = isHandsfree ? (String(main_hoodMessage.prefix(5)) + String(k_gradeId.suffix(5))).localized : (String(data_calculationKey.prefix(6)) + "r Of" + dataLaughBlastTicketStr.replacingOccurrences(of: "platform", with: "f")).localized
        //: updateWhenPlayDeviceChanged(isHandsfree: isHandsfree)
        eigenvalueOfASquareMatrix(isHandsfree: isHandsfree)
    }
}
...
extension KingLightChatManager {
    ...
    func partyDraw() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(operationOn(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(suspensionBridge(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(enterCookieNotif(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(operateUponNotif(notif:)),
    ...
    @objc private func operationOn(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func suspensionBridge(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func enterCookieNotif(notif: Notification) {
    ...
    @objc func operateUponNotif(notif: Notification) {
        ...
        case .newDeviceAvailable: // 听筒
            ...
            eigenvalueOfASquareMatrix(isHandsfree: false)
        ...
        case .oldDeviceUnavailable: // 免提
            ...
            self.handsfreeDesLab?.text = (String(main_hoodMessage.prefix(5)) + String(k_gradeId.suffix(5))).localized
            //: updateWhenPlayDeviceChanged(isHandsfree: self.config.isHandsfree)
            eigenvalueOfASquareMatrix(isHandsfree: self.config.isHandsfree)
...
extension KingLightChatManager {
    ...
    private func irrupt() {
        ...
        params.sdkAppId = k_managerLastId
        //: params.roomId = UInt32(logId)
        params.roomId = UInt32(logId)
        //: params.userId = AppManager.share.loginUserMode.userID
        params.userId = AppManager.share.loginUserMode.userID
        //: params.userSig = AppManager.share.appUserConfigMode.uSigTrtc
        params.userSig = AppManager.share.appUserConfigMode.uSigTrtc
    ...
    private func eigenvalueOfASquareMatrix(isHandsfree: Bool) {
    ...
    private func comeBack() {
        ...
        NiftyLogTool.paragraphFromRank(msg: (user_replacePath + "频通话销毁" + String(showProcessKey.suffix(5)) + String(app_napName)) + "\(logId).")
        //: guard self.config.isMutual else { return }
        guard self.config.isMutual else { return }

        //: if self.config.isHandsfree == false {
        if self.config.isHandsfree == false { // 恢复系统扬声器播放声音
            //: updateWhenPlayDeviceChanged(isHandsfree: true)
            eigenvalueOfASquareMatrix(isHandsfree: true)
        }
...
extension KingLightChatManager: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        NiftyLogTool.paragraphFromRank(msg: (mainVictoryValue.replacingOccurrences(of: "price", with: "TR") + " 音\u{9891}通" + notiBirthId + String(dataAmStr.prefix(5))) + "\(errCode)" + (String(userGapUrl.prefix(5)) + "Msg:") + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
            //: self.func__showStatusBarErrorMsg(showMsg: errMsg!)
            self.diskTerms(showMsg: errMsg!)
        }
    }
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        NiftyLogTool.paragraphFromRank(msg: (String(show_sessionStr) + String(constVersionFormat)) + "\(warningCode)" + (String(app_principalName) + String(k_redMsg.suffix(2))) + "\(String(describing: warningMsg)).")
    }

    //: func onEnterRoom(_ result: Int) {
    func onEnterRoom(_ result: Int) {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话进入房间. result:\(result), logId：\(logId).")
        NiftyLogTool.paragraphFromRank(msg: (String(constRandomMessage) + String(const_manualGesturePath)) + "\(result)" + (String(constBuilderStr.prefix(5)) + String(showUserMutualStr)) + "\(logId).")
        //: if result < 0 {
        if result < 0 { // 加入失败
            //: self.config.disconnected = true
            self.config.disconnected = true
            //: TalkingSocketManager.shared.endTalkingPing()
            MigrationSocketDelegate.shared.scopeMethod()
            //: } else {
        } else {
            //: self.config.disconnected = false
            self.config.disconnected = false
    ...
    func onRemoteUserEnterRoom(_ userId: String) {
        ...
        printLog(message: "\(userId)" + (String(main_personalKey)))
    }

    //: func onRemoteUserLeaveRoom(_ userId: String, reason: Int) {
    func onRemoteUserLeaveRoom(_ userId: String, reason _: Int) {
        //: printLog(message: "\(userId) 离开了房间")
        printLog(message: "\(userId)" + (app_perAccountVersionTitle.replacingOccurrences(of: "must", with: "开")))
    }
}
