...
fileprivate let constThumbData:[UInt8] = [0x45,0x4a,0x45,0x50,0x4,0x3f,0x4b,0x40,0x41,0x4e,0x16,0x5,0xfc,0x44,0x3d,0x4f,0xfc,0x4a,0x4b,0x50,0xfc,0x3e,0x41,0x41,0x4a,0xfc,0x45,0x49,0x4c,0x48,0x41,0x49,0x41,0x4a,0x50,0x41,0x40]

fileprivate func notebookEntry(shall num: UInt8) -> UInt8 {
...
/*: "#171717" :*/
fileprivate let kAssociationUsedOppositeName:String = "#tickticktick"

/*: "icon_cameraoff" :*/
fileprivate let notiForeId:String = "icon_capply center step like close"
fileprivate let userPinionData:[Character] = ["o","f","f"]
...
class WindowView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.backgroundColor = UIColor.itInstall(51, 51, 51, 1)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constThumbData.map{notebookEntry(shall: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var bgView: UIImageView = {
        ...
        alphaV.backgroundColor = UIColor(hex: (kAssociationUsedOppositeName.replacingOccurrences(of: "tick", with: "17")))
        //: alphaV.alpha = 0.6
        alphaV.alpha = 0.6
        //: v.addSubview(alphaV)
        v.addSubview(alphaV)
    ...
    private lazy var icon: UIImageView = {
        ...
        v.image = UIImage.scheme(name: (String(notiForeId.prefix(6)) + "amera" + String(userPinionData)))
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
...
extension WindowView {
    ...
    func multi(urlStr: String?, isOpenCamera: StartCameraStatus) {
...
extension WindowView: UIGestureRecognizerDelegate {
    /// 拖拽位移手势
    /// - Parameter pan: 手势
    //: @objc private func dragWindow(pan: UIPanGestureRecognizer) {
    @objc private func mini(pan: UIPanGestureRecognizer) {
        ...
        newCenterX = min(newCenterX, show_explainId - self.frame.size.width * 0.5)
        //: newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        //: newCenterY = min(newCenterY, ScreenHeight - self.frame.size.height * 0.5)
        newCenterY = min(newCenterY, userSystemKey - self.frame.size.height * 0.5)
...
extension WindowView {
    ...
    private func smallScale() {
        // 切换视角手势
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(exchangeRenderContainer))
        let tap = UITapGestureRecognizer(target: self, action: #selector(container))
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(mini(pan:)))
        //: pan.delegate = self
        pan.delegate = self
