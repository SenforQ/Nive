...
fileprivate let main_elementFormat:[UInt8] = [0xfb,0x0,0xfb,0x6,0xba,0xf5,0x1,0xf6,0xf7,0x4,0xcc,0xbb,0xb2,0xfa,0xf3,0x5,0xb2,0x0,0x1,0x6,0xb2,0xf4,0xf7,0xf7,0x0,0xb2,0xfb,0xff,0x2,0xfe,0xf7,0xff,0xf7,0x0,0x6,0xf7,0xf6]

fileprivate func argumentRun(build num: UInt8) -> UInt8 {
...
/*: "#FFD13A" :*/
fileprivate let userOriginData:String = "adjustment we transfer try#FFD13"
fileprivate let kScreenFormat:String = "coordinator"

/*: "gift_gb_umnad_1" :*/
fileprivate let showFadeId:[Character] = ["g","i","f","t","_","g","b","_","u"]
fileprivate let constMultipleFormat:String = "mnad_1gender fill situation clock go"

/*: "icon_circle_pic_default" :*/
fileprivate let dataEnterExceptPath:String = "icon_cidentity target chart"
fileprivate let const_activeData:String = "carrier equallypic_"
fileprivate let const_additionalPath:String = "premium"

/*: "Open lucky blind-box" :*/
fileprivate let user_listValue:String = "Opencontinuing publicly method automatic"
fileprivate let k_kindTitle:String = "BLIND"

/*: "Sent " :*/
fileprivate let user_snowUrl:String = "Sent format just album"

/*: "PingFangSC-Semibold" :*/
fileprivate let dataParticleName:[Character] = ["P","i","n","g","F","a","n","g","S","C","-"]
fileprivate let kInvitationMakeupPath:String = "oxygen"
fileprivate let dataSpendFromUrl:String = "ebagbold"

/*: "Bombard" :*/
fileprivate let user_maximumId:String = "Bombardoutside statute"
...
class YesterdayItemView: UIView {
    //: var giftMsgModel = TalkingGiftMsgAnimatModel()
    var giftMsgModel = LightAnimatModel()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_elementFormat.map{argumentRun(build: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var nickLb: UILabel = {
        ...
        lb.font = .balanceSize(fontSize: 14)
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(userOriginData.suffix(6)) + kScreenFormat.replacingOccurrences(of: "coordinator", with: "A")))
    ...
    private lazy var pluralTip: BehaviorThen = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = BehaviorThen()
        //: lab.isHidden = true
        lab.isHidden = true
    ...
    private lazy var countLb: BehaviorThen = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = BehaviorThen()
...
extension YesterdayItemView {
    //: func setGiftMsgModel(model: TalkingGiftMsgAnimatModel) {
    func pure(model: LightAnimatModel) {
        ...
        self.bgImage.image = UIImage.scheme(name: (String(showFadeId) + String(constMultipleFormat.prefix(6))))

        //: let placeholder = UIImage.BundleImageNamed(name: "icon_circle_pic_default")
        let placeholder = UIImage.scheme(name: (String(dataEnterExceptPath.prefix(6)) + "ircle_" + String(const_activeData.suffix(4)) + "defaul" + const_additionalPath.replacingOccurrences(of: "premium", with: "t")))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == CreaseMultiplierTarget.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(user_listValue.prefix(4)) + " lucky " + k_kindTitle.lowercased() + "-box").localized
            //: }else {
        } else {
        ...
        self.avatarImage.commonClose(urlStr: giftMsgModel.fromHeadPic, placeImg: placeholder)
        //: self.giftImage.setUrlImage(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        self.giftImage.commonClose(urlStr: giftMsgModel.giftPic, placeImg: placeholder)

        //: self.giftName.text = "Sent ".localized + giftMsgModel.pname
        self.giftName.text = (String(user_snowUrl.prefix(5))).localized + giftMsgModel.pname
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(dataParticleName) + kInvitationMakeupPath.replacingOccurrences(of: "oxygen", with: "S") + dataSpendFromUrl.replacingOccurrences(of: "bag", with: "mi")), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = falseName(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(dataParticleName) + kInvitationMakeupPath.replacingOccurrences(of: "oxygen", with: "S") + dataSpendFromUrl.replacingOccurrences(of: "bag", with: "mi")), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = falseName(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.orientationDownSpread(texts: strs, colors: colors, fonts: fonts)
        ...
        proposition(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func proposition(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(dataParticleName) + kInvitationMakeupPath.replacingOccurrences(of: "oxygen", with: "S") + dataSpendFromUrl.replacingOccurrences(of: "bag", with: "mi")), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = falseName(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(dataParticleName) + kInvitationMakeupPath.replacingOccurrences(of: "oxygen", with: "S") + dataSpendFromUrl.replacingOccurrences(of: "bag", with: "mi")), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = falseName(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.itInstall(150, 77, 20, 1), UIColor.white, UIColor.itInstall(150, 77, 20, 1)]
        ...
        attText = attText.orientationDownSpread(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.ocular(attributedText: attText)
        ...
        countLb.superimposeWithoutSpectacles(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func noLimited(giftMsgModel: LightAnimatModel) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(actuate), object: nil)
        ...
        self.perform(#selector(actuate), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func alongGap(model _: LightAnimatModel, animate _: Bool) {
    ...
    func spectaclesFrame(frame: CGRect) {
        //: UIView.animate(withDuration: 0.2, delay: 0.1, options: [.layoutSubviews]) {
        UIView.animate(withDuration: 0.2, delay: 0.1, options: [.layoutSubviews]) {
            //: self.frame = frame
            self.frame = frame
...
extension YesterdayItemView {
    ...
    func falseName(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension YesterdayItemView {
    ...
    private func compose() {
        ...
        font = falseName(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.orientationDownSpread(texts: [(String(user_maximumId.prefix(7))).localized], colors: [UIColor.itInstall(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.superimposeWithoutSpectacles(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.ocular(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.superimposeWithoutSpectacles(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
