...
fileprivate let mainTempValue:[UInt8] = [0x94,0x93,0x94,0x89,0xd5,0x9e,0x92,0x99,0x98,0x8f,0xc7,0xd4,0xdd,0x95,0x9c,0x8e,0xdd,0x93,0x92,0x89,0xdd,0x9f,0x98,0x98,0x93,0xdd,0x94,0x90,0x8d,0x91,0x98,0x90,0x98,0x93,0x89,0x98,0x99]

/*: "rating_guide_bg" :*/
fileprivate let main_degreeId:String = "rasteping"
fileprivate let dataAppropriateTitle:[Character] = ["b","g"]

/*: "#FF0084" :*/
fileprivate let k_dueStr:[Character] = ["#","F","F","0","0","8","4"]

/*: "points" :*/
fileprivate let show_reportSenseText:[UInt8] = [0x73,0x74,0x6e,0x69,0x6f,0x70]

/*: "coins" :*/
fileprivate let show_illegalUrl:[Character] = ["c","o","i","n","s"]

/*: "Get  :*/
fileprivate let k_establishExactlyContent:String = "play letter brow redGet "
...
class OriginalGuideView: UIView {
    // 右边按钮点击事件
    //: var rightBlcok: (() -> Void)?
    var rightBlcok: (() -> Void)?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainTempValue.map{$0^253}, encoding: .utf8)!)
    }
    ...
    private lazy var bgImgView: UIImageView = {
        ...
        v.image = UIImage.scheme(name: (main_degreeId.replacingOccurrences(of: "step", with: "t") + "_guide_" + String(dataAppropriateTitle)))
    ...
    private lazy var rewardLab: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(k_dueStr)))!
        //: label.font = UIFont.pingfangFont(type: .Medium, fontSize: 22)
        label.font = UIFont.shareOff(type: .Medium, fontSize: 22)
        //: let text = AppManager.share.loginUserMode.sex == Gender.female.rawValue ? "points":"coins"
        let text = AppManager.share.loginUserMode.sex == SharedMirrorKingCustomReflectable.female.rawValue ? String(bytes: show_reportSenseText.reversed(), encoding: .utf8)! : (String(show_illegalUrl))
        //: label.text = "Get \(AppManager.share.appUserConfigMode.ratingReward) \(text)!"
        label.text = (String(k_establishExactlyContent.suffix(4))) + "\(AppManager.share.appUserConfigMode.ratingReward) \(text)!"
        //: return label
        return label
        //: }()
    }()
    ...
    private lazy var goBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(errThroughSnap), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension OriginalGuideView {
    ...
    @objc private func errThroughSnap() {
        //: dismiss()
        beyondThreshold()
        //: rightBlcok?()
        rightBlcok?()
...
extension OriginalGuideView {
    ...
    private func handle() {
        ...
        rewardLab.snp.makeConstraints { make in
            ...
            let currLanguage = CombinedLanguageManager.shared.currLanguage
            //: if currLanguage == "es" {
            if currLanguage == "es" {
