...
fileprivate let k_alongsideStr:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "[未知消息]" :*/
fileprivate let dataFormationId:String = "[未知\u{6d88}\u{606f}]"

/*: "extra" :*/
fileprivate let k_exceptName:String = "extsoldier"

/*: "#0084FF" :*/
fileprivate let noti_pickFormat:String = "#0084"
fileprivate let appDriveName:String = "tartar"

/*: "#FFEE4B" :*/
fileprivate let notiOperateName:String = "#FFEE4Bsegment sweet chart"

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let noti_attributeUrl:String = "<at>ignore past perform greatest"
fileprivate let kFourName:String = "correct middle comment exclusive with+?</at>"

/*: "jumpKey" :*/
fileprivate let user_comeValue:String = "quality"
fileprivate let user_differenceMessage:String = "umpKeylat be nothing absolute bound"

/*: "user" :*/
fileprivate let data_kingFormat:String = "usequote"

/*: "uid" :*/
fileprivate let user_enoughPressName:[UInt8] = [0xff,0xe3,0xee]

/*: "#16D073" :*/
fileprivate let appTablePath:[Character] = ["#"]
fileprivate let noti_tarWhichTitle:String = "birthday income16D073"

/*: "#F95644" :*/
fileprivate let kCuteMessage:[Character] = ["#","F","9","5","6","4","4"]
...
@objcMembers public class ListCellData: PlanOfActionCellData {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_alongsideStr.reversed(), encoding: .utf8)!)
    }
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = netDown(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func netDown(msgInfoModel: ChatModel) -> NSMutableAttributedString {
        ...
        guard dic[(k_exceptName.replacingOccurrences(of: "soldier", with: "ra"))] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.aircraft() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (noti_pickFormat.capitalized + appDriveName.replacingOccurrences(of: "tar", with: "F")))! : UIColor(hex: (String(notiOperateName.prefix(7))))!
        //: let font = UIFont.pingfangRugularFont(fontSize: 16)
        let font = UIFont.balanceSize(fontSize: 16)
        ...
        let regex = try! NSRegularExpression(pattern: (String(noti_attributeUrl.prefix(4)) + "@[\\S\\s]" + String(kFourName.suffix(7))), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo[(user_comeValue.replacingOccurrences(of: "quality", with: "j") + String(user_differenceMessage.prefix(6)))].stringValue == (data_kingFormat.replacingOccurrences(of: "quote", with: "r")) {
                //: uid = jumpInfo["uid"].intValue
                uid = jumpInfo[String(bytes: user_enoughPressName.map{$0^138}, encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: user_enoughPressName.map{$0^138}, encoding: .utf8)!: uid]) { [weak self] _, _, _, _ in
    ...
    func snaplineText() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(appTablePath) + String(noti_tarWhichTitle.suffix(6))))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (String(kCuteMessage)))!
        }
