...
fileprivate let app_ehTitle:[UInt8] = [0xe8,0xef,0xe8,0xf5,0xa9,0xe2,0xee,0xe5,0xe4,0xf3,0xbb,0xa8,0xa1,0xe9,0xe0,0xf2,0xa1,0xef,0xee,0xf5,0xa1,0xe3,0xe4,0xe4,0xef,0xa1,0xe8,0xec,0xf1,0xed,0xe4,0xec,0xe4,0xef,0xf5,0xe4,0xe5]

/*: "icon_chat_miss" :*/
fileprivate let userCognitiveId:String = "icon_creward analysis bear media"
fileprivate let constAddContent:[Character] = ["m","i","s","s"]

/*: "icon_chat_yes" :*/
fileprivate let show_countStr:String = "privacycon"
fileprivate let dataDismissTitle:String = "harvests"
...
class DomainViewCell: ConversionView {
//    var transBlock: (() -> Void)?

    //: var textData: MirrorCellData?
    var textData: MirrorCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_ehTitle.map{$0^129}, encoding: .utf8)!)
    }
    ...
    lazy var quoteView: OriginalQuoteView = {
        //: let quoteView = TalkingMsgQuoteView()
        let quoteView = OriginalQuoteView()
        //: quoteView.isHidden = false
        quoteView.isHidden = false
        //: quoteView.addTarget(self, action: #selector(quoteBtnClick), for: .touchUpInside)
        quoteView.addTarget(self, action: #selector(enrich), for: .touchUpInside)
        //: return quoteView
        return quoteView
        //: }()
    }()
    ...
    lazy var transLB: YYLabel = {
        ...
        label.font = UIFont.balanceSize(fontSize: 16)
        //: return label
        return label
        //: }()
    }()
    ...
    override func metallicElement(with data: OldThen) {
        ...
        if let temTextData = data as? MirrorCellData {
            ...
            if imgPreview.count > 0, temTextData.msgModel.msgInfo.messageType == .MessageTypeWelcome {
                ...
                giftImagV.scrubFinish(urlStr: imgPreview)
            }
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != DelineateThen.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.loadTitle() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transStatusImagV.image = UIImage.scheme(name: (String(userCognitiveId.prefix(6)) + "hat_" + String(constAddContent)))
                ...
                self.transStatusImagV.image = UIImage.scheme(name: (show_countStr.replacingOccurrences(of: "privacy", with: "i") + "_chat_y" + dataDismissTitle.replacingOccurrences(of: "harvest", with: "e")))
            }
    ...
    func deadline() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(engageMember(tap:)))
        ...
        self.container.addGestureRecognizer(containerTap)
        //: let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == DelineateThen.getXiaoMiID())
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == DelineateThen.slowModify())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            can()
...
//            }
    ...
    func can() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(noSuccess(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func TouchCellMessageAction(tap: UITapGestureRecognizer?) {
    @objc func engageMember(tap _: UITapGestureRecognizer?) {
    ...
    @objc func noSuccess(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
...
//    }
    ...
    @objc func enrich() {
        ...
        self.delegate?.spanMoment(cell: self)
    }
}
