...
fileprivate let show_smartAssistMessage:[UInt8] = [0x6e,0x69,0x6e,0x73,0x2f,0x64,0x68,0x63,0x62,0x75,0x3d,0x2e,0x27,0x6f,0x66,0x74,0x27,0x69,0x68,0x73,0x27,0x65,0x62,0x62,0x69,0x27,0x6e,0x6a,0x77,0x6b,0x62,0x6a,0x62,0x69,0x73,0x62,0x63]

/*: "all" :*/
fileprivate let showMeanStr:String = "afit"
...
class ResolveViewCell: ConversionView {
    //: var giftData: TalkingGroupChatGiftMsgCellData?
    var giftData: PulseCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_smartAssistMessage.map{$0^7}, encoding: .utf8)!)
    }
...
extension ResolveViewCell {
    ...
    override func metallicElement(with data: OldThen) {
        ...
        self.giftData = data as? PulseCellData
        ...
        if giftData.msgModel.toUser.uid == (showMeanStr.replacingOccurrences(of: "fit", with: "ll")) { // 送礼所有人
            //: self.toUserImgView.image = UIImage.BundleImageNamed(name: giftData.msgModel.toUser.headPic)
            self.toUserImgView.image = UIImage.scheme(name: giftData.msgModel.toUser.headPic)
            //: } else {
        } else {
            //: self.toUserImgView.setUrlImage(urlStr: giftData.msgModel.toUser.headPic)
            self.toUserImgView.scrubFinish(urlStr: giftData.msgModel.toUser.headPic)
        }
    }
