...
fileprivate let const_pureFormat:[UInt8] = [0x51,0x56,0x51,0x4c,0x10,0x5b,0x57,0x5c,0x5d,0x4a,0x2,0x11,0x18,0x50,0x59,0x4b,0x18,0x56,0x57,0x4c,0x18,0x5a,0x5d,0x5d,0x56,0x18,0x51,0x55,0x48,0x54,0x5d,0x55,0x5d,0x56,0x4c,0x5d,0x5c]

/*: "CCCCCC" :*/
fileprivate let dataJourneyValue:String = "headheadhead"

/*: "icon_lounge" :*/
fileprivate let app_depthLogMsg:String = "ICON"

/*: "#FFEE4B" :*/
fileprivate let const_timeValue:[Character] = ["#","F","F","E","E","4","B"]

/*: "View read receipts?" :*/
fileprivate let k_layerData:String = "Viewformal required announcement"
fileprivate let dataOthersPageStr:String = "d reroot girl speak"
fileprivate let constNearbyTitle:String = "past when your shall labceipts?"

/*: "#FBE3FE" :*/
fileprivate let notiComputerValue:[Character] = ["#","F","B","E","3","F","E"]

/*: "#D8DDFF" :*/
fileprivate let noti_ageSheerJustId:[Character] = ["#","D","8","D","D","F","F"]
...
class ProposeMsgCell: ConversionView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_pureFormat.map{$0^56}, encoding: .utf8)!)
    }
    ...
    lazy var messageLB: YYLabel = {
        ...
        label.font = UIFont.balanceSize(fontSize: 15)
        //: label.textColor = UIColor.init(hex: "CCCCCC")
        label.textColor = UIColor(hex: (dataJourneyValue.replacingOccurrences(of: "head", with: "CC")))
    ...
    private lazy var vipReadReceiptBtn: ExtendedView = {
        //: let btn = TalkingButton()
        let btn = ExtendedView()
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_lounge"), for: .normal)
        btn.setImage(UIImage.scheme(name: (app_depthLogMsg.lowercased() + "_lounge")), for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        btn.titleLabel?.font = UIFont.shareOff(type: .Regular, fontSize: 15)
        //: btn.setTitleColor(UIColor(hex: "#FFEE4B"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(const_timeValue))), for: .normal)
        //: btn.setTitle("View read receipts?".localized, for: .normal)
        btn.setTitle((String(k_layerData.prefix(4)) + " rea" + String(dataOthersPageStr.prefix(4)) + String(constNearbyTitle.suffix(7))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(labAnagrammatise), for: .touchUpInside)
        ...
        let gColor = [UIColor(hex: (String(notiComputerValue)))!.withAlphaComponent(0.6).cgColor,
                      //: UIColor.init(hex: "#D8DDFF")!.withAlphaComponent(0.6).cgColor]
                      UIColor(hex: (String(noti_ageSheerJustId)))!.withAlphaComponent(0.6).cgColor]
        //: btn.setBackgroundImage(UIImage.imageGradientColor(colors: gColor, size: CGSize(width: btnWidth, height: btnHeight)), for: .normal)
        btn.setBackgroundImage(UIImage.textStud(colors: gColor, size: CGSize(width: btnWidth, height: btnHeight)), for: .normal)
...
extension ProposeMsgCell {
    /// vip订阅事件
    //: @objc private func vipReadReceiptClick() {
    @objc private func labAnagrammatise() {
...
extension ProposeMsgCell {
    ...
    override func metallicElement(with data: OldThen) {
        ...
        guard let tipCellData = data as? SimultaneouslyThen else { return }
        ...
        if dataLayout != nil {
            ...
            //: let layout = YYTextLayout.init(containerSize: CGSize.init(width: DelineateThen.getScreenWidth()-50, height: CGFloat(MAXFLOAT)), text: tipCellData.attributedString)
            let layout = YYTextLayout(containerSize: CGSize(width: DelineateThen.analysisDit() - 50, height: CGFloat(MAXFLOAT)), text: tipCellData.attributedString)
        ...
        if tipCellData.msgModel.tips.content == (String(k_layerData.prefix(4)) + " rea" + String(dataOthersPageStr.prefix(4)) + String(constNearbyTitle.suffix(7))).localized {
    ...
    override func updateConstraints() {
        ...
        self.container.snp.remakeConstraints { make in
            ...
            make.width.lessThanOrEqualTo(show_explainId - 40)
            //: make.top.equalTo(10)
            make.top.equalTo(10)
    ...
    func tab() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(belowSuccess(tap:)))
    ...
    @objc func belowSuccess(tap _: UITapGestureRecognizer) {
