...
fileprivate let userRawMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#AAAAAA" :*/
fileprivate let noti_pursueKey:String = "complete status access#AAAAA"
fileprivate let app_commonMessage:[Character] = ["A"]

/*: "btn_chatsettings_unchoice" :*/
fileprivate let noti_ignoreName:String = "btn_crecall statistical"
fileprivate let dataPeopleStr:[Character] = ["e","t","t","i","n","g","s","_","u","n","c","h","o","i","c","e"]

/*: "btn_chatsettings_choiced" :*/
fileprivate let noti_betterData:String = "btn_perform quality explain shot composition"
fileprivate let show_analyzePath:[Character] = ["e","t","t","i","n","g","s","_","c","h","o","i","c","e","d"]

/*: "Top" :*/
fileprivate let dataAccuracyEveryUrl:String = "Topcontext box"

/*: "Save" :*/
fileprivate let dataExitWeekSphereMsg:String = "Savemanage happy give"

/*: "Cancel" :*/
fileprivate let showOptionValue:String = "cloud fact replyCance"
fileprivate let show_carrierPath:String = "L"
...
class CreaseView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userRawMessage.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIView = {
        ...
        let height = actualWidth(w: 475) + user_videoMsg
        //: let v = UIView(frame: CGRect(x: 0, y: ScreenHeight-height, width: ScreenWidth, height: height))
        let v = UIView(frame: CGRect(x: 0, y: userSystemKey - height, width: show_explainId, height: height))
        //: v.backgroundColor = .white
        v.backgroundColor = .white
        //: v.Corner(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        v.glide(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var textBgView: UIView = {
        ...
        v.backgroundColor = UIColor.referenceAndNonsolidColour()
    ...
    private lazy var limitLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(noti_pursueKey.suffix(6)) + String(app_commonMessage)))
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var topBtn: ExtendedView = {
        ...
        btn.setImage(UIImage.scheme(name: (String(noti_ignoreName.prefix(5)) + "hats" + String(dataPeopleStr))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_chatsettings_choiced"), for: .selected)
        btn.setImage(UIImage.scheme(name: (String(noti_betterData.prefix(4)) + "chats" + String(show_analyzePath))), for: .selected)
        ...
        btn.setTitle((String(dataAccuracyEveryUrl.prefix(3))).localized, for: .normal)
        //: btn.spaceBetweenTitleAndImage = 6
        btn.spaceBetweenTitleAndImage = 6
        //: btn.addTarget(self, action: #selector(topButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(balanceParent), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var saveBtn: UIButton = {
        ...
        btn.setTitle((String(dataExitWeekSphereMsg.prefix(4))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(pursue), for: .touchUpInside)
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(showOptionValue.suffix(5)) + show_carrierPath.lowercased()).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(capacityBubble), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension CreaseView: YYTextViewDelegate {
...
extension CreaseView {
    ...
    @objc private func balanceParent() {
    ...
    @objc private func capacityBubble() {
        //: dismiss()
        textDismiss()
    }
    ...
    func everyAccess(_ content: String, isTop: Int, toUid: String) {
        //: targetId = toUid
        targetId = toUid
...
extension CreaseView {
    ...
    private func subCalculateConstraint() {
