...
/*: "#ffffff" :*/
fileprivate let k_arrowMessage:String = "#fffffmirror highlight"
fileprivate let mainAreaMsg:String = "threshold"

/*: "earned" :*/
fileprivate let mainExistPath:String = "earexcessd"

/*: "Coins in" :*/
fileprivate let const_othersTitle:String = "proposal dock readerCoins i"
fileprivate let showLightPath:String = "model"

/*: "Error:  :*/
fileprivate let show_groupSayId:String = "Error: or region"

/*: "#C946D2" :*/
fileprivate let app_clueRowStr:String = "lower package#C946D2"

/*: "btn_chat_floatS_userBK_nor" :*/
fileprivate let showOxygenData:[Character] = ["b","t","n","_","c","h","a","t","_","f","l","o","a","t","S","_"]
fileprivate let kBillChamberName:[Character] = ["u","s","e","r"]
fileprivate let k_reserveMessage:[Character] = ["B","K","_","n","o","r"]
...
class PrepareThen: UIView {
    ...
    private let TextColor = (String(k_arrowMessage.prefix(6)) + mainAreaMsg.replacingOccurrences(of: "threshold", with: "f"))

    //: var clickBlock: (() -> Void)?
    var clickBlock: (() -> Void)?

    //: convenience init(model: TalkingGameFloatingScreenModel?) {
    convenience init(model: RealmTransformable?) {
        //: self.init()
        self.init()
    ...
    private func customModel() {
        ...
        if danumuMsgModel.bgPic.count > 0 {
            ...
            lastUrl(withUrl: danumuMsgModel.bgPic)
        }
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            do {
                ...
                if let range = attrStr.string.range(of: (mainExistPath.replacingOccurrences(of: "excess", with: "ne"))) {
                ...
                if let range = attrStr.string.range(of: (String(const_othersTitle.suffix(7)) + showLightPath.replacingOccurrences(of: "model", with: "n"))) {
                    //: attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                    attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                }
                //: attrStr.addAttributes([.font: UIFont.pingfangFont(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                attrStr.addAttributes([.font: UIFont.shareOff(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                ...
                printLog(message: (String(show_groupSayId.prefix(7))) + "\(error)")
            }
        }
    }

    //: func updateBackground(withUrl url: String) {
    func lastUrl(withUrl url: String) {
        ...
        self.backgroundImageView.commonClose(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.HeightOfBackgroundImageView {
                    ...
                    newImage = image.processIcon(with: CGSize(width: targetWidth, height: self.HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: RealmTransformable? {
        //: didSet {
        didSet {
            //: configModel()
            customModel()
    ...
    var totalTime: CGFloat {
        ...
        return (show_explainId + widthValue) * SpendTime / show_explainId
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * LineSpendTime / ScreenWidth
        return widthValue * LineSpendTime / show_explainId
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * SpendTime / ScreenWidth
        return widthValue * SpendTime / show_explainId
    }
    ...
    lazy var headAnimatedImageView: UIImageView = {
        ...
        h.layer.borderColor = UIColor(hex: (String(app_clueRowStr.suffix(7))))?.cgColor
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var headBackImageView: UIImageView = {
        ...
        h.image = UIImage.scheme(name: (String(showOxygenData) + String(kBillChamberName) + String(k_reserveMessage)))
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(bassShadow(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension PrepareThen {
    ...
    @objc func bassShadow(_: UITapGestureRecognizer) {
        //: guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        //: let httpsUrl = AppManager.share.appConfigMode.urlH5Domain+gameUrl
        let httpsUrl = AppManager.share.appConfigMode.urlH5Domain + gameUrl
