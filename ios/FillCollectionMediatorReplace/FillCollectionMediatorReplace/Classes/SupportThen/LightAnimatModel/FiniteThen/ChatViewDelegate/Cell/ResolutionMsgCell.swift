...
fileprivate let dataSumUrl:[UInt8] = [0x24,0x23,0x24,0x39,0x65,0x2e,0x22,0x29,0x28,0x3f,0x77,0x64,0x6d,0x25,0x2c,0x3e,0x6d,0x23,0x22,0x39,0x6d,0x2f,0x28,0x28,0x23,0x6d,0x24,0x20,0x3d,0x21,0x28,0x20,0x28,0x23,0x39,0x28,0x29]

/*: "icon_chat_translate_nor" :*/
fileprivate let dataCurrentlyPath:String = "broad origin divide intoicon_"
fileprivate let main_browValue:String = "rocketan"
fileprivate let notiShrinkUpWhichUrl:[Character] = ["e","_","n","o","r"]

/*: "icon_chat_translate_pre" :*/
fileprivate let noti_cognitiveFormat:String = "icon_cprofile window dark round observe"
fileprivate let userDiscKey:String = "ansnap"

/*: "icon_chat_miss" :*/
fileprivate let constBreezeDockMaskKey:[Character] = ["i","c","o","n","_","c","h","a","t","_","m","i","s","s"]

/*: "icon_chat_yes" :*/
fileprivate let showArrowBetterTurnText:String = "talk focus giving capacityicon_chat"
fileprivate let kTranslateMessage:[Character] = ["_","y","e","s"]
...
class ResolutionMsgCell: ConversionView {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataSumUrl.map{$0^77}, encoding: .utf8)!)
    }
    ...
    lazy var transBtn: UIButton = {
        ...
        btn.setImage(UIImage.scheme(name: (String(dataCurrentlyPath.suffix(5)) + "chat_" + main_browValue.replacingOccurrences(of: "rocket", with: "tr") + "slat" + String(notiShrinkUpWhichUrl))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.scheme(name: (String(noti_cognitiveFormat.prefix(6)) + "hat_tr" + userDiscKey.replacingOccurrences(of: "nap", with: "l") + "ate_pre")), for: .highlighted)
        //: btn.addTarget(self, action: #selector(transBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(bot), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ResolutionMsgCell {
    ...
    override func metallicElement(with data: OldThen) {
        ...
        //: let maxWidth = DelineateThen.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = DelineateThen.analysisDit() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transStatusImagV.image = UIImage.scheme(name: (String(constBreezeDockMaskKey)))
            ...
            self.transStatusImagV.image = UIImage.scheme(name: (String(showArrowBetterTurnText.suffix(9)) + String(kTranslateMessage)))
        }
    ...
    func localPlay() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(clear(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func clear(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
