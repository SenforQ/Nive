...
fileprivate let k_executeTitle:[UInt8] = [0x1b,0x20,0x1b,0x26,0xda,0x15,0x21,0x16,0x17,0x24,0xec,0xdb,0xd2,0x1a,0x13,0x25,0xd2,0x20,0x21,0x26,0xd2,0x14,0x17,0x17,0x20,0xd2,0x1b,0x1f,0x22,0x1e,0x17,0x1f,0x17,0x20,0x26,0x17,0x16]

fileprivate func mayHousehold(remark num: UInt8) -> UInt8 {
...
/*: "icon_lounge" :*/
fileprivate let kSearchFormat:[Character] = ["i","c","o","n","_","l","o","u","n","g","e"]

/*: "icon_talk_points" :*/
fileprivate let notiJumpStr:[Character] = ["i"]
fileprivate let userTruthValue:[Character] = ["c","o","n","_","t","a","l","k","_","p","o","i","n","t","s"]

/*: "C6BDFF" :*/
fileprivate let app_upperFormat:[Character] = ["C","6","B","D","F"]
fileprivate let showRelationHeadNeverStr:[Character] = ["F"]

/*: "#BBBBBB" :*/
fileprivate let main_planData:[Character] = ["#","B","B","B","B"]
fileprivate let data_mentallyAverageValue:String = "bb"

/*: "Reply to get points~" :*/
fileprivate let const_withoutStr:[Character] = ["R","e","p","l","y"," ","t","o"," ","g","e"]
fileprivate let noti_waitChamberFoundUrl:String = "currently appropriate appearance firm confirmt poi"

/*: "bth_unread_nor" :*/
fileprivate let user_listMessage:String = "bth_beyond path nose"
fileprivate let mainRetirementText:String = "_norsession interested heart shadow"

/*: "icon_male_default" :*/
fileprivate let const_betterPath:[UInt8] = [0x67,0x6d,0x61,0x60,0x51,0x63,0x6f,0x62,0x6b,0x51,0x6a,0x6b,0x68,0x6f,0x7b,0x62,0x7a]

private func supposedCamera(prepare num: UInt8) -> UInt8 {
    return num ^ 14
}

/*: "icon_female_default" :*/
fileprivate let mainBurnId:String = "ICON"
fileprivate let show_herMsg:[Character] = ["_","f","e","m","a","l","e","_","d","e","f","a","u","l","t"]

/*: "+%@ points" :*/
fileprivate let userLateUrl:[Character] = ["+","%","@"," ","p"]
fileprivate let noti_tradeStr:String = "oiadditionals"

/*: "bth_read_pre" :*/
fileprivate let const_partnerMsg:String = "migration illegal intobth_r"
fileprivate let show_activeStr:String = "PRE"
...
//: class TalkingChatBaseMsgCell: SuckThen {
class ConversionView: SuckThen {
    //: var cellData: TalkingChatMsgBaseCellData?
    var cellData: PlanOfActionCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_executeTitle.map{mayHousehold(remark: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.scheme(name: (String(kSearchFormat)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var ageLab: UILabel = {
        ...
        label.font = UIFont.pingfangBusy(fontSize: 10)
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var coinIconImg: UIImageView = {
        ...
        imgV.image = UIImage.scheme(name: (String(notiJumpStr) + String(userTruthValue)))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    public lazy var coinLabel: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(app_upperFormat) + String(showRelationHeadNeverStr)))
        //: label.isHidden = true
        label.isHidden = true
    ...
    public lazy var replyTipLab: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(main_planData) + data_mentallyAverageValue.uppercased()))
        //: label.isHidden = true
        label.isHidden = true
        //: label.font = UIFont.pingfangRugularFont(fontSize: 14)
        label.font = UIFont.balanceSize(fontSize: 14)
        //: label.text = "Reply to get points~".localized
        label.text = (String(const_withoutStr) + String(noti_waitChamberFoundUrl.suffix(5)) + "nts~").localized
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var isReadImg: UIImageView = {
        ...
        imgV.image = UIImage.scheme(name: (String(user_listMessage.prefix(4)) + "unread" + String(mainRetirementText.prefix(4))))
        //: return imgV
        return imgV
        //: }()
    }()
...
extension ConversionView {
    ...
    override func metallicElement(with data: OldThen) {
        ...
        self.cellData = data as? PlanOfActionCellData
        ...
        let userModel = newData.msgModel.user
        //: let loungePlus = (AppManager.share.loginUid == String(userModel.uid)) ? AppManager.share.loginUserMode.loungePlus : userModel.loungePlus
        let loungePlus = (AppManager.share.loginUid == String(userModel.uid)) ? AppManager.share.loginUserMode.loungePlus : userModel.loungePlus

        //: self.nameLabel.text = userModel.nickname
        self.nameLabel.text = userModel.nickname
        //: self.nameLabel.textColor = loungePlus ? .userVipColor() : .appValueColor()
        self.nameLabel.textColor = loungePlus ? .cancer() : .capVidNonsolidColorConfirm()
        //: self.nameLabel.font = UIFont.pingfangMediumFont(fontSize: 13)
        self.nameLabel.font = UIFont.pingfangBusy(fontSize: 13)
        //: let imgStr = userModel.sex == Int(Gender.male.rawValue) ? "icon_male_default" : "icon_female_default"
        let imgStr = userModel.sex == Int(SharedMirrorKingCustomReflectable.male.rawValue) ? String(bytes: const_betterPath.map{supposedCamera(prepare: $0)}, encoding: .utf8)! : (mainBurnId.lowercased() + String(show_herMsg))
        //: sexIconBtn.setBackgroundImage(UIImage.BundleImageNamed(name: imgStr), for: .normal)
        sexIconBtn.setBackgroundImage(UIImage.scheme(name: imgStr), for: .normal)
        ...
        self.avatarView.scrubFinish(urlStr: userModel.headPic)
        ...
        self.iconBorder.glyceroliseHeadVisualizationFrameStr(urlStr: userModel.headPicFrame)

        //: let isShowCoin = newData.msgIncome > 0
        let isShowCoin = newData.msgIncome > 0
        //: self.coinIconImg.isHidden = !(isShowCoin && AppManager.share.appStatus == AppSkinStatus.normal.rawValue)
        self.coinIconImg.isHidden = !(isShowCoin && AppManager.share.appStatus == EarnLetterCustomReflectable.normal.rawValue)
        ...
        self.coinLabel.text = (String(userLateUrl) + noti_tradeStr.replacingOccurrences(of: "additional", with: "nt")).queryionLocalized(NSNumber(value: Float(newData.msgIncome)))
        // 女性逻辑, 未回复付费消息提示
        //: if TalkingPrivateChatManager.needShowReplyTipsMessage(newData.direction,
        if LetterChatManager.instanceTime(newData.direction,
        ...
        if LetterChatManager.anagrammatizeFrame(cellData: newData) {
            //: self.isReadImg.isHidden = false
            self.isReadImg.isHidden = false
            //: var readImgStr = "bth_unread_nor"
            var readImgStr = (String(user_listMessage.prefix(4)) + "unread" + String(mainRetirementText.prefix(4)))
            // 消息对端是否已读
            //: if TalkingPrivateChatManager.msgIsRead(cellData: newData) {
            if LetterChatManager.anagramToJudaicaData(cellData: newData) {
                //: readImgStr = "bth_read_pre"
                readImgStr = (String(const_partnerMsg.suffix(5)) + "ead_" + show_activeStr.lowercased())
            }
            //: self.isReadImg.image = UIImage.BundleImageNamed(name: readImgStr)
            self.isReadImg.image = UIImage.scheme(name: readImgStr)
    ...
    override public func updateConstraints() {
        ...
        if let temCellData = self.cellData {
            //: loungePlus = (AppManager.share.loginUid == String(temCellData.msgModel.user.uid)) ? AppManager.share.loginUserMode.loungePlus : temCellData.msgModel.user.loungePlus
            loungePlus = (AppManager.share.loginUid == String(temCellData.msgModel.user.uid)) ? AppManager.share.loginUserMode.loungePlus : temCellData.msgModel.user.loungePlus
        }
