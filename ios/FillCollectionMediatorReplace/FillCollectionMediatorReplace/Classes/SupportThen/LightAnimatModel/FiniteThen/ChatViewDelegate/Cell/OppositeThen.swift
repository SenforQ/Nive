...
fileprivate let app_distanceFormat:[UInt8] = [0xb7,0xbc,0xb7,0xc2,0x76,0xb1,0xbd,0xb2,0xb3,0xc0,0x88,0x77,0x6e,0xb6,0xaf,0xc1,0x6e,0xbc,0xbd,0xc2,0x6e,0xb0,0xb3,0xb3,0xbc,0x6e,0xb7,0xbb,0xbe,0xba,0xb3,0xbb,0xb3,0xbc,0xc2,0xb3,0xb2]

fileprivate func cancerName(premium num: UInt8) -> UInt8 {
...
/*: "move" :*/
fileprivate let main_bulletPath:String = "movstaff"
...
class OppositeThen: UIView {
    ...
    var danumuMsgArray: [RealmTransformable] = []
    //: var danumuItemViewArray: [TalkingGameFloatingScreenItemView] = []
    var danumuItemViewArray: [PrepareThen] = []

    //: let TapAction: PublishSubject<TalkingGameFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<RealmTransformable?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_distanceFormat.map{cancerName(premium: $0)}, encoding: .utf8)!)
    }
    ...
    func strength(_ danumuMsgModel: RealmTransformable?) {
    ...
    @objc func my(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(my(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension OppositeThen: UIGestureRecognizerDelegate {
    ...
    func titleTag(for danumuMsgModel: RealmTransformable?, isFirst: Bool) {
        //: let itemView = TalkingGameFloatingScreenItemView(model: danumuMsgModel)
        let itemView = PrepareThen(model: danumuMsgModel)
        ...
        var frame = CGRect(x: show_explainId, y: isFirst ? 7 : 50, width: itemView.widthValue, height: 24)
        //: var endFrame = frame
        var endFrame = frame
        ...
        if CombinedLanguageManager.shared.direction == .rightToLeft {
            ...
            endFrame.origin.x = show_explainId
        }
        //: addSubview(itemView)
        addSubview(itemView)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (main_bulletPath.replacingOccurrences(of: "staff", with: "e")))
    ...
    func tip(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                titleTag(for: model, isFirst: isFirst)
