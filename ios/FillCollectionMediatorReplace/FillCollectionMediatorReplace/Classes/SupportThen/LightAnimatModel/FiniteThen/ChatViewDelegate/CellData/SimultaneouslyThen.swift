...
fileprivate let kChallengeUrl:[UInt8] = [0x16,0x11,0x16,0xb,0x57,0x1c,0x10,0x1b,0x1a,0xd,0x45,0x56,0x5f,0x17,0x1e,0xc,0x5f,0x11,0x10,0xb,0x5f,0x1d,0x1a,0x1a,0x11,0x5f,0x16,0x12,0xf,0x13,0x1a,0x12,0x1a,0x11,0xb,0x1a,0x1b]

private func okAudience(method num: UInt8) -> UInt8 {
    return num ^ 127
}

/*: "View read receipts?" :*/
fileprivate let user_snapValue:String = "forth tun lipView"
fileprivate let appHoldMessage:String = " receuntil reach spot"
fileprivate let noti_followingText:String = "below output depth lack lineipts?"

/*: "text" :*/
fileprivate let constOrangeName:[Character] = ["t","e","x","t"]

/*: "color" :*/
fileprivate let const_goMsg:[UInt8] = [0x34,0x38,0x3b,0x38,0x25]

private func maxHer(speak num: UInt8) -> UInt8 {
    return num ^ 87
}

/*: "<xml>%@</xml>" :*/
fileprivate let const_withUrl:[Character] = ["<","x","m","l",">","%","@","<","/","x","m","l",">"]

/*: "href" :*/
fileprivate let user_habitId:[Character] = ["h","r","e","f"]

/*: "type" :*/
fileprivate let dataForceStrengthMsg:[UInt8] = [0xe1,0xec,0xe5,0xf0]

/*: "txt" :*/
fileprivate let userDrownMsg:String = "actualt"

/*: "padding" :*/
fileprivate let constStepName:String = "model"
fileprivate let mainExistWholeValue:String = "blackding"

/*: "size" :*/
fileprivate let data_everyName:[Character] = ["s","i","z","e"]

/*: "maxFont" :*/
fileprivate let main_deviceStr:String = "maxFontgap snap color sick later"

/*: "link" :*/
fileprivate let k_strongId:String = "haveink"

/*: "PingFangSC-Regular" :*/
fileprivate let main_informKey:String = "examine meet force dynamics acrossPingF"
fileprivate let show_toText:String = "animal square-Regular"

/*: "content" :*/
fileprivate let dataSharedMsg:[UInt8] = [0x4,0x10,0xf,0x15,0x6,0xf,0x15]

fileprivate func layWho(comment num: UInt8) -> UInt8 {
...
@objcMembers public class SimultaneouslyThen: PlanOfActionCellData {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kChallengeUrl.map{okAudience(method: $0)}, encoding: .utf8)!)
    }

    //: public override func heightOfWidth(_ width: CGFloat) ->CGFloat {
    override public func wrapBroadcast(_: CGFloat) -> CGFloat {
        //: return contentSize().height+20
        return netSize().height + 20
    }
    ...
    override public func netSize() -> CGSize {
        ...
        if self.msgModel.tips.content == (String(user_snapValue.suffix(4)) + " read" + String(appHoldMessage.prefix(5)) + String(noti_followingText.suffix(5))).localized {
            //: return CGSize(width: ScreenWidth, height: 30)
            return CGSize(width: show_explainId, height: 30)
        }
        ...
        //: let rect = self.attributedString.boundingRect(with: CGSize.init(width: DelineateThen.getScreenWidth()-50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil )
        let rect = self.attributedString.boundingRect(with: CGSize(width: DelineateThen.analysisDit() - 50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil)
    ...
    public lazy var attributedString: NSMutableAttributedString = {
        ...
        var attr = twine(info: self.msgModel.tips)
        //: return attr
        return attr
        //: }()
    }()
...
extension SimultaneouslyThen {
    //: func formatMessageToStringM(info: AbTalkingChatMsgTipsModel) -> NSMutableAttributedString {
    func twine(info: TemporaryModel) -> NSMutableAttributedString {
...
//        let dic = NSDictionary(jsonString: extra ?? "")
        ...
        return spark(tips: content)
    }

    //: func designTextFream(tips: String) -> NSMutableAttributedString {
    func spark(tips: String) -> NSMutableAttributedString {
        ...
        _ = stripByVid(msg: tips, info: attrbutedArray)
        //: self.yyLayout = designYYTextLayout(array: attrbutedArray, displayMaxWidth: DelineateThen.getScreenWidth()-50, para: para)
        self.yyLayout = queryPara(array: attrbutedArray, displayMaxWidth: DelineateThen.analysisDit() - 50, para: para)
    ...
    func stripByVid(msg: String, info: NSMutableArray) -> String {
        ...
        let pure_msg = everyCustomerResolution(source: msg, attrbAM: contentAttrArray)
        //: for  textInfo in contentAttrArray {
        for textInfo in contentAttrArray {
            //: let textDic = textInfo as![String: Any]
            let textDic = textInfo as! [String: Any]
            //: let text: String = textDic["text"] as! String
            let text: String = textDic[(String(constOrangeName))] as! String

            //: var textColor: UIColor?
            var textColor: UIColor?

            //: var obj = textDic["color"]
            var obj = textDic[String(bytes: const_goMsg.map{maxHer(speak: $0)}, encoding: .utf8)!]
            //: if obj is UIColor {
            if obj is UIColor {
                //: obj = obj as? UIColor
                obj = obj as? UIColor
            ...
            contentDict.removeObjects(forKeys: [String(bytes: const_goMsg.map{maxHer(speak: $0)}, encoding: .utf8)!, (String(constOrangeName))])

            //: let attrDict = ctDictionaryForText(content: text, color: textColor)
            let attrDict = who(content: text, color: textColor)
    ...
    func everyCustomerResolution(source: String, attrbAM: NSMutableArray) -> String {
        ...
        do {
            ...
            let document = try ONOXMLDocument(string: String(format: (String(const_withUrl)), newSource), encoding: CFStringConvertEncodingToNSStringEncoding(CFStringBuiltInEncodings.UTF8.rawValue))
            //: let element = document.rootElement
            let element = document.rootElement
            //: parseXMLElement(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: ["color": UIColor.white])
            coverAttrib(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: [String(bytes: const_goMsg.map{maxHer(speak: $0)}, encoding: .utf8)!: UIColor.white])
            ...
            attrbAM.add([(String(constOrangeName)): newSource, String(bytes: const_goMsg.map{maxHer(speak: $0)}, encoding: .utf8)!: UIColor.white])
            //: return newSource
            return newSource
        }
    }
    ...
    func coverAttrib(element: ONOXMLElement, arrM: NSMutableArray, attrib: NSDictionary) {
        ...
        if children.count > 0 {
            ...
            if element.tag == "u" { // 下滑线
                ...
                let url: String = element.attributes[(String(user_habitId))] as! String
                //: if url.count > 0 {
                if url.count > 0 {
                    //: parentInfo["href"] = url
                    parentInfo[(String(user_habitId))] = url
                }
            }
            //: for  subElement in children {
            for subElement in children {
                //: parseXMLElement(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
                coverAttrib(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
            }
            ...
            if text != nil {
                ...
                //: info["text"] = text
                info[(String(constOrangeName))] = text
                //: arrM.add(info)
                arrM.add(info)
            }
        }
    ...
    func queryPara(array: NSArray, displayMaxWidth: CGFloat, para: NSParagraphStyle?) -> YYTextLayout {
        //: let attrString = designAttribStr(array: array, paragraphStyle: para)
        let attrString = investStyle(array: array, paragraphStyle: para)
    ...
    func investStyle(array: NSArray, paragraphStyle: NSParagraphStyle?) -> NSAttributedString {
        ...
        array.enumerateObjects { obj, _, _ in
            ...
            let type = newObj[String(bytes: dataForceStrengthMsg.map{$0^149}, encoding: .utf8)!] as? String
            //: var tmp = NSAttributedString.init()
            var tmp = NSAttributedString()
            //: if type  == "txt" {
            if type == (userDrownMsg.replacingOccurrences(of: "actual", with: "tx")) {
                //: tmp = designAttribStrByTextDic(dict: newObj as NSDictionary)
                tmp = available(dict: newObj as NSDictionary)
                //: } else if type == "padding" {
            } else if type == (constStepName.replacingOccurrences(of: "model", with: "p") + mainExistWholeValue.replacingOccurrences(of: "black", with: "ad")) {
                //: let imageView = UIImageView.init()
                let imageView = UIImageView()
                //: imageView.size = (newObj["size"] as! NSValue).cgSizeValue
                imageView.size = (newObj[(String(data_everyName))] as! NSValue).cgSizeValue
                ...
                tmp = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: newObj[(String(main_deviceStr.prefix(7)))] as! UIFont, alignment: .center)
            }
            ...
            if underline {
                ...
                if len > 0 {
                    ...
                    _ = YYTextDecoration(style: .single, width: 1.5, color: newObj[String(bytes: const_goMsg.map{maxHer(speak: $0)}, encoding: .utf8)!] as? UIColor)
    ...
    func linkOf(dict: NSDictionary) -> NSAttributedString {
        //: let imageView = UIImageView.init()
        let imageView = UIImageView()
        //: imageView.size = (dict["size"]as! NSValue).cgSizeValue
        imageView.size = (dict[(String(data_everyName))] as! NSValue).cgSizeValue
        ...
        let attachText = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: dict[(String(main_deviceStr.prefix(7)))] as! UIFont, alignment: .center)
        //: return attachText
        return attachText
    }

    //: func designAttribStrByTextDic(dict: NSDictionary) ->NSMutableAttributedString {
    func available(dict: NSDictionary) -> NSMutableAttributedString {
        //: let type = dict["type"] as! String
        let type = dict[String(bytes: dataForceStrengthMsg.map{$0^149}, encoding: .utf8)!] as! String
        //: if type != "link" && type != "txt" {
        if type != (k_strongId.replacingOccurrences(of: "have", with: "l")) && type != (userDrownMsg.replacingOccurrences(of: "actual", with: "tx")) {
        ...
        let color = dict[String(bytes: const_goMsg.map{maxHer(speak: $0)}, encoding: .utf8)!] as? UIColor
        //: if color != nil {
        if color != nil {
        ...
        let fontSize = (dict[(String(data_everyName))] as? NSNumber)?.floatValue
        //: if (fontSize ?? 0 > 0) {
        if fontSize ?? 0 > 0 {
            //: let fontRef = CTFontCreateWithName("PingFangSC-Regular" as CFString, CGFloat(fontSize!), nil)
            let fontRef = CTFontCreateWithName((String(main_informKey.suffix(5)) + "angSC" + String(show_toText.suffix(8))) as CFString, CGFloat(fontSize!), nil)
            //: attributes[kCTFontAttributeName] = fontRef
            attributes[kCTFontAttributeName] = fontRef
        }
        //: var content = (dict["content"] != nil) ? dict["content"] : ""
        var content = (dict[String(bytes: dataSharedMsg.map{layWho(comment: $0)}, encoding: .utf8)!] != nil) ? dict[String(bytes: dataSharedMsg.map{layWho(comment: $0)}, encoding: .utf8)!] : ""
    ...
    func host() -> NSMutableDictionary {
        //: let font = UIFont.pingfangRugularFont(fontSize: 15)
        let font = UIFont.balanceSize(fontSize: 15)
    ...
    func who(content: String, color: UIColor?) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
        ...
        if color == nil {
            ...
            return publicationEnable(content: content)
        }
        //: return ["type": "txt",
        return [String(bytes: dataForceStrengthMsg.map{$0^149}, encoding: .utf8)!: (userDrownMsg.replacingOccurrences(of: "actual", with: "tx")),
                //: "content": content,
                String(bytes: dataSharedMsg.map{layWho(comment: $0)}, encoding: .utf8)!: content,
                //: "color": color ?? .black]
                String(bytes: const_goMsg.map{maxHer(speak: $0)}, encoding: .utf8)!: color ?? .black]
    }

    //: func ctDictionaryForText(content: String) ->NSDictionary {
    func publicationEnable(content: String) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
            //: return NSDictionary.init()
            return NSDictionary()
