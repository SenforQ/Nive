...
fileprivate let constProximateTitle:String = "information threshold businessTSyst"
fileprivate let notiNatureValue:String = "extendextendage"
fileprivate let show_produceUrl:String = "_Reunumb guide listen"

/*: "init(coder:) has not been implemented" :*/
fileprivate let data_yardMessage:[UInt8] = [0xc3,0xc8,0xc3,0xce,0x82,0xbd,0xc9,0xbe,0xbf,0xcc,0x94,0x83,0x7a,0xc2,0xbb,0xcd,0x7a,0xc8,0xc9,0xce,0x7a,0xbc,0xbf,0xbf,0xc8,0x7a,0xc3,0xc7,0xca,0xc6,0xbf,0xc7,0xbf,0xc8,0xce,0xbf,0xbe]

fileprivate func houseFile(unless num: UInt8) -> UInt8 {
...
/*: "icon_talk_notic" :*/
fileprivate let dataArtAfterFormat:String = "pi occur special electric recommendicon_tal"
fileprivate let app_phaseName:String = "k_noticbound reminder single photo"
...
@objc public protocol SalesDelegate: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: SuckThen)
    func dialogThumb(cell: SuckThen)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: SuckThen)
    func extensionPhoneCypher(cell: SuckThen)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: SuckThen)
    func promptbook(cell: SuckThen)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: SuckThen)
    func followingSum(cell: SuckThen)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: SuckThen)
    func spanMoment(cell: SuckThen)
}

//: @objcMembers public class SuckThen: TCommonTableViewCell {
@objcMembers public class SuckThen: OriginalThen {
    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\PressureReplyCellData.h
     */
    //: public var messageData: PressureReplyCellData?
    public var messageData: PressureReplyCellData?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(constProximateTitle.suffix(5)) + "emMe" + notiNatureValue.replacingOccurrences(of: "extend", with: "s") + "Cell" + String(show_produceUrl.prefix(4)) + "seId")

    /**
     *  协议委托
     *  负责实现 SalesDelegate 协议中的功能。
     */
    //: public weak var delegate: TMessageCellDelegate?
    public weak var delegate: SalesDelegate?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_yardMessage.map{houseFile(unless: $0)}, encoding: .utf8)!)
    }
    ...
    override public func metallicElement(with data: OldThen) {
        //: super.fill(with: data)
        super.metallicElement(with: data)
        //: let newData: PressureReplyCellData = data as! PressureReplyCellData
        let newData: PressureReplyCellData = data as! PressureReplyCellData
    ...
    override public func updateConstraints() {
        ...
        let csize = self.messageData?.netSize()
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(primaFacie(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        ...
        lb.font = UIFont.pingfangBusy(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(recognizer(recognizer:)))
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(selection(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        imagV.image = UIImage.scheme(name: (String(dataArtAfterFormat.suffix(8)) + String(app_phaseName.prefix(7))))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(installBold(recognizer:)))
...
extension SuckThen {
    ...
    @objc func primaFacie(recognizer _: UIGestureRecognizer) {
        //: self.delegate?.onSelectMessageAvatar(cell: self)
        self.delegate?.followingSum(cell: self)
    }

    //: @objc func onLongPress(recognizer: UIGestureRecognizer) {
    @objc func selection(recognizer: UIGestureRecognizer) {
