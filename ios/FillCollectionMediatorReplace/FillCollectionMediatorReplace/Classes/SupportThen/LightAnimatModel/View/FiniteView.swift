...
/*: "#ffffff" :*/
fileprivate let user_skinMsg:[Character] = ["#","f","f","f","f","f","f"]

/*: "/dist/loungePlus/index.html" :*/
fileprivate let mainPeerFailureUrl:String = "/dist/lonext include am"
fileprivate let mainEssentialKey:String = "ungePlless nose abs title"
fileprivate let constPolicyEquallyId:String = "remote away heavydex.html"

/*: "https://" :*/
fileprivate let main_correctTipFormat:String = "edit street slim pinghttps://"

/*: "http://" :*/
fileprivate let appPlusMessage:[Character] = ["h","t"]
fileprivate let noti_positionStr:String = "bad big snow blanktp://"
...
class FiniteView: UIView {
    ...
    let Broadcast_TextColor = (String(user_skinMsg))
    //: let Broadcast_ContentFont: UIFont = UIFont.pingfangMediumFont(fontSize: 12)
    let Broadcast_ContentFont: UIFont = .pingfangBusy(fontSize: 12)
    ...
    convenience init(model: BindHandyJSON?) {
        //: self.init()
        self.init()
    ...
    private func household() {
        ...
        if danumuMsgModel.headPic.count > 0 {
            ...
            headAnimatedImageView.scrubFinish(urlStr: danumuMsgModel.headPic)
        }
        //: if danumuMsgModel.bgPic.count > 0 {
        if danumuMsgModel.bgPic.count > 0 {
            //: updateBackground(withUrl: danumuMsgModel.bgPic)
            pic(withUrl: danumuMsgModel.bgPic)
        }
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            var mutableAtt = parse.indexBit(withFontSize: 12, insert: "", atIndex: 0)
            // 限制字符长度
    ...
    func pic(withUrl url: String) {
        //: self.backgroundImageView.setUrlImage(urlStr: url) {[weak self] result, image in
        self.backgroundImageView.commonClose(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.Broadcast_HeightOfBackgroundImageView {
                    ...
                    newImage = image.processIcon(with: CGSize(width: targetWidth, height: self.Broadcast_HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: BindHandyJSON? {
        //: didSet {
        didSet {
            //: configModel()
            household()
    ...
    var totalTime: CGFloat {
        ...
        return (show_explainId + widthValue) * Broadcast_SpendTime / show_explainId
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * Broadcast_LineSpendTime / ScreenWidth
        return widthValue * Broadcast_LineSpendTime / show_explainId
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * Broadcast_SpendTime / ScreenWidth
        return widthValue * Broadcast_SpendTime / show_explainId
    }
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(loadProduct(_:)))
        //: return t
        return t
        //: }()
    }()
    ...
    lazy var contentLabel: UILabel = {
        ...
        l.font = UIFont.pingfangBusy(fontSize: 12)
        //: l.textColor = .white
        l.textColor = .white
        //: l.numberOfLines = 1
        l.numberOfLines = 1
...
extension FiniteView {
    /// 弹幕点击跳转事件
    //: @objc func tapAction(_ tapGesture: UITapGestureRecognizer) {
    @objc func loadProduct(_: UITapGestureRecognizer) {
        ...
        //: case "1": // 个人中心
        case "1": // 个人中心
            //: ElementEarnReactiveCompatible.share.func__pushToUserDetailVC(uid: self.danumuMsgModel?.jumpKey)
            ElementEarnReactiveCompatible.share.datePress(uid: self.danumuMsgModel?.jumpKey)

        //: case "2": // 直播间
        case "2": // 直播间
            //: if let uid = self.danumuMsgModel?.jumpKey {
            if let uid = self.danumuMsgModel?.jumpKey {
                //: ElementEarnReactiveCompatible.share.func_audiencePushToLiveRoomVC(uid: uid, enterType: .unknown)
                ElementEarnReactiveCompatible.share.calledPerforate(uid: uid, enterType: .unknown)
            }
        ...
        case "3": // H5页面
            ...
            if let url = self.danumuMsgModel?.jumpKey {
                ...
                if url.contains((String(mainPeerFailureUrl.prefix(8)) + String(mainEssentialKey.prefix(6)) + "us/in" + String(constPolicyEquallyId.suffix(8)))) {
                ...
                //: let httpsUrl = AppManager.share.appConfigMode.urlH5Domain+url
                let httpsUrl = AppManager.share.appConfigMode.urlH5Domain + url
                //: let httpUrl = httpsUrl.replacingOccurrences(of: "https://", with: "http://")
                let httpUrl = httpsUrl.replacingOccurrences(of: (String(main_correctTipFormat.suffix(8))), with: (String(appPlusMessage) + String(noti_positionStr.suffix(5))))
                //: ElementEarnReactiveCompatible.share.func__pushToWebVC(urlStr: httpUrl)
                ElementEarnReactiveCompatible.share.lab(urlStr: httpUrl)
            }
...
extension FiniteView {
    ...
    private func oldBoyNetworkQuantityeraction() {
