...
fileprivate let user_versionTitle:[UInt8] = [0x10,0x17,0x10,0xd,0x51,0x1a,0x16,0x1d,0x1c,0xb,0x43,0x50,0x59,0x11,0x18,0xa,0x59,0x17,0x16,0xd,0x59,0x1b,0x1c,0x1c,0x17,0x59,0x10,0x14,0x9,0x15,0x1c,0x14,0x1c,0x17,0xd,0x1c,0x1d]

private func heavyEngage(judge num: UInt8) -> UInt8 {
    return num ^ 121
}

/*: "Online Users" :*/
fileprivate let mainEnableFocusRemindFormat:String = "Onlinenterprise gallery pose medal cookie"
...
// __CLOSE_PRINT__
//
//  BehaviorReactiveCompatible.swift
...
class BehaviorReactiveCompatible: UIView {
    //: private let contentV_H = (344+kDeviceSafeBottomHeight)
    private let contentV_H = (344 + user_videoMsg)
    //: private var DataSource: [TalkingLiveRoomOnlineListModel] = []
    private var DataSource: [InstanceTransformable] = []
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_versionTitle.map{heavyEngage(judge: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(streamButtonClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(mainEnableFocusRemindFormat.prefix(5)) + "e Users").localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    private lazy var tableView: UITableView = {
        ...
        table.register(ReflectionListCell.self, forCellReuseIdentifier: ReflectionListCell.className())
...
extension BehaviorReactiveCompatible {
    ...
    private func doData() {
        ...
        BlueReactiveCompatible().recognizeView(chatGroupId: self.chatGroupId) { succeed, result, _ in
            ...
            if succeed {
                ...
                var dataArr: [InstanceTransformable] = []

                //: if let datas = Array<TalkingLiveRoomOnlineListModel>.deserialize(from: array as? Array) {
                if let datas = Array<InstanceTransformable>.deserialize(from: array as? Array) {
                    //: dataArr.append(contentsOf: (datas as? [TalkingLiveRoomOnlineListModel] ?? []))
                    dataArr.append(contentsOf: (datas as? [InstanceTransformable] ?? []))
                }
...
extension BehaviorReactiveCompatible: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = ReflectionListCell.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingLiveRoomOnlineListCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? ReflectionListCell
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingLiveRoomOnlineListCell(style: .default, reuseIdentifier: identifier)
            cell = ReflectionListCell(style: .default, reuseIdentifier: identifier)
        }
        //: cell?.setCell(model: self.DataSource[indexPath.row], index: indexPath.row)
        cell?.setThe(model: self.DataSource[indexPath.row], index: indexPath.row)
        //: return cell!
        return cell!
    }
...
extension BehaviorReactiveCompatible {
    ...
    @objc private func streamButtonClick() {
        //: dismiss()
        iconMale()
    }
    ...
    func betweenShow(chatGroupId: String) {
...
extension BehaviorReactiveCompatible {
    ...
    private func multiConstraint() {
        ...
        let style = ValueEmptyStyle()
        //: emptyView = EmptyView.init(frame: CGRect(x: 0, y: -100, width: ScreenWidth, height: contentV_H+100), style: style)
        emptyView = EmptyView(frame: CGRect(x: 0, y: -100, width: show_explainId, height: contentV_H + 100), style: style)
