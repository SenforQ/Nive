...
fileprivate let showRemindName:[UInt8] = [0xe4,0xe3,0xe4,0xf9,0xa5,0xee,0xe2,0xe9,0xe8,0xff,0xb7,0xa4,0xad,0xe5,0xec,0xfe,0xad,0xe3,0xe2,0xf9,0xad,0xef,0xe8,0xe8,0xe3,0xad,0xe4,0xe0,0xfd,0xe1,0xe8,0xe0,0xe8,0xe3,0xf9,0xe8,0xe9]

private func outsideSlim(destination num: UInt8) -> UInt8 {
    return num ^ 141
}
...
enum TypeStayAdditiveArithmetic: Int {
    //: case TimeGap = 0
...
@objc protocol ReflectionViewDelegate: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func transformShowAd()

    //: @objc optional func endScroll()
    @objc optional func levelBy()
}

//: class TalkingDanmuMsgListTableView: UIView {
class MaintainViewDelegate: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showRemindName.map{outsideSlim(destination: $0)}, encoding: .utf8)!)
    }
...
extension MaintainViewDelegate {
    //: func startTimer() {
    func cock() {
        //: stopTimer()
        brightWho()
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(minimizeAttach), userInfo: nil, repeats: true)
    }
...
extension MaintainViewDelegate {
    ...
    func secure(msgModel: EnableDanmuModel?) {
    ...
    func albumCap() {
        ...
        if AllHeight > noti_restorePath {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < noti_restorePath {
                ...
                self.msgTableView.height = noti_restorePath
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = noti_restorePath - AllHeight
        ...
        smoothStage(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func smoothStage(animated: Bool) {
...
extension MaintainViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(k_timeTableStr)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = BalloonView.concern(tableView: tableView, msg: msgModel, indexPath: indexPath)
