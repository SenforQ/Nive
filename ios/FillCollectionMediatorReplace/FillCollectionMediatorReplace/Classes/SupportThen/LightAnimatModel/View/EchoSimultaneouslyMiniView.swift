...
fileprivate let showCompositionKey:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#2DF2FF" :*/
fileprivate let noti_produceFormat:String = "#2DF2along maybe seek world evaluate"
fileprivate let notiMeFormat:[Character] = ["F","F"]

/*: "Live" :*/
fileprivate let appOuterMsg:String = "format possible match statement listLive"

/*: "img_home_shadow_icon" :*/
fileprivate let mainNoteProcessingMsg:String = "img_htemplate very"
fileprivate let user_redUrl:String = "haenableow"
fileprivate let const_superiorListenValue:String = "eye bag day truth_icon"

/*: "get json error" :*/
fileprivate let showASeparateUrl:String = "aspect up like dealget j"
fileprivate let data_multiMaxMessage:[Character] = ["s","o","n"," ","e","r","r","o","r"]

/*: "#FF2348" :*/
fileprivate let data_contentName:String = "#FF2348section vertical"

/*: "#35C759" :*/
fileprivate let mainQuantityryPath:[Character] = ["#","3","5","C","7","5","9"]

/*: "mfBean" :*/
fileprivate let appHugeMsg:String = "native judgemfBean"

/*: "points\n≈ $" :*/
fileprivate let app_formatRobotKey:[Character] = ["p","o","i"]
fileprivate let app_instanceContent:String = "nts\n\u{2248} $"
...
// __CLOSE_PRINT__
//
//  EchoSimultaneouslyMiniView.swift
//  AbroadTalking
//
...
public var show_halfText: Int {
    //: return AppManager.share.appConfigMode.countdown1 + AppManager.share.appConfigMode.countdown2
    return AppManager.share.appConfigMode.countdown1 + AppManager.share.appConfigMode.countdown2
}

//: class TalkingLiveMiniView: TalkingBaseMiniView {
class EchoSimultaneouslyMiniView: GoodPhotoMiniView {
    // 未检测出人脸时长（间隔1秒）
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= AppManager.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                svgaNumLab.text = "\(show_halfText - noFaceTime)"
                //: } else {
            } else {
                //: refreshMiniView()
                vanguard()
    ...
    override init(frame: CGRect) {
        ...
        //: if Double(AppManager.share.loginUserMode.mf_bean) > 0 {
        if Double(AppManager.share.loginUserMode.mf_bean) > 0 {
            //: setPointText(mfBean: 0.0)
            borderAnima(mfBean: 0.0)
        }
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showCompositionKey.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var liveStatusImgV: ExtendedView = {
        ...
        let image = UIImage.productionSearch(color: UIColor(hex: (String(noti_produceFormat.prefix(5)) + String(notiMeFormat)))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.imageContainer(), for: .normal)
        img.setImage(image.imageContainer(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(appOuterMsg.suffix(4))).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: self.addSubview(img)
        self.addSubview(img)
    ...
    private lazy var shadowImg: UIImageView = {
        ...
        img.image = UIImage.scheme(name: (String(mainNoteProcessingMsg.prefix(5)) + "ome_s" + user_redUrl.replacingOccurrences(of: "enable", with: "d") + String(const_superiorListenValue.suffix(5))))
        //: img.layer.cornerRadius = 8
        img.layer.cornerRadius = 8
        //: img.layer.masksToBounds = true
        img.layer.masksToBounds = true
    ...
    private lazy var svgaPlayer: SVGAPlayer = {
        ...
        do {
            ...
            printLog(message: (String(showASeparateUrl.suffix(5)) + String(data_multiMaxMessage)))
        }
        //: return player
        return player
        //: }()
    ...
    private lazy var svgaNumLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(data_contentName.prefix(7))))
        //: self.addSubview(lab)
        self.addSubview(lab)
        //: return lab
        return lab
    ...
    private lazy var msgCountLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(mainQuantityryPath)))
        //: lab.font = UIFont.pingfangFont(type: .Medium, fontSize: 11)
        lab.font = UIFont.shareOff(type: .Medium, fontSize: 11)
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.textAlignment = .center
        lab.textAlignment = .center
...
extension EchoSimultaneouslyMiniView {
    /// 构建视频小窗口
    //: static func buildLiveMiniView() -> TalkingLiveMiniView {
    static func here() -> EchoSimultaneouslyMiniView {
        //: let view = TalkingLiveMiniView(frame: .zero)
        let view = EchoSimultaneouslyMiniView(frame: .zero)
        //: let position = TalkingSocketManager.shared.liveMiniPosition
        let position = MigrationSocketDelegate.shared.liveMiniPosition
        ...
        y = min(userSystemKey - view.bounds.size.height, y)
        //: view.frame = CGRect(x: x, y: y, width: view.width, height: view.height)
        view.frame = CGRect(x: x, y: y, width: view.width, height: view.height)
        //: TalkingSocketManager.shared.containerView?.addSubview(view)
        MigrationSocketDelegate.shared.containerView?.addSubview(view)
        //: return view
        return view
    }
    ...
    func vanguard() {
        ...
        liveStatusImgV.isHidden = !AfterPushListener.at().isLive
...
extension EchoSimultaneouslyMiniView {
    ...
    private func shadeNotification() {
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(pointchange(notification:)),
                                               selector: #selector(agent(notification:)),
    ...
    @objc private func agent(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: guard let mfBean = userinfo["mfBean"] as? Double else { return }
        guard let mfBean = userinfo[(String(appHugeMsg.suffix(6)))] as? Double else { return }
        //: setPointText(mfBean: mfBean)
        borderAnima(mfBean: mfBean)
    }

    //: func setPointText(mfBean: Double) {
    func borderAnima(mfBean: Double) {
        //: let nowBean = AppManager.share.loginUserMode.mf_bean + mfBean
        let nowBean = AppManager.share.loginUserMode.mf_bean + mfBean
        //: AppManager.share.loginUserMode.mf_bean = nowBean
        AppManager.share.loginUserMode.mf_bean = nowBean
        ...
        mfBeanLB.text = "\(Int(nowBean))\((String(app_formatRobotKey) + app_instanceContent).localized) \(divisor)"
