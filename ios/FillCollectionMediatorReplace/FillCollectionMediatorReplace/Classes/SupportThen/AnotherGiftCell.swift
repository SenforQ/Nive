...
fileprivate let kShakePoseId:[UInt8] = [0xf0,0xf7,0xf0,0xed,0xb1,0xfa,0xf6,0xfd,0xfc,0xeb,0xa3,0xb0,0xb9,0xf1,0xf8,0xea,0xb9,0xf7,0xf6,0xed,0xb9,0xfb,0xfc,0xfc,0xf7,0xb9,0xf0,0xf4,0xe9,0xf5,0xfc,0xf4,0xfc,0xf7,0xed,0xfc,0xfd]

/*: "icon_coinxxx" :*/
fileprivate let const_ditUrl:String = "channel explore transport presentationicon_"
fileprivate let constHighlightKey:[Character] = ["c","o","i","n","x","x","x"]

/*: "icon_gift_num_bg" :*/
fileprivate let kRemarkData:String = "compute away dry transform charicon_g"
fileprivate let noti_blankContent:String = "missing"
...
class AnotherGiftCell: UICollectionViewCell {
    //: var model = TalkingRoomGiftModel()
    var model = PrepareDisplayTransformable()
    //: let checked = false
    let checked = false
    //: let giftNum = 0
    let giftNum = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kShakePoseId.map{$0^153}, encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        ...
        lb.font = UIFont.shareOff(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: ExtendedView = {
        //: let btn = TalkingButton.init()
        let btn = ExtendedView()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.shareOff(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.scheme(name: (String(const_ditUrl.suffix(5)) + String(constHighlightKey))), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let imag = UIImage.scheme(name: (String(kRemarkData.suffix(6)) + "ift_num_b" + noti_blankContent.replacingOccurrences(of: "missing", with: "g")))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        lb.font = UIFont.shareOff(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
    }()
...
extension AnotherGiftCell {
    /// 更改选中状态
    //: func setChecked(checked: Bool) {
    func enable(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderColor = UIColor.retirementProgram().cgColor
    ...
    func largesse(currenmodel: PrepareDisplayTransformable, giftType: DirectoryValueConvertible) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.scrubFinish(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.scrubFinish(urlStr: model.cornerMark, placeImg: nil)
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.shareOff(type: .Regular, fontSize: 11),
...
extension AnotherGiftCell {
    ...
    func ownEnable() {
        ...
        bgView.backgroundColor = UIColor.resignHaphazard()
