...
fileprivate let kMiniTitle:[UInt8] = [0x18,0x1d,0x18,0x23,0xd7,0x12,0x1e,0x13,0x14,0x21,0xe9,0xd8,0xcf,0x17,0x10,0x22,0xcf,0x1d,0x1e,0x23,0xcf,0x11,0x14,0x14,0x1d,0xcf,0x18,0x1c,0x1f,0x1b,0x14,0x1c,0x14,0x1d,0x23,0x14,0x13]

fileprivate func makePro(pure num: UInt8) -> UInt8 {
...
/*: "icon_photo_giftbg" :*/
fileprivate let mainFenceDeviceDismissMsg:String = "icon_phonote service female"
fileprivate let kLovelyPath:String = "to_dimension occur"
fileprivate let noti_noseStr:String = "gismartbg"

/*: "Send" :*/
fileprivate let k_countAliveId:String = "Sendaway supposed save language measurement"

/*: "nav_back_black_nor" :*/
fileprivate let user_makerPath:[Character] = ["n","a","v","_","b","a","c","k"]
fileprivate let const_shadowConversionPath:String = "medium violation regain enjoy related_bla"
fileprivate let show_tarNaturalFormat:String = "would please sequence not latck_nor"

/*: "toUid" :*/
fileprivate let app_ceremonyMsg:[Character] = ["t","o","U","i","d"]

/*: "giftId" :*/
fileprivate let mainAdjustId:String = "giftIddrag popular size"

/*: "giftNum" :*/
fileprivate let mainSameFormat:String = "giftNumhalf wrap"

/*: "unlockMsgId" :*/
fileprivate let appCountryPath:[Character] = ["u","n","l","o","c","k","M","s","g"]
fileprivate let app_aircraftId:[Character] = ["I","d"]

/*: "Load failed, return to retry" :*/
fileprivate let const_readerPath:String = "track far always recordLoad "
fileprivate let userEnoughStr:String = "d, rnature continuing into advanced minute"
fileprivate let show_beginMsg:String = "eturn maximum coordinate happy bind"
fileprivate let mainCompareTitle:String = "to retryfollow choice tool have match"

/*: "Need a %@ (%@ Gold coins) to unlock" :*/
fileprivate let constPictureMsg:[UInt8] = [0x6b,0x63,0x6f,0x6c,0x6e,0x75,0x20,0x6f,0x74,0x20,0x29,0x73,0x6e,0x69,0x6f,0x63,0x20,0x64,0x6c,0x6f,0x47,0x20,0x40,0x25,0x28,0x20,0x40,0x25,0x20,0x61,0x20,0x64,0x65,0x65,0x4e]

/*: "icon_photo_lock" :*/
fileprivate let main_openlyTitle:String = "minute spend squareicon_"
fileprivate let noti_instanceStr:[Character] = ["o","c","k"]

/*: "Click on the screen to play this video(Burn After Reading)" :*/
fileprivate let k_tellPrisonId:[UInt8] = [0x29,0x67,0x6e,0x69,0x64,0x61,0x65,0x52,0x20,0x72,0x65,0x74,0x66,0x41,0x20,0x6e,0x72,0x75,0x42,0x28,0x6f,0x65,0x64,0x69,0x76,0x20,0x73,0x69,0x68,0x74,0x20,0x79,0x61,0x6c,0x70,0x20,0x6f,0x74,0x20,0x6e,0x65,0x65,0x72,0x63,0x73,0x20,0x65,0x68,0x74,0x20,0x6e,0x6f,0x20,0x6b,0x63,0x69,0x6c,0x43]

/*: "icon_photo_burn" :*/
fileprivate let main_visitorFormat:[Character] = ["i","c","o","n","_","p","h"]
fileprivate let appSelectFormat:String = "oto_burnaddress part eff field import"

/*: "Already burned" :*/
fileprivate let mainEarlyAliveId:[Character] = ["A","l","r","e","a","d","y"," ","b","u"]
fileprivate let userSpringMeanTinValue:String = "tempned"
...
// __CLOSE_PRINT__
//
//  SupportEnsureViewDelegate.swift
//  AbroadTalking
//
...
typealias IntimateVideoBlock = (_ model: SchemeExcludeModel) -> Void
//: typealias VideoStatusBlock = (_ status: VideoPlayerStatus) -> Void
typealias VideoStatusBlock = (_ status: MaskerPlayerStatus) -> Void

//: class TalkingIntimateVideoPreviewVC: TalkingBaseViewController {
class SupportEnsureViewDelegate: MirrorRealmReactiveCompatible {
    ...
    //: private var videoTotalData = ButtonReactiveCompatible(direction: .MsgDirectionIncoming)
    private var videoTotalData = ButtonReactiveCompatible(direction: .MsgDirectionIncoming) // 视频消息数据
    //: private var videoModel = AbTalkingChatMsgVideoModel()
    private var videoModel = SchemeExcludeModel() // 当前私密视频模型
    ...
    ///   - videoCellData: 视频模型
    ///   - toUid: 对方Id
    //: init(videoCellData: ButtonReactiveCompatible, toUid: String) {
    init(videoCellData: ButtonReactiveCompatible, toUid: String) {
        ...
        //: targetId = toUid
        targetId = toUid
        //: isMySelf = (videoTotalData.msgModel.user.uid == Int(AppManager.share.loginUserMode.userID))
        isMySelf = (videoTotalData.msgModel.user.uid == Int(AppManager.share.loginUserMode.userID))
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kMiniTitle.map{makePro(pure: $0)}, encoding: .utf8)!)
    }
    ...
    override func loadView() {
        ...
        if #available(iOS 13.2, *) {
            ...
            if AppManager.share.appConfigMode.disableShootScreen {
                ...
                let bgView = ArgumentView(frame: CGRect(x: 0, y: 0, width: show_explainId, height: userSystemKey - main_hugeFormat))
                //: self.view = bgView
                self.view = bgView
            }
            //: } else {
    ...
    private lazy var videoProgressView: BalloonThen = {
    ...
    private lazy var giftBgView: UIImageView = {
        ...
        img.image = UIImage.scheme(name: (String(mainFenceDeviceDismissMsg.prefix(8)) + String(kLovelyPath.prefix(3)) + noti_noseStr.replacingOccurrences(of: "smart", with: "ft")))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftIcon: UIImageView = {
        ...
        img.scrubFinish(urlStr: videoModel.giftIcon)
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var desLab: UILabel = {
        ...
        lab.font = UIFont.pingfangBusy(fontSize: 19)
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.setTitle((String(k_countAliveId.prefix(4))).localized, for: .normal)
        //: btn.setTitleColor(.white, for: .normal)
        btn.setTitleColor(.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Medium, fontSize: 19)
        btn.titleLabel?.font = UIFont.shareOff(type: .Medium, fontSize: 19)
        ...
        btn.addTarget(self, action: #selector(lash), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var progressView: CombinedIntervaldownView = {
        //: let frame = CGRect(x: ScreenWidth-42-15, y: StatusBarHeight+10, width: 42, height: 42)
        let frame = CGRect(x: show_explainId - 42 - 15, y: dataInfoTrustName + 10, width: 42, height: 42)
        //: let v = CircleCountdownView(frame: frame)
        let v = CombinedIntervaldownView(frame: frame)
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.scheme(name: (String(user_makerPath) + String(const_shadowConversionPath.suffix(4)) + String(show_tarNaturalFormat.suffix(6)))).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
        //: btn.addTarget(self, action: #selector(backButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(chooseMedia), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension SupportEnsureViewDelegate {
    /// 调用送礼接口
    /// - Parameters:
    ///   - giftId: 礼物Id
    ///   - giftNum: 礼物数量
    //: func req_sendGiftMsg(giftId: String, giftNum: String) {
    func implementConvert(giftId: String, giftNum: String) {
        ...
        params[(String(app_ceremonyMsg))] = self.targetId
        //: params["giftId"] = giftId
        params[(String(mainAdjustId.prefix(6)))] = giftId
        //: params["giftNum"] = giftNum
        params[(String(mainSameFormat.prefix(7)))] = giftNum
        //: params["unlockMsgId"] = self.videoTotalData.msgModel.msgInfo.msgId
        params[(String(appCountryPath) + String(app_aircraftId))] = self.videoTotalData.msgModel.msgInfo.msgId
...
extension SupportEnsureViewDelegate {
    ...
    @objc private func lash() {
        //: req_sendGiftMsg(giftId: videoModel.giftId, giftNum: "1")
        implementConvert(giftId: videoModel.giftId, giftNum: "1")
    }

    /// 阅后即焚15s倒计时开始
    //: @objc private func burnVideoTapGesture() {
    @objc private func throughWithGesture() {
        ...
        UniversalProgressHUD.increase(superView: self.view)
        ...
        self.videoStatusBlock = { [weak self] status in
            ...
            guard status == .Playing else {
                ...
                if status == .Failed {
                    ...
                    self.diskTerms(showMsg: (String(const_readerPath.suffix(5)) + "faile" + String(userEnoughStr.prefix(4)) + String(show_beginMsg.prefix(6)) + String(mainCompareTitle.prefix(8))).localized)
                }
                //: return
                return
            }
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
                ...
                self.progressView.journey(totalTime: 600, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension SupportEnsureViewDelegate {
    ...
    private func scanOf(urlStr: String, isMosaic: Bool) {
        ...
        guard isMySelf == false else { // 本人发的图片，展示本地缓存资源
            ...
            //: if isMosaic {
            if isMosaic {
                //: self.imageView.image = UIImage.step(image, add: 90)
                self.imageView.image = UIImage.step(image, add: 90)
                //: } else {
            } else {
        ...
        imageView.linemen(urlStr: urlStr) { [weak self] image in
            //: guard let self = self else { return }
            guard let self = self else { return }
    ...
    private func clickVideo() {
        ...
        self.player.occur(url: videoPath, view: self.videoView)
    }
}

// MARK: - GamePlanPlayerDelegate, EconomyThen

//: extension TalkingIntimateVideoPreviewVC: TalkingVideoPlayerDelegate, InteractionViewDelegate {
extension SupportEnsureViewDelegate: GamePlanPlayerDelegate, EconomyThen {
    ...
    func full(player _: SalesReactiveCompatible, status: MaskerPlayerStatus) {
        ...
        self.videoProgressView.scheduleStatus(status: status)
        //: if status == .Playing {
        if status == .Playing {
    ...
    func identityAppearance(player _: SalesReactiveCompatible, duration: Int, currentTime: Int) {
        ...
        self.videoProgressView.original(duration: duration, currentTime: currentTime)
    }
    ...
    func starBright(player _: SalesReactiveCompatible, progress: CGFloat) {
        ...
        self.videoProgressView.everyLanguage(value: progress)
    }
...
extension SupportEnsureViewDelegate {
    ...
    private func shrink() {
        ...
        videoProgressView.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(main_hugeFormat + 30)
            //: make.bottom.equalToSuperview().offset(-(kDeviceSafeBottomHeight+64))
            make.bottom.equalToSuperview().offset(-(user_videoMsg + 64))
        }
        ...
        desLab.snp.makeConstraints { make in
            ...
            make.centerY.equalToSuperview().offset(main_hugeFormat / 2)
        }
        ...
        progressView.snp.remakeConstraints { make in
            ...
            make.leading.equalTo(show_explainId - 42 - 15)
    ...
    private func goInput() {
        ...
        if videoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.scanOf(urlStr: videoModel.coverImg, isMosaic: true)
            ...
            self.videoProgressView.original(duration: videoModel.duration, currentTime: 0)
            //: self.maskView.isHidden = false
            self.maskView.isHidden = false
            //: self.desLab.text = "Need a %@ (%@ Gold coins) to unlock".localizedArguments(videoModel.giftName, videoModel.giftPrice)
            self.desLab.text = String(bytes: constPictureMsg.reversed(), encoding: .utf8)!.queryionLocalized(videoModel.giftName, videoModel.giftPrice)
            ...
            if videoModel.burnStatus == 0 { // 未销毁
                ...
                self.intimateStatusImgV.image = UIImage.scheme(name: (String(main_openlyTitle.suffix(5)) + "photo_l" + String(noti_instanceStr)))
                //: self.desLab.text = "Click on the screen to play this video(Burn After Reading)".localized
                self.desLab.text = String(bytes: k_tellPrisonId.reversed(), encoding: .utf8)!.localized
                // 添加手势
                //: let tap = UITapGestureRecognizer(target: self, action: #selector(burnVideoTapGesture))
                let tap = UITapGestureRecognizer(target: self, action: #selector(throughWithGesture))
                ...
                self.intimateStatusImgV.image = UIImage.scheme(name: (String(main_visitorFormat) + String(appSelectFormat.prefix(8))))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(mainEarlyAliveId) + userSpringMeanTinValue.replacingOccurrences(of: "temp", with: "r")).localized
            }
