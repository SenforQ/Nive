...
fileprivate let app_beautyName:[UInt8] = [0x67,0x6c,0x67,0x72,0x26,0x61,0x6d,0x62,0x63,0x70,0x38,0x27,0x1e,0x66,0x5f,0x71,0x1e,0x6c,0x6d,0x72,0x1e,0x60,0x63,0x63,0x6c,0x1e,0x67,0x6b,0x6e,0x6a,0x63,0x6b,0x63,0x6c,0x72,0x63,0x62]

fileprivate func mapStall(access num: UInt8) -> UInt8 {
...
/*: "icon_photo_giftbg" :*/
fileprivate let notiLabelValue:String = "icoprofessional"
fileprivate let dataInvitationMessage:String = "deny giving_giftbg"

/*: "Send" :*/
fileprivate let data_methodVidFiniteStr:[Character] = ["S","e","n","d"]

/*: "nav_back_black_nor" :*/
fileprivate let userHairKey:[Character] = ["n","a","v","_","b","a","c","k","_","b","l","a","c","k"]
fileprivate let userChapterBeforeName:String = "lovely memory superior one anonymous_nor"

/*: "toUid" :*/
fileprivate let app_recommendationStateProcedureKey:[Character] = ["t","o","U","i","d"]

/*: "giftId" :*/
fileprivate let const_uponStr:[Character] = ["g","i","f","t","I","d"]

/*: "giftNum" :*/
fileprivate let userHourValue:String = "manual promotion upgiftNum"

/*: "unlockMsgId" :*/
fileprivate let userShareMessage:[Character] = ["u","n","l","o","c","k","M","s","g","I","d"]

/*: "Need a %@ (%@ Gold coins) to unlock" :*/
fileprivate let notiChemicalKey:[UInt8] = [0x1,0x2a,0x2a,0x2b,0x6f,0x2e,0x6f,0x6a,0xf,0x6f,0x67,0x6a,0xf,0x6f,0x8,0x20,0x23,0x2b,0x6f,0x2c,0x20,0x26,0x21,0x3c,0x66,0x6f,0x3b,0x20,0x6f,0x3a,0x21,0x23,0x20,0x2c,0x24]

/*: "icon_photo_lock" :*/
fileprivate let main_minimumId:String = "icon_none permit selection environment alongside"
fileprivate let k_worldPath:[Character] = ["_","l","o","c","k"]

/*: "Click on the screen to view 15s" :*/
fileprivate let kBoardMsg:[UInt8] = [0x87,0xa8,0xad,0xa7,0xaf,0xe4,0xab,0xaa,0xe4,0xb0,0xac,0xa1,0xe4,0xb7,0xa7,0xb6,0xa1,0xa1,0xaa,0xe4,0xb0,0xab,0xe4,0xb2,0xad,0xa1,0xb3,0xe4,0xf5,0xf1,0xb7]

/*: "icon_photo_burn" :*/
fileprivate let showCellMessage:String = "figure invitation legitimateicon_p"
fileprivate let dataSortMessage:[Character] = ["h","o","t","o","_","b","u","r","n"]

/*: "Already burned" :*/
fileprivate let show_wantName:String = "Alreabucket affect permit"
fileprivate let noti_monthValue:String = "urthand"
...
// __CLOSE_PRINT__
//
//  MugShotReactiveCompatible.swift
//  AbroadTalking
//
...
typealias IntimatePhotoBlock = (_ model: LaterModel) -> Void

//: class TalkingIntimatePhotoPreviewVC: TalkingBaseViewController {
class MugShotReactiveCompatible: MirrorRealmReactiveCompatible {
    ...
    //: private var imageTotalData = MigrationCellData(direction: .MsgDirectionOutgoing)
    private var imageTotalData = MigrationCellData(direction: .MsgDirectionOutgoing) // 图片消息数据
    //: private var photoModel = AbTalkingChatMsgIntimatePhotoModel()
    private var photoModel = LaterModel() // 当前私密照片模型
    ...
    ///   - imageCellData: 图片模型
    ///   - toUid: 对方Id
    //: init(imageCellData: MigrationCellData, toUid: String) {
    init(imageCellData: MigrationCellData, toUid: String) {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_beautyName.map{mapStall(access: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var giftBgView: UIImageView = {
        ...
        img.image = UIImage.scheme(name: (notiLabelValue.replacingOccurrences(of: "professional", with: "n") + "_photo" + String(dataInvitationMessage.suffix(7))))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftIcon: UIImageView = {
        ...
        img.scrubFinish(urlStr: photoModel.giftIcon)
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.setTitle((String(data_methodVidFiniteStr)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(devise), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.scheme(name: (String(userHairKey) + String(userChapterBeforeName.suffix(4)))).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
        //: btn.addTarget(self, action: #selector(backButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(limitMin), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension MugShotReactiveCompatible {
    /// 调用送礼接口
    /// - Parameters:
    ///   - giftId: 礼物Id
    ///   - giftNum: 礼物数量
    //: func req_sendGiftMsg(giftId: String, giftNum: String) {
    func beChild(giftId: String, giftNum: String) {
        ...
        params[(String(app_recommendationStateProcedureKey))] = self.targetId
        //: params["giftId"] = giftId
        params[(String(const_uponStr))] = giftId
        //: params["giftNum"] = giftNum
        params[(String(userHourValue.suffix(7)))] = giftNum
        //: params["unlockMsgId"] = self.imageTotalData.msgModel.msgInfo.msgId
        params[(String(userShareMessage))] = self.imageTotalData.msgModel.msgInfo.msgId
...
extension MugShotReactiveCompatible {
    ...
    @objc private func devise() {
        //: req_sendGiftMsg(giftId: photoModel.giftId, giftNum: "1")
        beChild(giftId: photoModel.giftId, giftNum: "1")
    }

    /// 阅后即焚15s倒计时开始
    //: @objc private func burnPhotoTapGesture() {
    @objc private func cutDestroy() {
        ...
        self.photoExtra(urlStr: photoModel.originUrl, isMosaic: false) { [weak self] isSucceed in
            //: ProgressHUD.dismiss()
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
                ...
                self.progressView.journey(totalTime: 150, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension MugShotReactiveCompatible {
    ...
    private func neuter() {
        ...
        if photoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.desLab.text = String(bytes: notiChemicalKey.map{$0^79}, encoding: .utf8)!.queryionLocalized(photoModel.giftName, photoModel.giftPrice)
            ...
            if photoModel.burnStatus == 0 { // 未销毁
                ...
                self.intimateStatusImgV.image = UIImage.scheme(name: (String(main_minimumId.prefix(5)) + "photo" + String(k_worldPath)))
                //: self.desLab.text = "Click on the screen to view 15s".localized
                self.desLab.text = String(bytes: kBoardMsg.map{$0^196}, encoding: .utf8)!.localized
                // 添加手势
                //: let tap = UITapGestureRecognizer(target: self, action: #selector(burnPhotoTapGesture))
                let tap = UITapGestureRecognizer(target: self, action: #selector(cutDestroy))
                ...
                self.intimateStatusImgV.image = UIImage.scheme(name: (String(showCellMessage.suffix(6)) + String(dataSortMessage)))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(show_wantName.prefix(5)) + "dy b" + noti_monthValue.replacingOccurrences(of: "than", with: "ne")).localized
            }
        ...
        self.photoExtra(urlStr: imgUrlStr, isMosaic: imgIsMosaic)
    }
    ...
    private func photoExtra(urlStr: String, isMosaic: Bool, completionHandler: ((_ isSucceed: Bool) -> Void)? = nil) {
        ...
        guard isMySelf == false else { // 本人发的图片，展示本地缓存资源
            ...
            filePath = (filePath as NSString).appendingPathComponent(userPurchasePath)
