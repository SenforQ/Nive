...
fileprivate let show_islandUrl:[UInt8] = [0x66,0x6b,0x66,0x71,0x25,0x60,0x6c,0x61,0x62,0x6f,0x37,0x26,0x1d,0x65,0x5e,0x70,0x1d,0x6b,0x6c,0x71,0x1d,0x5f,0x62,0x62,0x6b,0x1d,0x66,0x6a,0x6d,0x69,0x62,0x6a,0x62,0x6b,0x71,0x62,0x61]

fileprivate func civilSame(show num: UInt8) -> UInt8 {
    let value = Int(num) + 3
    if value > 255 {
...
/*: "AdditionLightThen" :*/
fileprivate let constHungPath:String = "house seaDrop"
fileprivate let show_zzData:String = "erMenuCelshould harsh sheer"
fileprivate let app_shouldLimitCanData:String = "on"

/*: "roomId" :*/
fileprivate let showFlushFeeKey:String = "ask pure sum domain bridgeroomId"

/*: "type" :*/
fileprivate let appMobileStr:[Character] = ["t","y","p","e"]

/*: "page" :*/
fileprivate let userDownAllThatStr:String = "paexposure"

/*: "icon_chats_pcr" :*/
fileprivate let data_gravityTitle:[Character] = ["i","c","o","n","_","c","h"]
fileprivate let dataExitTitle:String = "cur fault viaats_pcr"

/*: "All Numbers" :*/
fileprivate let notiApprovalVisitorStr:String = "push"
fileprivate let userCameraPath:[Character] = ["l","l"," ","N","u","m","b","e","r","s"]

/*: "all" :*/
fileprivate let show_towardKey:[Character] = ["a","l","l"]

/*: "Recent Gifts" :*/
fileprivate let data_firstKey:String = "gross mouth given merge provideRecent"
fileprivate let const_portText:[Character] = [" "]
fileprivate let show_readyId:String = "Giftsfemale position task popularity attach"
...
class BagExtraViewDelegate: UIView {
    //: var pageIndex = 0
    var pageIndex = 0
    //: var roomID = ""
    var roomID = ""
    //: var menuDelegate: DropDownMemberMenuViewDelegate?
    var menuDelegate: PrepareViewDelegate?
    ...
    public var triangleHeight = Config.PriceConfig.triangleHeight
    /**
     *  调整使下拉优先 当向下偏转屏幕距离足够，优先向下偏转
     */
    //: public var adjustPullDown = Config.DropMenuConfig.adjustPullDown
    public var adjustPullDown = Config.PriceConfig.adjustPullDown
    ...
    fileprivate lazy var contentView: UIView = self.lockout()
    //: fileprivate lazy var mTable: UITableView = self.makeTableView()
    fileprivate lazy var mTable: UITableView = self.marque()
    //: fileprivate var anchorRect = CGRect.zero
    fileprivate var anchorRect = CGRect.zero

    //: static let shared = DropDownMemberMenuView(frame: .zero)
    static let shared = BagExtraViewDelegate(frame: .zero)
    ...
    init(frame: CGRect, anchorView: UIView, roomID: String) {
        ...
        super.init(frame: frame)
        //: let window = DelineateThen.getWindow()
        let window = DelineateThen.premiumOperation()
        //: self.roomID = roomID
        self.roomID = roomID
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_islandUrl.map{civilSame(show: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func lockout() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func marque() -> UITableView {
    ...
    private func writtenSumry() {
        ...
        mTable.register(AdditionLightThen.self, forCellReuseIdentifier: (String(constHungPath.suffix(4)) + "Memb" + String(show_zzData.prefix(9)) + app_shouldLimitCanData.replacingOccurrences(of: "on", with: "l")))
    ...
    func stripLight() {
        ...
        var dict = [String: Any]()
        //: dict["roomId"] = roomID
        dict[(String(showFlushFeeKey.suffix(6)))] = roomID
        //: dict["type"] = "3"
        dict[(String(appMobileStr))] = "3"
        //: dict["page"] = String(pageIndex)
        dict[(userDownAllThatStr.replacingOccurrences(of: "exposure", with: "ge"))] = String(pageIndex)
        ...
        LabelGiftManager.share.dataCombine(params: dict, completion: { array in
            ...
            if self.pageIndex == 0 {
                ...
                let model = PossibilityUponReactiveCompatible()
                //: model.headPic = "icon_chats_pcr"
                model.headPic = (String(data_gravityTitle) + String(dataExitTitle.suffix(7)))
                //: model.nickname = "All Numbers".localized
                model.nickname = (notiApprovalVisitorStr.replacingOccurrences(of: "push", with: "A") + String(userCameraPath)).localized
                //: model.uid = "all"
                model.uid = (String(show_towardKey))
...
extension BagExtraViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(constHungPath.suffix(4)) + "Memb" + String(show_zzData.prefix(9)) + app_shouldLimitCanData.replacingOccurrences(of: "on", with: "l")), for: indexPath) as! AdditionLightThen
        ...
        if recentmenuArray.count > 0 && indexPath.section == 0 {
            ...
            cellModel = recentmenuArray[indexPath.row] as! PossibilityUponReactiveCompatible
            //: }else {
        } else {
            //: cellModel = menuArray[indexPath.row] as! TalkingChatRoomMemberModel
            cellModel = menuArray[indexPath.row] as! PossibilityUponReactiveCompatible
        }
        //: cell.setDropMemberMenuCell(model: cellModel)
        cell.sumerestFee(model: cellModel)
        //: return cell
        return cell
    }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        menuDelegate?.when(model: cellModel)
    }

    //: func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    func tableView(_: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        //: let headeView = UILabel.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: 23))
        let headeView = UILabel(frame: CGRect(x: 0, y: 0, width: show_explainId, height: 23))
        //: headeView.backgroundColor = UIColor.appBgColor()
        headeView.backgroundColor = UIColor.referenceAndNonsolidColour()
        //: let titleLB = UILabel.init()
        let titleLB = UILabel()
        //: titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        //: titleLB.font = .pingfangFont(type: .Medium, fontSize: 12)
        titleLB.font = .shareOff(type: .Medium, fontSize: 12)
        //: titleLB.textColor = .appValueDetailColor()
        titleLB.textColor = .eigenvalueOfAMatrix()
        //: if recentmenuArray.count > 0 && section == 0 {
        if recentmenuArray.count > 0 && section == 0 {
            //: titleLB.text = "Recent Gifts".localized
            titleLB.text = (String(data_firstKey.suffix(6)) + String(const_portText) + String(show_readyId.prefix(5))).localized
            //: }else {
        } else {
            //: titleLB.text = "All Numbers".localized
            titleLB.text = (notiApprovalVisitorStr.replacingOccurrences(of: "push", with: "A") + String(userCameraPath)).localized
        }
