...
/*: "uid" :*/
fileprivate let constPositionData:[Character] = ["u","i","d"]

/*: "onlineStatus" :*/
fileprivate let appFlagSoapUrl:String = "permissionnline"
fileprivate let show_entityMsg:String = "one license outer uniteStatus"

/*: "isNewUser" :*/
fileprivate let main_attributeUrl:[Character] = ["i","s","N","e","w","U","s","e","r"]

/*: "isTPAuth" :*/
fileprivate let const_followingBackRemarkStr:[Character] = ["i","s","T","P","A","u","t","h"]

/*: "totalIntimate" :*/
fileprivate let main_bulletSmartData:String = "rolling currency shirt permission correcttotalI"
fileprivate let dataMenuMsg:String = "TE"

/*: "userStatus" :*/
fileprivate let showNamePath:String = "usnod"
fileprivate let showAnotherRegionThreadData:[Character] = ["r","S","t","a","t","u","s"]

/*: "intimate" :*/
fileprivate let const_offFormat:[Character] = ["i","n","t"]
fileprivate let mainBehindValue:[Character] = ["i","m","a","t","e"]
...
enum SecretPlanQuantityLiteral: Int {
    //: case offical  = 0
    case offical = 0 // 官方
    //: case chatRoom = 1
    case chatRoom = 1 // 聊天室
...
enum ExtendedQuickLookable: Int {
    //: case top  = 0
    case top = 0 // 置顶
    //: case normal = 1
    case normal = 1 // 默认
...
class SizeListManager {
    ...
    private var moreMsgModel: MiddleThen = //: return MiddleThen.getPlaceholderModel(chatType: .moreMsg)
        .fun(chatType: .moreMsg)
    //: }()

    // 初始化数据
    //: func req_initData() {
    func guidance() {
        //: self.officialArr = ExistLeastManager.shared.getOfficialList()
        self.officialArr = ExistLeastManager.shared.advertisingOff()
        //: self.topArr = ExistLeastManager.shared.topConvList
        self.topArr = ExistLeastManager.shared.topConvList
        //: self.normalArr = ExistLeastManager.shared.norConvList
        self.normalArr = ExistLeastManager.shared.norConvList
        // 更多数据
        //: req_moreMsgInitData()
        exit()
    ...
    func cherry(completion: @escaping (_ succeed: Bool) -> Void) {
        //: guard self.isHalf == false else { return }
        guard self.isHalf == false else { return }
        //: ExistLeastManager.shared.getChatRoomData { succeed in
        ExistLeastManager.shared.orientationCompletion { succeed in
            //: if succeed {
            if succeed {
                //: self.chatRoomArr = ExistLeastManager.shared.chatRoomList
                self.chatRoomArr = ExistLeastManager.shared.chatRoomList
                //: completion(true)
                completion(true)
            }
    ...
    func successPath(indexPath: IndexPath) -> MiddleThen? {
    ...
    func afterBorder(model: MiddleThen?) -> MiddleThen? {
        //: guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        //: let userInfo: AccomplishThen? = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId)
        let userInfo: AccomplishThen? = FiniteInfoManager.doingeAgain(targetId: targetId)
    ...
    func cache(conversationList: [MiddleThen], finish: (() -> Void)?) {
        //: var tempArr = [String]()
        var tempArr = [String]()
        ...
        FiniteInfoManager.termsMap(with: tempArr) { _, error in
    ...
    func theoreticalAccount(conversationModel: MiddleThen) {
    ...
    func submitRecover(completion: @escaping (_ isFinish: Bool) -> Void) {
        //: ExistLeastManager.shared.func__getConversationList { arr, leftMore in
        ExistLeastManager.shared.discOrBright { _, leftMore in
            //: completion(leftMore)
            completion(leftMore)
        }
    }
    ...
    lazy var officialArr: [MiddleThen] = //: return Array<MiddleThen>()
        .init()
    //: }()

    //: lazy var topArr: [TalkingConversationModel] = {
    lazy var topArr: [MiddleThen] = //: return Array<MiddleThen>()
        .init()
    //: }()

    //: lazy var normalArr: [TalkingConversationModel] = {
    lazy var normalArr: [MiddleThen] = //: return Array<MiddleThen>()
        .init()
    //: }()

    //: lazy var chatRoomArr: [TalkingConversationModel] = {
    lazy var chatRoomArr: [MiddleThen] = //: return Array<MiddleThen>()
        .init()
    //: }()

    //: lazy var intimateNorArr: [TalkingConversationModel] = {
    lazy var intimateNorArr: [MiddleThen] = //: return Array<MiddleThen>()
        .init()
    //: }()

    //: lazy var intimateTopArr: [TalkingConversationModel] = {
    lazy var intimateTopArr: [MiddleThen] = //: return Array<MiddleThen>()
        .init()
    //: }()

    //: lazy var moreMsgArr: [TalkingConversationModel] = {
    lazy var moreMsgArr: [MiddleThen] = //: return Array<MiddleThen>()
        .init()
    //: }()
}
...
extension SizeListManager {
    ...
    func exit() {
        //: self.moreMsgArr = ExistLeastManager.shared.moreMsgList
        self.moreMsgArr = ExistLeastManager.shared.moreMsgList
    }

    /// 根据索引获取模型（更多消息）
    //: func req_moreMsgConversationModel(indexPath: IndexPath) -> TalkingConversationModel? {
    func recordTop(indexPath: IndexPath) -> MiddleThen? {
    ...
    func failure() -> MiddleThen {
        ...
        moreMsgModel.unreadCount = calculateCollection()
...
extension SizeListManager {
    ...
    func completionNet(isFirst: Bool, completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        FiniteInfoManager.enablee(with: data) { resultArr, _ in
            ...
            if resultArr?.count ?? 0 <= 0 { // 无数据
                ...
                resultArr?.forEach { dict in
                    ...
                    let uid = json[(String(constPositionData))].stringValue
                    //: let onlineStatus = json["onlineStatus"].intValue
                    let onlineStatus = json[(appFlagSoapUrl.replacingOccurrences(of: "permission", with: "o") + String(show_entityMsg.suffix(6)))].intValue
                    //: let isNewUser = json["isNewUser"].boolValue
                    let isNewUser = json[(String(main_attributeUrl))].boolValue
                    //: let istpAuth = json["isTPAuth"].boolValue
                    let istpAuth = json[(String(const_followingBackRemarkStr))].boolValue
                    //: let totalIntimate = json["totalIntimate"].int ?? 0
                    let totalIntimate = json[(String(main_bulletSmartData.suffix(6)) + "ntima" + dataMenuMsg.lowercased())].int ?? 0
                    //: let userStatus = json["userStatus"].int ?? 1
                    let userStatus = json[(showNamePath.replacingOccurrences(of: "nod", with: "e") + String(showAnotherRegionThreadData))].int ?? 1
                    ...
                    self.modifyEachMagnitudeimate(uid: uid, intimate: totalIntimate)
                    //: })
                }
            }
            //: completion(true)
    ...
    func package(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(String(constPositionData))] as! String
        //: let onlineStatus = userInfo["onlineStatus"] as! Int
        let onlineStatus = userInfo[(appFlagSoapUrl.replacingOccurrences(of: "permission", with: "o") + String(show_entityMsg.suffix(6)))] as! Int
        //: let isNewUser = userInfo["isNewUser"] as! Bool
        let isNewUser = userInfo[(String(main_attributeUrl))] as! Bool

        /// 用户状态更新
        //: let userStatus = userInfo["userStatus"] as? Int ?? 1
        let userStatus = userInfo[(showNamePath.replacingOccurrences(of: "nod", with: "e") + String(showAnotherRegionThreadData))] as? Int ?? 1
        //: var needuserStatus = false
        var needuserStatus = false
        //: if userInfo.keys.contains("userStatus") {
        if userInfo.keys.contains((showNamePath.replacingOccurrences(of: "nod", with: "e") + String(showAnotherRegionThreadData))) {
            //: needuserStatus = true
            needuserStatus = true
        }
...
extension SizeListManager {
    ...
    func reasonPartner() {
        ...
        self.intimateTopArr.removeAll()
        //: for model in ExistLeastManager.shared.norConvList {
        for model in ExistLeastManager.shared.norConvList {
            //: if model.totalIntimate >= AppManager.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= AppManager.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
                let userModel = self.afterBorder(model: model)
                //: self.intimateNorArr.append(userModel!)
                self.intimateNorArr.append(userModel!)
            }
        }
        //: for model in ExistLeastManager.shared.topConvList {
        for model in ExistLeastManager.shared.topConvList {
            //: if model.totalIntimate >= AppManager.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= AppManager.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
    ...
    func asData(convModel: MiddleThen, isTop: Bool) {
        //: if isTop {
        if isTop {
    ...
    func urge(indexPath: IndexPath) -> MiddleThen? {
        //: switch indexPath.section {
        switch indexPath.section {
    ...
    func modifyEachMagnitudeimate(uid: String, intimate: Int) {
        ...
        FiniteInfoManager.ghb(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
    }

    /// 更新消息列表单个用户亲密度
    //: func update_userIntimate(userInfo: [String: Any]) -> Bool {
    func duringBind(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(String(constPositionData))] as! String
        //: let intimate = userInfo["intimate"] as! Int
        let intimate = userInfo[(String(const_offFormat) + String(mainBehindValue))] as! Int
        //: var updateSucceed = false
        var updateSucceed = false
        //: if let topModel = self.topArr.filter({ $0.userID == uid }).first {
        if let topModel = self.topArr.filter({ $0.userID == uid }).first {
