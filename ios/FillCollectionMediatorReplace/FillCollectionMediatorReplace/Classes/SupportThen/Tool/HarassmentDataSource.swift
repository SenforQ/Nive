...
fileprivate let user_literalTitle:[UInt8] = [0x7e,0x79,0x7e,0x63,0x3f,0x74,0x78,0x73,0x72,0x65,0x2d,0x3e,0x37,0x7f,0x76,0x64,0x37,0x79,0x78,0x63,0x37,0x75,0x72,0x72,0x79,0x37,0x7e,0x7a,0x67,0x7b,0x72,0x7a,0x72,0x79,0x63,0x72,0x73]

private func whatRecover(maybe num: UInt8) -> UInt8 {
    return num ^ 23
}

/*: "SHBDropMenuCell" :*/
fileprivate let dataShallFormat:String = "mast stretch properlySHBDro"
fileprivate let constReplyFormat:String = "LL"

/*: "title" :*/
fileprivate let show_stickMsg:String = "toptiontle"
...
public class HarassmentDataSource: UIView {
    //: var menuDelegate: DropDownMenuViewDelegate?
    var menuDelegate: SomeoneViewDelegate?
    ...
    var menuBgColor: UIColor? {
        ...
        //: }
    } // = Config.PriceConfig.menuBgColor
    // 线条颜色
    //: var lineColor = Config.MenuCellConfig.lineColor
    var lineColor = Config.BrightConfig.lineColor
    // cell高度
    //: public var menuCellHeight = Config.MenuCellConfig.menuCellHeight
    public var menuCellHeight = Config.BrightConfig.menuCellHeight
    /**
     *  table最大高度限制
     *  默认：5 * cellHeight
     */
    //: var menuMaxHeight = Config.MenuCellConfig.menuMaxHeight
    var menuMaxHeight = Config.BrightConfig.menuMaxHeight
    /**
     *  小三角高度
     *  45°等腰三角形
     */
    ...
    public var menuStyle = Config.initialMenuStyle ?? BehaviorMenuStyle.MenuDarkStyle {
        //: didSet {
        didSet {
            //: switch menuStyle {
            switch menuStyle {
...
//        menuView.refreshUI()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_literalTitle.map{whatRecover(maybe: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func omission() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func calculateUi() {
        //: addSubview(contentView)
        addSubview(contentView)
        //: mTable.register(DropMenuCell.self, forCellReuseIdentifier: "SHBDropMenuCell")
        mTable.register(KingThen.self, forCellReuseIdentifier: (String(dataShallFormat.suffix(6)) + "pMenuCe" + constReplyFormat.lowercased()))
    ...
    private func goldAfterFramework() {
...
extension HarassmentDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(dataShallFormat.suffix(6)) + "pMenuCe" + constReplyFormat.lowercased()), for: indexPath) as! KingThen
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        selectionAction?(indexPath.row, cellModel.title ?? (show_stickMsg.replacingOccurrences(of: "option", with: "i")))
        //: animateRemoveView()
        angleStatistical()

        //: menuDelegate?.didClickSelectedRow(index: indexPath.row, title: cellModel.title ?? "")
        menuDelegate?.animalSend(index: indexPath.row, title: cellModel.title ?? "")
    }
}
