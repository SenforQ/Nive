...
/*: "gifts" :*/
fileprivate let app_taValue:[Character] = ["g","i","f","t","s"]

/*: "vipGifts" :*/
fileprivate let show_warningPath:[Character] = ["v","i","p","G","i","f","t","s"]

/*: "packGifts" :*/
fileprivate let showSampleValue:String = "packGiftdrop cap people conversion jaw"
fileprivate let showResentPath:String = "statement"

/*: "giftSendItem" :*/
fileprivate let data_shadeMsg:[Character] = ["g","i","f","t","S","e"]
fileprivate let show_responsePath:String = "ndItemit transition communicate estimated"
...
class LabelGiftManager: NSObject {
    /// 是否展示背包红点
    ...
    private var cacheDict = [String: [DirectoryValueConvertible: [Any]]]()
    // 数据锁
    ...
    private var giftType = ViewCustomReflectable.normal

    //: static let share = TalkingChatGiftManager()
    static let share = LabelGiftManager()
...
extension LabelGiftManager {
    ...
    func magnitudeervalCompletion(type: ViewCustomReflectable = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        LabelGiftManager.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[DirectoryValueConvertible.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            LabelGiftManager.share.colourBlind(completion: { isSucceed in
            ...
            LabelGiftManager.share.completion(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            LabelGiftManager.share.captureGuide(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func captureGuide(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = LabelGiftManager.share.giftType
        //: TalkingChatRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        HouseThen.childEach(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [DirectoryValueConvertible: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(String(app_taValue))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<PrepareDisplayTransformable>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[DirectoryValueConvertible.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(show_warningPath))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<PrepareDisplayTransformable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[DirectoryValueConvertible.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(showSampleValue.prefix(8)) + showResentPath.replacingOccurrences(of: "statement", with: "s"))] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<PrepareDisplayTransformable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[DirectoryValueConvertible.Bags] = bagsArr
                }
                ...
                var firstModel = SchemeTransformable()
                //: firstModel.num = 1
                firstModel.num = 1
                ...
                let sendItem: Array = data[(String(data_shadeMsg) + String(show_responsePath.prefix(6)))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<SchemeTransformable>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func colourBlind(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = LabelGiftManager.share.giftType
        //: TalkingChatRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        HouseThen.productCompletion(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func dataCombine(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: TalkingChatRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        HouseThen.completeCompletion(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<PossibilityUponReactiveCompatible>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func completion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = ViewCustomReflectable.live
        //: TalkingChatRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        HouseThen.make(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func commonType(perPageCount: Int, dataType: DirectoryValueConvertible) -> Int {
    ...
    func nameType(dataType: DirectoryValueConvertible) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func charmQuote() -> [Any] {
        ...
        if LabelGiftManager.share.giftType == .groupChat {
            //: return chatRoomSendItems
            return chatRoomSendItems
            //: } else if TalkingChatGiftManager.share.giftType == .live {
        } else if LabelGiftManager.share.giftType == .live {
    ...
    func beyondAlongside(dataType: DirectoryValueConvertible, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension LabelGiftManager {
    ...
    private func dropNotification() {
        // 刷新礼物背包
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(receiveNewGiftPackage),
                                               selector: #selector(fence),
    ...
    @objc private func fence() {
        //: gift_clearMemoryCache(showRed: true)
        skinNeedRed(showRed: true)
    }
}
...
extension LabelGiftManager {
    ...
    func skinNeedRed(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        LabelGiftManager.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
