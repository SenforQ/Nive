...
fileprivate let main_keyName:[Character] = ["\u{8bbe}","置","普","通","状","\u{6001}","的","文","字","\u{989c}","色","\u{65f6}"," ","请","使","用"]
fileprivate let show_seeFormat:String = "RGagree"
fileprivate let userPastMsg:String = "bear间的颜色值"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let constReRunningData:[Character] = ["设","置","选","中","状","态","的","文","\u{5b57}","颜","色","时"]
fileprivate let show_standardTitle:String = " pull使用"
fileprivate let constInsideMessage:[Character] = ["间","的","颜","色","值"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let k_backKey:[UInt8] = [0xec,0xeb,0xec,0xf1,0xad,0xe6,0xea,0xe1,0xe0,0xf7,0xbf,0xac,0xa5,0xed,0xe4,0xf6,0xa5,0xeb,0xea,0xf1,0xa5,0xe7,0xe0,0xe0,0xeb,0xa5,0xec,0xe8,0xf5,0xe9,0xe0,0xe8,0xe0,0xeb,0xf1,0xe0,0xe1]

private func linkChart(crease num: UInt8) -> UInt8 {
    return num ^ 133
}

/*: "设置的下标不合法!!" :*/
fileprivate let show_heCapPath:String = "设置的\u{4e0b}标"
fileprivate let constEffMeasurePath:[Character] = ["不","\u{5408}","法","!","!"]

/*: "Bags" :*/
fileprivate let user_argumentMessage:String = "selected sample restrictionBags"
...
open class ConserveThen: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: PrepareSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.decisionFor(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(main_keyName) + show_seeFormat.replacingOccurrences(of: "agree", with: "B") + userPastMsg.replacingOccurrences(of: "bear", with: "空")))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.decisionFor(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(constReRunningData) + show_standardTitle.replacingOccurrences(of: "pull", with: "请") + "RGB空" + String(constInsideMessage)))
        }
    ...
    fileprivate func decisionFor(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: PrepareSegmentStyle, titles: [String]) {
    ...
    @objc func alongDenounceMoveRubric(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? UponElementView else { return }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_backKey.map{linkChart(crease: $0)}, encoding: .utf8)!)
    }
}
...
public extension ConserveThen {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func validBy(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (show_heCapPath + String(constEffMeasurePath)))
    ...
    func paraGlobal(_ titles: [String]) {
        // 移除所有的scrollView子视图
...
//: extension ScrollSegmentView {
extension ConserveThen {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func guidanceNames() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = UponElementView(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.alongDenounceMoveRubric(_:)))
            ...
            if title == (String(user_argumentMessage.suffix(4))).localized {
    ...
    fileprivate func readingNet() {
        ...
        if let firstLabel = labelsArray[0] as? UponElementView {
    ...
    fileprivate func aluminiseCover() {
    ...
    func allow(_: Bool) {
        ...
        if currentIndex == titles.firstIndex(of: (String(user_argumentMessage.suffix(4))).localized) {
            //: redBagsLab.isHidden = true
            redBagsLab.isHidden = true
            //: TalkingChatGiftManager.share.showBagsRed = false
            LabelGiftManager.share.showBagsRed = false
        }

        //: guard currentIndex != oldIndex else { return }
        guard currentIndex != oldIndex else { return }
        //: let oldLabel = labelsArray[oldIndex] as! CustomLabel
        let oldLabel = labelsArray[oldIndex] as! UponElementView
        //: let currentLabel = labelsArray[currentIndex] as! CustomLabel
        let currentLabel = labelsArray[currentIndex] as! UponElementView
        // 让选中标签居中显示
    ...
    public func enjoy(_ currentIndex: Int) {
...
open class UponElementView: UILabel {
    /// 用来记录当前label的缩放比例
