...
/*: "&sr=6" :*/
fileprivate let kAcceptMsg:[Character] = ["&","s","r","=","6"]
...
enum StillMethodType: String, CaseIterable {
...
enum ExtraScalarLiteral: String {
...
class AfterManager: NSObject {
    //: class func getFullUrl(urlType: WebUrlType)-> String {
    class func stagePrice(urlType: ExtraScalarLiteral) -> String {
        //: var url = AppManager.share.appConfigMode.urlH5Domain
        var url = AppManager.share.appConfigMode.urlH5Domain
        //: switch urlType {
        switch urlType {
        //: case .RechargeFullPage:
        case .RechargeFullPage:
            //: url.append(AppManager.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : AppManager.share.appUserConfigMode.fullRechargePage)
            url.append(AppManager.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : AppManager.share.appUserConfigMode.fullRechargePage)

        //: break
        //: case .RechargeHalfPage:
        case .RechargeHalfPage:
            //: url.append(AppManager.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : AppManager.share.appUserConfigMode.halfRechargePage)
            url.append(AppManager.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : AppManager.share.appUserConfigMode.halfRechargePage)

        //: case .RechargeToVideoInitivHalfPage:
        case .RechargeToVideoInitivHalfPage:
            //: let newstr = AppManager.share.appUserConfigMode.halfRechargePage.appending("&sr=6")
            let newstr = AppManager.share.appUserConfigMode.halfRechargePage.appending((String(kAcceptMsg)))
            //: url.append(AppManager.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
            url.append(AppManager.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
        //: default:
        default:
