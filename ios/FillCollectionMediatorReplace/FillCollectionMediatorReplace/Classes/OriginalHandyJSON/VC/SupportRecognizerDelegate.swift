...
fileprivate let dataReduceClearStr:[UInt8] = [0x74,0x73,0x74,0x69,0x35,0x7e,0x72,0x79,0x78,0x6f,0x27,0x34,0x3d,0x75,0x7c,0x6e,0x3d,0x73,0x72,0x69,0x3d,0x7f,0x78,0x78,0x73,0x3d,0x74,0x70,0x6d,0x71,0x78,0x70,0x78,0x73,0x69,0x78,0x79]

private func inmateRemote(tar num: UInt8) -> UInt8 {
    return num ^ 29
}

/*: "Language" :*/
fileprivate let show_taStr:String = "Languagemessage paper tail flow shadow"

/*: "mf/user/getLang" :*/
fileprivate let showReplyTitle:String = "mf/usmoment tender function lab"
fileprivate let kCivilMessage:[Character] = ["e","r","/","g","e","t","L","a","n","g"]

/*: "First language" :*/
fileprivate let noti_challengeContent:String = "Firstlegitimate bone trace"
fileprivate let mainPathBottleVolumeData:String = "ainteraction"

/*: "7166F9" :*/
fileprivate let app_handValue:String = "7166F9zz remind do margin beam"

/*: "EAE8FE" :*/
fileprivate let constShowMessage:[Character] = ["E","A","E","8","F","E"]

/*: "icon_edit_Language_add" :*/
fileprivate let constHolderGameName:String = "list quote brighticon_ed"
fileprivate let show_increaseName:String = "it_Landirect age extent"
fileprivate let appMapStr:String = "visitor full tempe_add"

/*: "Second language" :*/
fileprivate let user_pokeBeatMsg:[Character] = ["S","e","c","o","n","d"," ","l","a","n","g"]
fileprivate let dataAvailableMessage:String = "browge"
...
// __CLOSE_PRINT__
//
//  SupportRecognizerDelegate.swift
//  Pods
//
...
class SupportRecognizerDelegate: MirrorRealmReactiveCompatible {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataReduceClearStr.map{inmateRemote(tar: $0)}, encoding: .utf8)!)
    }
    ...
    override func viewDidLoad() {
        ...
        self.title = (String(show_taStr.prefix(8))).localized
        //: self.reqTagData()
        self.social()
        //: seleteSecondLang = AppManager.share.loginUserMode.customSecondLang!
        seleteSecondLang = AppManager.share.loginUserMode.customSecondLang!
        //: seleteFirstLang = AppManager.share.loginUserMode.customFirstLang!
        seleteFirstLang = AppManager.share.loginUserMode.customFirstLang!

        //: self.setupSubviews()
        self.crop()
        //: self.setupSubViewsConstraint()
        self.afterClubConstraint()
...
extension SupportRecognizerDelegate {
    //: func reqTagData() {
    func social() {
        //: let reqModel = TalkingRequestModel.init()
        let reqModel = ExplosiveTraceDetectionRequestModel()
        //: reqModel.requestPath = "mf/user/getLang"
        reqModel.requestPath = (String(showReplyTitle.prefix(5)) + String(kCivilMessage))
        //: reqModel.requestType = .POST
        reqModel.requestType = .POST
        //: ProgressHUD.show()
        UniversalProgressHUD.addUp()
        //: GJ.startRequest(model: reqModel) { succeed, result, errorModel in
        user_formalMsg.endurance(model: reqModel) { succeed, result, _ in
            ...
            if succeed {
                ...
                for dict in data {
                    ...
                    guard let model = FactoryMeasurable.deserialize(from: dict as? Dictionary) else { return }
    ...
    @objc func skin() {
        ...
        let view = ListReactiveCompatible(frame: CGRect(x: 0, y: 0, width: show_explainId, height: userSystemKey))
        //: view.show()
        view.speaker()
    ...
    @objc func atMilk2() {
        ...
        view.addSuccessTagBlock = { [weak self] in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: self.seleteSecondLang = AppManager.share.loginUserMode.customSecondLang!
            self.seleteSecondLang = AppManager.share.loginUserMode.customSecondLang!
            //: self.setupSubviews2()
            self.succeed()
        }
...
extension SupportRecognizerDelegate {
    ...
    private func crop() {
        ...
        lb.text = (String(noti_challengeContent.prefix(5)) + " langu" + mainPathBottleVolumeData.replacingOccurrences(of: "interaction", with: "ge")).localized
        //: view.addSubview(lb)
        view.addSubview(lb)
        ...
        firstlb.textColor = UIColor(hex: (String(app_handValue.prefix(6))))
        //: firstlb.font = UIFont.pingfangRugularFont(fontSize: 14)
        firstlb.font = UIFont.balanceSize(fontSize: 14)
        ...
        firstlb.layer.borderColor = UIColor(hexString: (String(app_handValue.prefix(6))))?.cgColor
        //: firstlb.backgroundColor = UIColor(hexString: "EAE8FE")
        firstlb.backgroundColor = UIColor(hexString: (String(constShowMessage)))
        ...
        btn.setImage(UIImage.scheme(name: (String(constHolderGameName.suffix(7)) + String(show_increaseName.prefix(6)) + "guag" + String(appMapStr.suffix(5)))), for: .normal)
        //: btn.addTarget(self, action: #selector(addBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(skin), for: .touchUpInside)
        //: view.addSubview(btn)
        view.addSubview(btn)
    ...
    private func succeed() {
        ...
        lb.text = (String(user_pokeBeatMsg) + dataAvailableMessage.replacingOccurrences(of: "brow", with: "ua")).localized
        ...
        for (i, str) in seleteSecondLang.enumerated() {
            ...
            secondlb.textColor = UIColor(hex: (String(app_handValue.prefix(6))))
            //: secondlb.font = UIFont.pingfangRugularFont(fontSize: 14)
            secondlb.font = UIFont.balanceSize(fontSize: 14)
            ...
            secondlb.layer.borderColor = UIColor(hexString: (String(app_handValue.prefix(6))))?.cgColor
            //: secondlb.backgroundColor = UIColor(hexString: "EAE8FE")
            secondlb.backgroundColor = UIColor(hexString: (String(constShowMessage)))
            //: secondlb.text = str
            secondlb.text = str
        ...
        secondBtn.setImage(UIImage.scheme(name: (String(constHolderGameName.suffix(7)) + String(show_increaseName.prefix(6)) + "guag" + String(appMapStr.suffix(5)))), for: .normal)
        //: secondBtn.addTarget(self, action: #selector(addBtnClick2), for: .touchUpInside)
        secondBtn.addTarget(self, action: #selector(atMilk2), for: .touchUpInside)
...
//            secondBtn.frame = CGRect(x: self.secondLastLb.frame.origin.x + self.secondLastLb.frame.size.width + 12, y: self.secondLastLb.frame.origin.y, width: 72, height: 30)
    ...
    private func afterClubConstraint() {}
}
