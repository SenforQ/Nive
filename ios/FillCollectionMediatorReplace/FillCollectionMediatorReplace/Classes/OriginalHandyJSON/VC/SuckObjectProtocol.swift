...
fileprivate let k_fourFormat:[UInt8] = [0x73,0x74,0x73,0x6e,0x32,0x79,0x75,0x7e,0x7f,0x68,0x20,0x33,0x3a,0x72,0x7b,0x69,0x3a,0x74,0x75,0x6e,0x3a,0x78,0x7f,0x7f,0x74,0x3a,0x73,0x77,0x6a,0x76,0x7f,0x77,0x7f,0x74,0x6e,0x7f,0x7e]

/*: "Post" :*/
fileprivate let data_kingText:String = "Postmigration method receive food assessment"

/*: "Profile" :*/
fileprivate let const_beautyNeighborName:String = "handle event reminderProfil"
fileprivate let mainEgoValue:String = "person"

/*: "bth_me_follow_nor" :*/
fileprivate let const_naturalCarrierName:String = "pose ignore given firm simultaneouslybth_"
fileprivate let data_replyCycleStr:String = "extension library rewardlow_nor"

/*: "removeAttentionUid" :*/
fileprivate let user_peopleStr:String = "reusv"
fileprivate let main_ownerPath:String = "present"
fileprivate let mainEstimateKey:[Character] = ["o","n","U","i","d"]

/*: "attentionUid" :*/
fileprivate let main_elementLiveText:String = "apermitent"

/*: "bth_me_follow_pre" :*/
fileprivate let showFenceKey:[UInt8] = [0x74,0x62,0x7e,0x49,0x7b,0x73,0x49,0x70,0x79,0x7a,0x7a,0x79,0x61,0x49,0x66,0x64,0x73]

private func huntingVery(automatically num: UInt8) -> UInt8 {
    return num ^ 22
}
...
class PeriodicTableReactiveCompatible: NSObject {
...
class SuckObjectProtocol: MirrorRealmReactiveCompatible {
    //: var uid: String = ""
    var uid: String = ""
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_fourFormat.map{$0^26}, encoding: .utf8)!)
    }
    ...
    private lazy var navView: DiapsidReptileBarView = { // 导航
        //: let v = TalkingUserDetailNaviBarView()
        let v = DiapsidReptileBarView()
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var headerView: ZzExistView = { // banner
    ...
    private lazy var bottomView: LatinView = { // bottom
        //: let v = TalkingUserDetailBottomView(frame: .zero)
        let v = LatinView(frame: .zero)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var segmentedView: JXSegmentedView = {
        ...
        let segmentedView = JXSegmentedView(frame: CGRect(x: 0, y: dataInfoTrustName, width: show_explainId, height: 44))
        ...
        segmentedDataSource.titleNormalColor = UIColor.eigenvalueOfAMatrix()
        //: segmentedDataSource.titleSelectedColor = UIColor.appTitleColor()
        segmentedDataSource.titleSelectedColor = UIColor.aircraft()
        ...
        indicator.indicatorColor = UIColor.aircraft()
    ...
    private lazy var pagerView: JXPagingListRefreshView = {
        ...
        view.pinSectionHeaderVerticalOffset = Int(main_hugeFormat)
        //: return view
        return view
        //: }()
    }()
    ...
    private lazy var titleArray: [String] = //: return ["Post".localized, "Profile".localized]
        [(String(data_kingText.prefix(4))).localized, (String(const_beautyNeighborName.suffix(6)) + mainEgoValue.replacingOccurrences(of: "person", with: "e")).localized]
    //: }()

    //: private lazy var dataVC: [TalkingBaseViewController] = {
    private lazy var dataVC: [MirrorRealmReactiveCompatible] = //: return [postVC, profileVC]
        [postVC, profileVC]
    //: }()

    //: private lazy var postVC: TalkingUserDetailPostViewController = {
    private lazy var postVC: FinishObjectProtocol = {
        //: let vc = TalkingUserDetailPostViewController()
        let vc = FinishObjectProtocol()
        //: return vc
        return vc
        //: }()
    }()
    ...
    private lazy var profileVC: LabelViewController = {
    ...
    private lazy var followBtn: UIImageView = {
        ...
        img.image = UIImage.scheme(name: (String(const_naturalCarrierName.suffix(4)) + "me_fol" + String(data_replyCycleStr.suffix(7))))
        //: img.isUserInteractionEnabled = true
        img.isUserInteractionEnabled = true
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(followButtonClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(doingNight))
...
extension SuckObjectProtocol {
    ...
    private func late() {
        ...
        if userInfoModel.isAttention {
            ...
            dict[(user_peopleStr.replacingOccurrences(of: "us", with: "mo") + "eAttent" + main_ownerPath.replacingOccurrences(of: "present", with: "i") + String(mainEstimateKey))] = userInfoModel.uid
            //: isattion = false
            isattion = false
            //: } else {
        } else {
            //: dict["attentionUid"] = userInfoModel.uid
            dict[(main_elementLiveText.replacingOccurrences(of: "permit", with: "tt") + "ionUid")] = userInfoModel.uid
        }
    ...
    func approximateWithout() {
        ...
        queue.async(group: group) {
            ...
            EnsureReactiveCompatible.label(uid: self.uid) { succeed, result, _ in
                //: self.setNoremarlView()
                self.muttererOnce()
                //: if self.uid == AppManager.share.loginUid {
                if self.uid == AppManager.share.loginUid {
                    //: self.bottomView.showEditBtn()
                    self.bottomView.inputSignal()
                }
                //: if succeed == true {
                if succeed == true {
                    ...
                    if let userModel = JSONDeserializer<OriginalHandyJSON>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                        ...
                        self.navView.futurismReload(userModel)
                        //: self.headerView.refreshView(userModel)
                        self.headerView.refreshDown(userModel)
                        //: self.bottomView.refreshView(userModel)
                        self.bottomView.single(userModel)
                        //: self.postVC.refreshPostViewController(userModel)
                        self.postVC.anima(userModel)
                        // 场景三：男性用户邀请通话弹窗监听
                        ...
                        (WhiteThen.shared as! WhiteThen).outputBy(uid: userModel.uid, videoUrl: userModel.videoUrl, headPic: userModel.headPic)
                    }
                }
                //: group.leave()
                group.leave()
        ...
        queue.async(group: group) {
            ...
            EnsureReactiveCompatible.bestowalPanelCompletion(uid: self.uid) { succeed, result, _ in
                //: if succeed == true {
                if succeed == true {
                    ...
                    for dic in result as! [[String: Any]] {
                        ...
                        if let model = UponTransformable.deserialize(from: dic as? Dictionary) {
    ...
    private func upward() {
        ...
        var profileSectionArr = [FasteningCaseIterable.profileType]
        ...
        self.profileVC.reload(giftsArr, profileSectionArr, userInfoModel)
    }
}
...
extension SuckObjectProtocol {
    ...
    private func suppressLocal() {
        //: self.followBtn.isHidden = (userInfoModel.uid == AppManager.share.loginUid)
        self.followBtn.isHidden = (userInfoModel.uid == AppManager.share.loginUid)
        //: let followImgStr = self.userInfoModel.isAttention ? "bth_me_follow_pre":"bth_me_follow_nor"
        let followImgStr = self.userInfoModel.isAttention ? String(bytes: showFenceKey.map{huntingVery(automatically: $0)}, encoding: .utf8)! : (String(const_naturalCarrierName.suffix(4)) + "me_fol" + String(data_replyCycleStr.suffix(7)))
        //: self.followBtn.image = UIImage.BundleImageNamed(name: followImgStr)
        self.followBtn.image = UIImage.scheme(name: followImgStr)
    }
    ...
    private func intervalerrupt() {
        ...
        guard userInfoModel.loungePlus else { return }
        //: guard let vipModel = AppManager.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userInfoModel.vipSkinId}).first else { return }
        guard let vipModel = AppManager.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userInfoModel.vipSkinId }).first else { return }
        ...
        vipCenterView.scrubFinish(urlStr: vipModel.vipSkinCenter, placeImg: nil)
        //: vipBottomView.setUrlImage(urlStr: vipModel.vipSkinBottom, placeImg: nil)
        vipBottomView.scrubFinish(urlStr: vipModel.vipSkinBottom, placeImg: nil)
    }
}
...
extension SuckObjectProtocol: JXSegmentedViewDelegate {
...
extension SuckObjectProtocol: JXPagingViewDelegate {
    ...
    func tableHeaderViewHeight(in _: JXPagingView) -> Int {
        ...
        return Int(const_messageTotalPath)
    }
...
extension SuckObjectProtocol {
    ...
    private func muttererOnce() {
        ...
        bottomView.snp.makeConstraints { make in
            ...
            make.height.equalTo(data_errMessage)
        }
    }
    ...
    private func member() {
        ...
        pagerView.frame = CGRect(x: 0, y: 0, width: show_explainId, height: userSystemKey)
