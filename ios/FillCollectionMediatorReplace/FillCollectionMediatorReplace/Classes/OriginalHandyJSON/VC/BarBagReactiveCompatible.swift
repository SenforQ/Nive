...
fileprivate let constBeforeStr:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Price Settings" :*/
fileprivate let kNoteTitle:String = "activity advantage under ingredient observerPrice"

/*: "5.00" :*/
fileprivate let noti_combinedData:[Character] = ["5",".","0","0"]

/*: "Chat price settings" :*/
fileprivate let mainRelieveId:[Character] = ["C","h","a","t"," ","p","r","i","c","e"," ","s","e","t","t","i","n","g","s"]

/*: "Video call price settings" :*/
fileprivate let app_deliveryData:String = "Video tag con person essay resent"
fileprivate let k_communicateKey:String = " pricpeople channel pose trigger"
fileprivate let constKeyTitle:[Character] = ["e"," ","s","e","t","t","i","n","g","s"]

/*: "Voice call price settings" :*/
fileprivate let dataMiddlePath:String = "written eachVoice"
fileprivate let noti_aircraftName:String = " priceburn cheek forehead mic"
...
// __CLOSE_PRINT__
//
//  BarBagReactiveCompatible.swift
//  AbroadTalking
//
...
class BarBagReactiveCompatible: MirrorRealmReactiveCompatible {
    // MARK: - 属性声明 & 生命周期方法

    //: lazy var messageArray: [ChatPriceModel] = []
    lazy var messageArray: [LapPriceModel] = []
    //: lazy var videoArray: [ChatPriceModel] = []
    lazy var videoArray: [LapPriceModel] = []
    //: lazy var voiceArray: [ChatPriceModel] = []
    lazy var voiceArray: [LapPriceModel] = []
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constBeforeStr.reversed(), encoding: .utf8)!)
    }
    ...
    override func viewDidLoad() {
        ...
        self.title = (String(kNoteTitle.suffix(5)) + " Settings").localized
    ...
    lazy var mainTableView = UITableView(frame: .zero, style: .plain).then {
        ...
        $0.register(PhotoReactiveCompatible.self, forCellReuseIdentifier: PhotoReactiveCompatible.className())
        //: view.addSubview($0)
        view.addSubview($0)
...
extension BarBagReactiveCompatible {
    //: private func setupData() {
    private func satisfyOf() {
        ...
        for tempModel in AppManager.share.appUserConfigMode.chatPriceSettings {
            ...
            if tempModel.price == Int(Double(AppManager.share.loginUserMode.messagePrice ?? (String(noti_combinedData)))!) {
...
extension BarBagReactiveCompatible: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: PhotoReactiveCompatible.className(), for: indexPath) as! PhotoReactiveCompatible
        //: var price = 0
        var price = 0
        //: var level = 0
        var level = 0
    ...
    func tableView(_: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        let headerView = UIView(frame: CGRect(x: 0.0, y: 0.0, width: show_explainId, height: 45))
        //: headerView.backgroundColor = UIColor.appBgColor()
        headerView.backgroundColor = UIColor.referenceAndNonsolidColour()

        //: let titleLab = UILabel.init(frame: CGRect(x: 15, y: 16, width: ScreenWidth-30, height: 20))
        let titleLab = UILabel(frame: CGRect(x: 15, y: 16, width: show_explainId - 30, height: 20))
        //: switch section {
        switch section {
        //: case 0:
        case 0:
            //: titleLab.text = "Chat price settings".localized
            titleLab.text = (String(mainRelieveId)).localized
        //: break
        //: case 1:
        case 1:
            //: titleLab.text = "Video call price settings".localized
            titleLab.text = (String(app_deliveryData.prefix(6)) + "call" + String(k_communicateKey.prefix(5)) + String(constKeyTitle)).localized
        //: break
        //: case 2:
        case 2:
            //: titleLab.text = "Voice call price settings".localized
            titleLab.text = (String(dataMiddlePath.suffix(5)) + " call" + String(noti_aircraftName.prefix(6)) + " settings").localized
        //: break
        //: default:
        default:
            //: break
        ...
        titleLab.textColor = .eigenvalueOfAMatrix()
        //: titleLab.font = .pingfangFont(type: .Regular, fontSize: 16)
        titleLab.font = .shareOff(type: .Regular, fontSize: 16)
    ...
    func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let vc = SizeDataSource(frame: self.view.frame)
        //: switch indexPath.section {
        switch indexPath.section {
        //: case 0:
        case 0:
