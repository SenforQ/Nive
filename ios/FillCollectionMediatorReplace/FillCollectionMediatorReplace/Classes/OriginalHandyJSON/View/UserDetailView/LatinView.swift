...
fileprivate let showNextMessage:[UInt8] = [0xe3,0xe8,0xe3,0xee,0xa2,0xdd,0xe9,0xde,0xdf,0xec,0xb4,0xa3,0x9a,0xe2,0xdb,0xed,0x9a,0xe8,0xe9,0xee,0x9a,0xdc,0xdf,0xdf,0xe8,0x9a,0xe3,0xe7,0xea,0xe6,0xdf,0xe7,0xdf,0xe8,0xee,0xdf,0xde]

fileprivate func brakeRemote(soap num: UInt8) -> UInt8 {
...
/*: "bth_me_chat_nor" :*/
fileprivate let show_normallyFormat:String = "bth_mperson album opposition"
fileprivate let const_stageDryData:String = "at_norgiven value"

/*: "#6ADEFF" :*/
fileprivate let show_extraStr:String = "wide next medium table#6ADEFF"

/*: "#5D6FFF" :*/
fileprivate let user_exclusiveThickId:[Character] = ["#","5"]
fileprivate let userTruthStr:[Character] = ["D","6","F","F","F"]

/*: "icon_lounge" :*/
fileprivate let notiResourceMsg:String = "icon_loattention milk trade drag exception"
fileprivate let data_aroundBreezeUrl:String = "uncapitale"

/*: "icon_video_call" :*/
fileprivate let k_snapMessage:String = "own stack media signalicon_v"
fileprivate let userPrisonKey:String = "say fine security terms_call"

/*: "iv_me_vip" :*/
fileprivate let noti_weekSwitchceId:String = "iv_mplace deadline mm woman"

/*: "Edit" :*/
fileprivate let const_yaData:[Character] = ["E","d","i","t"]

/*: "icon_Chat_freetimes" :*/
fileprivate let main_remainingStr:[Character] = ["i","c","o","n","_","C","h","a","t","_","f"]
fileprivate let show_rowMsg:String = "reetfindes"

/*: "#AB57F6" :*/
fileprivate let main_surgeryAllName:String = "chase prison description formal#AB57F6"

/*: "#FC57B7" :*/
fileprivate let user_loopIdentityPath:[Character] = ["#","F","C","5","7","B"]
fileprivate let mainKitMessage:[Character] = ["7"]

/*: "#FE9074" :*/
fileprivate let show_toleranceUrl:[Character] = ["#","F","E","9","0","7","4"]

/*: "Video Call" :*/
fileprivate let data_investId:[Character] = ["V","i","d","e","o"]
fileprivate let app_revenueStr:String = "enough equal attach food snow Call"
...
let data_errMessage = (15 + actualWidth(w: 36) + 15 + user_videoMsg)

//: class TalkingUserDetailBottomView: UIView {
class LatinView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showNextMessage.map{brakeRemote(soap: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var shadowLayer: CAGradientLayer = {
        ...
        layer.frame = CGRect(x: 0, y: 0, width: show_explainId, height: 15 + actualWidth(w: 38) + 15)
    ...
    private lazy var chatBtn: UIImageView = {
        ...
        img.image = UIImage.scheme(name: (String(show_normallyFormat.prefix(5)) + "e_ch" + String(const_stageDryData.prefix(6))))
        //: img.isUserInteractionEnabled = true
        img.isUserInteractionEnabled = true
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(chatButtonClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(unsoiled))
    ...
    private lazy var videoBtn: UIButton = {
        ...
        let colors = [UIColor(hex: (String(show_extraStr.suffix(7))))!.cgColor, UIColor(hex: (String(user_exclusiveThickId) + String(userTruthStr)))!.cgColor]
        ...
        btn.setBackgroundImage(UIImage.textStud(colors: colors, size: CGSize(width: actualWidth(w: 250), height: actualWidth(w: 36))), for: .normal)
        //: btn.layer.cornerRadius = actualWidth(w: 18)
        btn.layer.cornerRadius = actualWidth(w: 18)
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(videoButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(history), for: .touchUpInside)
        //: btn.isHidden = true
        btn.isHidden = true
        //: return btn
        return btn
    ...
    private lazy var vipIcon: UIImageView = {
        ...
        img.image = UIImage.scheme(name: (String(notiResourceMsg.prefix(7)) + data_aroundBreezeUrl.replacingOccurrences(of: "capital", with: "g")))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var callIcon: UIImageView = {
        ...
        img.image = UIImage.scheme(name: (String(k_snapMessage.suffix(6)) + "ideo" + String(userPrisonKey.suffix(5))))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var maleVipPriceBtn: UIImageView = {
        ...
        img.image = UIImage.scheme(name: (String(noti_weekSwitchceId.prefix(4)) + "e_vip"))
        //: img.isUserInteractionEnabled = true
        img.isUserInteractionEnabled = true
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(maleVipPriceButtonClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(canvass))
        //: img.addGestureRecognizer(tap)
        img.addGestureRecognizer(tap)
        //: img.isHidden = true
        img.isHidden = true
    ...
    lazy var editBtn: UIButton = {
        ...
        btn.setTitle((String(const_yaData)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(bracketAndSnap), for: .touchUpInside)
        //: btn.isHidden = true
        btn.isHidden = true
    ...
    lazy var freeImg: UIImageView = {
        ...
        img.image = UIImage.scheme(name: (String(main_remainingStr) + show_rowMsg.replacingOccurrences(of: "find", with: "im")))
        //: return img
        return img
        //: }()
    }()
...
extension LatinView {
    ...
    @objc func history() {
        ...
        noti_nearbyStr.tamperConnect(eventID: kMovieStr, toUid: userInfoModel.uid)
        //: if AppManager.share.loginUserMode.sex == Gender.male.rawValue {
        if AppManager.share.loginUserMode.sex == SharedMirrorKingCustomReflectable.male.rawValue { // 男性用户
            ...
            videoCallManager.eachReply(videoPrice: userInfoModel.videoBean, voicePrice: userInfoModel.voiceBean, vipPrompt: userInfoModel.prompt)
        }
    }

    /// chat
    //: @objc private func chatButtonClick() {
    @objc private func unsoiled() {
        //: ElementEarnReactiveCompatible.share.func__pushToPriveteChatVC(chatID: userInfoModel.uid)
        ElementEarnReactiveCompatible.share.totalAgain(chatID: userInfoModel.uid)
        //: uploadRecord.uploadRecordEvent(eventID: ClickProfileChatButton, toUid: userInfoModel.uid)
        noti_nearbyStr.tamperConnect(eventID: kLastPlainValue, toUid: userInfoModel.uid)
    }

    /// edit
    //: @objc private func editButtonClick() {
    @objc private func bracketAndSnap() {
...
extension LatinView {
    ...
    func single(_ userModel: OriginalHandyJSON) {
        ...
        if userModel.uid == AppManager.share.loginUid { // 本人资料页
            ...
            return
                    ...
                    //: } else if AppManager.share.loginUid != DelineateThen.getCustomerServiceID(),
        } else if AppManager.share.loginUid != DelineateThen.enterId(),
        ...
        if AppManager.share.loginUserMode.sex == SharedMirrorKingCustomReflectable.male.rawValue {
            ...
            if AppManager.share.loginUserMode.loungePlus == false { // 男性非会员
                ...
                if AppManager.share.appStatus == EarnLetterCustomReflectable.normal.rawValue {
                    ...
                    maleVipPriceLab.text = String.iterateAt(videoPrice: userInfoModel.videoVIPPrice)
                }
                //: vipIcon.isHidden = true
                vipIcon.isHidden = true
                //: callStrLab.text = String.getMaleVIPVideoPriceString(videoPrice: userInfoModel.videoPrice)
                callStrLab.text = String.iterateAt(videoPrice: userInfoModel.videoPrice)

                //: } else {
            } else {
                //: let colors = [UIColor(hex: "#AB57F6")!.cgColor, UIColor(hex: "#FC57B7")!.cgColor, UIColor(hex: "#FE9074")!.cgColor]
                let colors = [UIColor(hex: (String(main_surgeryAllName.suffix(7))))!.cgColor, UIColor(hex: (String(user_loopIdentityPath) + String(mainKitMessage)))!.cgColor, UIColor(hex: (String(show_toleranceUrl)))!.cgColor]
                ...
                videoBtn.setBackgroundImage(UIImage.textStud(colors: colors, size: CGSize(width: actualWidth(w: 250), height: actualWidth(w: 36))), for: .normal)
                ...
                callStrLab.text = String.iterateAt(videoPrice: userInfoModel.videoVIPPrice)
            }
            ...
            callStrLab.text = (String(data_investId) + String(app_revenueStr.suffix(5))).localized
        }
    }
    ...
    private func outConfinement() {
        ...
        bottomView.snp.makeConstraints { make in
            ...
            make.height.equalTo(user_videoMsg)
        }
    }
}
