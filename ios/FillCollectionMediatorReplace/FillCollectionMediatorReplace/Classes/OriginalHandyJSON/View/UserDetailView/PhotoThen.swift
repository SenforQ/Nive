...
fileprivate let main_countValue:[UInt8] = [0x6,0xb,0x6,0x11,0xc5,0x0,0xc,0x1,0x2,0xf,0xd7,0xc6,0xbd,0x5,0xfe,0x10,0xbd,0xb,0xc,0x11,0xbd,0xff,0x2,0x2,0xb,0xbd,0x6,0xa,0xd,0x9,0x2,0xa,0x2,0xb,0x11,0x2,0x1]

fileprivate func anyoneBook(up num: UInt8) -> UInt8 {
...
/*: "More" :*/
fileprivate let app_wordAblePath:String = "Moreblind window pop lash yesterday"

/*: "icon_details_arrow" :*/
fileprivate let notiMediumLensUrl:String = "favorite trust request rolling viewicon_d"
fileprivate let appPracticalMsg:String = "ls_arrorelation view success"
fileprivate let notiFigureOperationTitle:String = "movie"

/*: "Post" :*/
fileprivate let userMeasureFullName:[Character] = ["P","o","s","t"]

/*: "btn_dynamic_stop_nor" :*/
fileprivate let notiPendingData:String = "thumb processor opposition together decidebtn_dy"
fileprivate let constTargetMessage:String = "stop_noras brush"
...
class PhotoThen: UITableViewCell {
    //: var userId: String = ""
    var userId: String = ""
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_countValue.map{anyoneBook(up: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var maskMoreView: UIView = {
        ...
        view.backgroundColor = .itInstall(0, 0, 0, 0.4)
    ...
    lazy var moreButton: ExtendedView = .init().then {
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 17)
        $0.titleLabel?.font = .shareOff(type: .Regular, fontSize: 17)
        //: $0.setTitle("More".localized, for: .normal)
        $0.setTitle((String(app_wordAblePath.prefix(4))).localized, for: .normal)
        //: $0.setImage(UIImage(named: "icon_details_arrow"), for: .normal)
        $0.setImage(UIImage(named: (String(notiMediumLensUrl.suffix(6)) + "etai" + String(appPracticalMsg.prefix(7)) + notiFigureOperationTitle.replacingOccurrences(of: "movie", with: "w"))), for: .normal)
    ...
    lazy var pushBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(dailyShow), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension PhotoThen {
    ...
    func block(userModel: OriginalHandyJSON) {
        ...
        strA = [(String(userMeasureFullName)).localized, str]
        //: colorA = [UIColor.appTitleColor(), UIColor.appValueColor()]
        colorA = [UIColor.aircraft(), UIColor.capVidNonsolidColorConfirm()]
        //: fontA = [UIFont.pingfangMediumFont(fontSize: 18), UIFont.pingfangMediumFont(fontSize: 16)]
        fontA = [UIFont.pingfangBusy(fontSize: 18), UIFont.pingfangBusy(fontSize: 16)]
        ...
        for index in 0 ..< itemCount {
            ...
            if index == 0 {
                ...
                let model: CreaseModelType = (userModel.momentPic?.list)![index]
                ...
                firstImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.endAdd())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: firstImgView)
                    from(btn: firstImgView)
                }
                ...
                secondImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.endAdd())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: secondImgView)
                    from(btn: secondImgView)
                }
                //: } else {
            } else {
                //: let model: TalkingUserInfoMomentItemModel = (userModel.momentPic?.list)![index]
                ...
                thirdImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.endAdd())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: thirdImgView)
                    from(btn: thirdImgView)
                }
            }
        }
    }
    ...
    @objc func dailyShow() {
        ...
        let vc = StatusViewController(uid: userId)
    ...
    func from(btn: UIImageView) {
        ...
        let imag = UIImageView(image: UIImage(named: (String(notiPendingData.suffix(6)) + "namic_" + String(constTargetMessage.prefix(8)))))
...
extension PhotoThen {
    ...
    private func mpConstraint() {
        ...
        let height = (show_explainId - actualWidth(w: 15 * 4)) / 3.0
