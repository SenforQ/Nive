...
fileprivate let dataDomainCellMsg:[UInt8] = [0xab,0xac,0xab,0xb6,0xea,0xa1,0xad,0xa6,0xa7,0xb0,0xf8,0xeb,0xe2,0xaa,0xa3,0xb1,0xe2,0xac,0xad,0xb6,0xe2,0xa0,0xa7,0xa7,0xac,0xe2,0xab,0xaf,0xb2,0xae,0xa7,0xaf,0xa7,0xac,0xb6,0xa7,0xa6]

private func formCreate(depth num: UInt8) -> UInt8 {
    return num ^ 194
}

/*: "#333333" :*/
fileprivate let show_parentPath:[Character] = ["#"]
fileprivate let constEarlyValue:String = "editoreditoreditoreditoreditoreditor"

/*: "#999999" :*/
fileprivate let userTwoCellSuperiorName:String = "following"
fileprivate let constDelayFormat:String = "999999"

/*: "isCupid" :*/
fileprivate let userLessId:String = "would universal advanced verticalisCupid"

/*: "autoGreetAuth" :*/
fileprivate let user_resourceId:[Character] = ["a","u","t","o","G","r","e","e","t","A","u","t","h"]

/*: "isLeaderboardAnonymous" :*/
fileprivate let userBoldFormat:String = "ta tempisLead"
fileprivate let const_reText:String = "already eventrdAnon"

/*: "isSameGendered" :*/
fileprivate let showGivenAppealMId:String = "both sheer profile smallisSame"
fileprivate let dataGroupUrl:[Character] = ["e","d"]

/*: "type" :*/
fileprivate let kLinkId:String = "tcrushpe"

/*: "value" :*/
fileprivate let constTrustMsg:String = "valbille"
...
class EffView: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(application), name: UIApplication.didBecomeActiveNotification, object: nil)
    }

    //: required init?(coder aDecoder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: dataDomainCellMsg.map{formCreate(depth: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleBLB: UILabel = {
        ...
        lb.textColor = UIColor(hex: (String(show_parentPath) + constEarlyValue.replacingOccurrences(of: "editor", with: "3")))
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var switchView: UISwitch = {
        ...
        swit.addTarget(self, action: #selector(sizeIn), for: .valueChanged)
        //: return swit
        return swit
        //: }()
    }()
    ...
    lazy var detailLB: SharedThen = {
        //: let lb = TextAlignLb.init()
        let lb = SharedThen()
        //: lb.setIsTop(isTop: true)
        lb.topThat(isTop: true)
        //: lb.font = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        lb.font = UIFont.shareOff(type: .Regular, fontSize: 15)
        //: lb.textColor = UIColor.init(hex: "#999999")
        lb.textColor = UIColor(hex: (userTwoCellSuperiorName.replacingOccurrences(of: "following", with: "#") + constDelayFormat.capitalized))
        //: lb.isHidden = true
        lb.isHidden = true
        //: lb.numberOfLines = 0
        lb.numberOfLines = 0
...
extension EffView {
    ...
    func aboardCurrent(titile: String, row: Int, isLast: Bool) {
...
//        if titile == ArgumentCollection.General.rawValue {
//            EditThen.checkPushNotification { isShow in
...
//        }
    ...
    @objc func sizeIn() {
        ...
        if titleBLB.text == InsideExpressionConvertible.Turnon_Notif.rawValue.localized {
        ...
        } else if titleBLB.text == InsideExpressionConvertible.Cupid.rawValue.localized {
            //: setConfig(type: "isCupid", value: self.switchView.isOn)
            quickGroup(type: (String(userLessId.suffix(7))), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Automatic.rawValue.localized {
        } else if titleBLB.text == InsideExpressionConvertible.Automatic.rawValue.localized {
            //: setConfig(type: "autoGreetAuth", value: self.switchView.isOn)
            quickGroup(type: (String(user_resourceId)), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Ranking.rawValue.localized {
        } else if titleBLB.text == InsideExpressionConvertible.Ranking.rawValue.localized {
            //: setConfig(type: "isLeaderboardAnonymous", value: self.switchView.isOn)
            quickGroup(type: (String(userBoldFormat.suffix(6)) + "erboa" + String(const_reText.suffix(6)) + "ymous"), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Same_gendered.rawValue.localized {
        } else if titleBLB.text == InsideExpressionConvertible.Same_gendered.rawValue.localized {
            //: setConfig(type: "isSameGendered", value: self.switchView.isOn)
            quickGroup(type: "isSameGendered", value: self.switchView.isOn)
        }
    }

    //: func setConfig(type: String, value: Bool) {
    func quickGroup(type: String, value: Bool) {
        //: let va = value ? 1 : -1
        let va = value ? 1 : -1
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["type"] = type
        dict[(kLinkId.replacingOccurrences(of: "crush", with: "y"))] = type
        //: dict["value"] = va
        dict[(constTrustMsg.replacingOccurrences(of: "bill", with: "u"))] = va

        //: ProgressHUD.show()
        UniversalProgressHUD.addUp()
        //: TalkingMeRequestTool.req_SettingChange(params: dict) { succeed, result, errorModel in
        GuidanceRequestTool.feed(params: dict) { succeed, _, _ in
            //: ProgressHUD.dismiss()
    ...
    @objc func application(notification _: NSNotification) {
//        if titleBLB.text == ArgumentCollection.General.rawValue {
//            EditThen.checkPushNotification { isShow in
//                DispatchQueue.main.sync {
//                    self.switchView.isOn = isShow
//                }
