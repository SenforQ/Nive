...
fileprivate let userHerContent:[UInt8] = [0xb6,0xb1,0xb6,0xab,0xf7,0xbc,0xb0,0xbb,0xba,0xad,0xe5,0xf6,0xff,0xb7,0xbe,0xac,0xff,0xb1,0xb0,0xab,0xff,0xbd,0xba,0xba,0xb1,0xff,0xb6,0xb2,0xaf,0xb3,0xba,0xb2,0xba,0xb1,0xab,0xba,0xbb]

/*: "Edit Title" :*/
fileprivate let data_iconMsg:String = "Edit Tinobody method popularity affect minute"
fileprivate let show_injuryId:String = "tlbeautiful"

/*: "Finish" :*/
fileprivate let show_castId:[Character] = ["F","i","n","i","s","h"]

/*: "btn_delete" :*/
fileprivate let appCollectionToContent:[Character] = ["b","t","n","_","d","e","l","e","t","e"]
...
class DraftTextView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(needIgnore))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userHerContent.map{$0^223}, encoding: .utf8)!)
    }
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.text = (String(data_iconMsg.prefix(7)) + show_injuryId.replacingOccurrences(of: "beautiful", with: "e")).localized
    ...
    lazy var inputTView: ChangeoverTextView = {
    ...
    private lazy var finishBtn: UIButton = {
        ...
        btn.setTitle((String(show_castId)).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.textStud(colors: UIColor.photoByModify(), size: CGSize(width: 267, height: 50)), for: .normal)
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.scheme(name: (String(appCollectionToContent))), for: .normal)
...
extension DraftTextView {
    //: @objc func dismissView() {
    @objc func needIgnore() {}
...
extension DraftTextView: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > lengthLimit {
            ...
            if textNum > lengthLimit && lengthLimit > 0 {
                ...
                textView.text = kind(index: lengthLimit, text: textContent)
            }
        }
    ...
    private func kind(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
