...
fileprivate let const_gravityTitle:[UInt8] = [0x77,0x70,0x77,0x6a,0x36,0x7d,0x71,0x7a,0x7b,0x6c,0x24,0x37,0x3e,0x76,0x7f,0x6d,0x3e,0x70,0x71,0x6a,0x3e,0x7c,0x7b,0x7b,0x70,0x3e,0x77,0x73,0x6e,0x72,0x7b,0x73,0x7b,0x70,0x6a,0x7b,0x7a]

private func positSeed(bean num: UInt8) -> UInt8 {
    return num ^ 30
}

/*: "#333333" :*/
fileprivate let constReserveContent:String = "#mmm"

/*: "My photos" :*/
fileprivate let userParticularDoingceMessage:String = "My phballoon topic relation report merge"
fileprivate let noti_launchPointContent:String = "otojoin"

/*: "#999999" :*/
fileprivate let appCommunicationMsg:String = "#waywayway"

/*: "Click to change or delete photo." :*/
fileprivate let kAlongsideFormat:[UInt8] = [0xac,0x83,0x86,0x8c,0x84,0xcf,0x9b,0x80,0xcf,0x8c,0x87,0x8e,0x81,0x88,0x8a,0xcf,0x80,0x9d,0xcf,0x8b,0x8a,0x83,0x8a,0x9b,0x8a,0xcf,0x9f,0x87,0x80,0x9b,0x80,0xc1]

/*: "btn_me_edit_photo_add_nor" :*/
fileprivate let mainExtentId:String = "btn_me_actual compare"
fileprivate let data_capitalMessage:[Character] = ["e","d","i","t","_","p","h","o","t","o","_","a","d","d","_","n","o","r"]

/*: "btn_me_edit_photo_add_pre" :*/
fileprivate let app_familySlimKey:[Character] = ["b","t","n","_","m","e","_","e","d","i","t"]
fileprivate let noti_bookId:String = "section check_photo"

/*: "btn_me_edit_photo_delete" :*/
fileprivate let data_followingTwoFormat:String = "btn_me_eeastern urge personal bench skin"
fileprivate let main_maximumMsg:String = "dit_ppub project superior yet"
fileprivate let constReduceData:String = "dimplementlimplementtimplement"
...
public protocol GuidanceExtraThen: NSObject {
    /// 图片add
...
class DistinctThen: UITableViewCell {
    ...
    open weak var delegate: GuidanceExtraThen?
...
//        showMessageView()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_gravityTitle.map{positSeed(bean: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleBLB: UILabel = {
        ...
        lb.textColor = UIColor(hex: (constReserveContent.replacingOccurrences(of: "m", with: "33")))
        //: lb.text = "My photos".localized
        lb.text = (String(userParticularDoingceMessage.prefix(5)) + noti_launchPointContent.replacingOccurrences(of: "join", with: "s")).localized
    ...
    lazy var messageLB: UILabel = {
        ...
        lb.font = UIFont.shareOff(type: .Regular, fontSize: 12)
        //: lb.textColor = UIColor.init(hex: "#999999")
        lb.textColor = UIColor(hex: (appCommunicationMsg.replacingOccurrences(of: "way", with: "99")))
        //: lb.text = "Click to change or delete photo.".localized
        lb.text = String(bytes: kAlongsideFormat.map{$0^239}, encoding: .utf8)!.localized
        //: return lb
        return lb
        //: }()
    }()
...
extension DistinctThen {
    ...
    func filterOutView() {
        ...
        for i in 0 ..< 9 {
            ...
            colorBtn.setBackgroundImage(.scheme(name: (String(mainExtentId.prefix(7)) + String(data_capitalMessage))), for: .normal)
            //: colorBtn.setBackgroundImage(.BundleImageNamed(name: "btn_me_edit_photo_add_pre"), for: .highlighted)
            colorBtn.setBackgroundImage(.scheme(name: (String(app_familySlimKey) + String(noti_bookId.suffix(6)) + "_add_pre")), for: .highlighted)
            ...
            colorBtn.rx.tap.subscribe(onNext: { [weak self] in
                ...
                self.moveOldSender(sender: colorBtn)
            ...
            deleteBtn.setBackgroundImage(.scheme(name: (String(data_followingTwoFormat.prefix(8)) + String(main_maximumMsg.prefix(5)) + "hoto_" + constReduceData.replacingOccurrences(of: "implement", with: "e"))), for: .normal)
    ...
    @objc func moveOldSender(sender: UIButton) {
        //: var isReturn = false
        var isReturn = false
        ...
        EditThen.visitorBlock(true) { (isOpen: Bool) in
            ...
            if isOpen {
                ...
                let vc = MiddlePickTool.nevertheless(maxCount: 9 - self.count, allowPhoto: true, allowVideo: false)
                ...
                vc.didFinishPickingPhotosHandle = { [weak self] (photos: [UIImage]?,
                        ...
                        if photos!.count > 0 {
                            ...
                            self.delegate?.faultPhoto(photos!)
                        }
                        //: return
                }
            }
    ...
    func stack(selete: [NeighborMeasurable]) {
        ...
        for i in 0 ..< selete.count {
            ...
            if i < allPhoto.count {
                ...
                if selete[i].isLocal {
                    ...
                    Btn.mpCharacter(urlStr: selete[i].url)
                }
                //: _ = Btn.subviews.map { view in
                _ = Btn.subviews.map { view in
                    //: if (view is UIButton) {
