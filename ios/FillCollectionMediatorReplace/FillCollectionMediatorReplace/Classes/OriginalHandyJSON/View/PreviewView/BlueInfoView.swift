...
fileprivate let dataCurveTitle:[UInt8] = [0x9,0xe,0x9,0x14,0x48,0x3,0xf,0x4,0x5,0x12,0x5a,0x49,0x40,0x8,0x1,0x13,0x40,0xe,0xf,0x14,0x40,0x2,0x5,0x5,0xe,0x40,0x9,0xd,0x10,0xc,0x5,0xd,0x5,0xe,0x14,0x5,0x4]

private func denyPossible(go num: UInt8) -> UInt8 {
    return num ^ 96
}

/*: "icon_male_default" :*/
fileprivate let dataCuteMeetMsg:[UInt8] = [0x67,0x61,0x6d,0x6c,0x5d,0x6b,0x5f,0x6a,0x63,0x5d,0x62,0x63,0x64,0x5f,0x73,0x6a,0x72]

fileprivate func shadeNap(starting num: UInt8) -> UInt8 {
    let value = Int(num) + 2
    if value > 255 {
...
/*: "icon_female_default" :*/
fileprivate let app_mistAlongValue:[Character] = ["i","c","o","n","_","f","e","m","a","l"]
fileprivate let main_processorValue:String = "age your constrainte_de"

/*: "level_%@" :*/
fileprivate let constScreenMsg:String = "level_but final"
fileprivate let data_writtenPath:String = "%accounting"
...
class BlueInfoView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataCurveTitle.map{denyPossible(go: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .shareOff(type: .Medium, fontSize: 17)
        //: $0.textColor = .white
        $0.textColor = .white
    }
    ...
    lazy var ageLabel = UILabel().then {
        ...
        $0.font = .shareOff(type: .Regular, fontSize: 10)
        //: $0.textColor = .white
        $0.textColor = .white
    }
...
extension BlueInfoView {
    ...
    func runLess(model: StillMeasurable) {
        ...
        let imgStr = model.sex == SharedMirrorKingCustomReflectable.male.rawValue ? String(bytes: dataCuteMeetMsg.map{shadeNap(starting: $0)}, encoding: .utf8)! : (String(app_mistAlongValue) + String(main_processorValue.suffix(4)) + "fault")
        //: sexImgView.image = UIImage(named: imgStr)
        sexImgView.image = UIImage(named: imgStr)
        //: let levelStr = String(format: "level_%@", model.level)
        let levelStr = String(format: (String(constScreenMsg.prefix(6)) + data_writtenPath.replacingOccurrences(of: "accounting", with: "@")), model.level)
