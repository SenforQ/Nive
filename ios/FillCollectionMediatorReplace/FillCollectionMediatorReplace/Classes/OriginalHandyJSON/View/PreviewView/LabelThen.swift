...
fileprivate let main_designTitle:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img_loading_lose" :*/
fileprivate let const_pairPath:String = "stand plat child remaining additionalimg_lo"
fileprivate let k_tarLimitValue:String = "adeach"
fileprivate let dataCoreEmptyBoxFormat:[Character] = ["g","_","l","o","s","e"]
...
class LabelThen: UICollectionViewCell {
    ...
    override init(frame: CGRect) {
        ...
        self.scaleActionAdmin()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: main_designTitle.reversed(), encoding: .utf8)!)
    }
...
extension LabelThen {
    ...
    private func scaleActionAdmin() {
        //: let singleTap = UITapGestureRecognizer(target: self, action: #selector(singleTapAction))
        let singleTap = UITapGestureRecognizer(target: self, action: #selector(jump))

        //: doubleTap = UITapGestureRecognizer(target: self, action: #selector(doubleTapAction)).then {
        doubleTap = UITapGestureRecognizer(target: self, action: #selector(pickings)).then {
...
extension LabelThen {
    ...
    func become(imgModel: NeighborMeasurable) {
        ...
        if imgModel.imagePic != nil {
            ...
            imgView.kf.setImage(with: URL(string: imgModel.url), placeholder: UIImage(named: (String(const_pairPath.suffix(6)) + k_tarLimitValue.replacingOccurrences(of: "each", with: "in") + String(dataCoreEmptyBoxFormat))), options: [.keepCurrentImageWhileLoading]) { _ in
                //: self.recoverSubviews()
                self.segmentSubviews()
            }
        }
    ...
    func canny(imgUrl: String) {
        ...
        imgView.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage(named: (String(const_pairPath.suffix(6)) + k_tarLimitValue.replacingOccurrences(of: "each", with: "in") + String(dataCoreEmptyBoxFormat))), options: [.keepCurrentImageWhileLoading]) { _ in
            //: self.recoverSubviews()
            self.segmentSubviews()
        }
    }
...
extension LabelThen: UIScrollViewDelegate, UIGestureRecognizerDelegate {
...
extension LabelThen {
    ...
    func segmentSubviews() {
        ...
        var newW = show_explainId
        //: var newH = ScreenWidth * tempImg!.size.height / tempImg!.size.width
        var newH = show_explainId * tempImg!.size.height / tempImg!.size.width

        //: if newH > ScreenHeight {
        if newH > userSystemKey {
            //: newH = ScreenHeight
            newH = userSystemKey
            //: newW = tempImg!.size.width * ScreenHeight / tempImg!.size.height
            newW = tempImg!.size.width * userSystemKey / tempImg!.size.height
        }
        ...
        self.imgContainerView.origin = CGPoint(x: (show_explainId - newW) / 2.0, y: (userSystemKey - newH) / 2)
