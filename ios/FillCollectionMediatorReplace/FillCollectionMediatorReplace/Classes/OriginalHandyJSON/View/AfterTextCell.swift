...
fileprivate let main_assetId:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Pending review" :*/
fileprivate let app_trustMsg:String = "Pendmine location off modify nothing"
fileprivate let show_exposureUrl:[Character] = ["e","w"]
...
class AfterTextCell: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_assetId.reversed(), encoding: .utf8)!)
    }
...
extension AfterTextCell {
    //: func setGreetTextCell(model: TalkingGreetModel, row: Int, isLast: Bool) {
    func downwards(model: BotEchoGreetModel, row: Int, isLast: Bool) {
        //: index = row
        index = row
        //: islast = isLast
        islast = isLast
        ...
        if model.status == 0 {
            ...
            statusLb.text = (String(app_trustMsg.prefix(4)) + "ing revi" + String(show_exposureUrl)).localized
