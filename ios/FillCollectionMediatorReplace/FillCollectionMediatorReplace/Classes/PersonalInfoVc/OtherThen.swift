...
class OtherThen: MirrorRealmReactiveCompatible {
    //: var type = LoginType.PhoneLogin
    var type = BackClusterLiteral.PhoneLogin // 修改类型
    //: var content = ""                // 手机号
    var content = "" // 手机号
    //: var isNewPwd = false
    ...
    private lazy var pwdView: StillView = {
        //: let v = TalkingSetNewPwdView(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
        let v = StillView(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
...
extension OtherThen {
    ...
    private func global(_ pwd: String, _ vcode: String?) {
        //: ProgressHUD.show()
        ...
        PhotoPreserveReactiveCompatible.statistical(pwd: pwd, vcode: vcode) { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
...
extension OtherThen {
    ...
    private func stiffeningConstraint() {
