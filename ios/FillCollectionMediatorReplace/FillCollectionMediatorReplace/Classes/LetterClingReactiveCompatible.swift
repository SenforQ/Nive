...
// __CLOSE_PRINT__
//
//  LetterClingReactiveCompatible.swift
//  AbroadTalking
//
...
class LetterClingReactiveCompatible: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: EffQuickLookable?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.featureTo()
            }
        }
    ...
    func superior() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.mottle(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.referenceAndNonsolidColour()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.aircraft(), NSAttributedString.Key.font: UIFont.shareOff(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.mottle(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.aircraft(), NSAttributedString.Key.font: UIFont.shareOff(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.aircraft()
...
extension LetterClingReactiveCompatible: UINavigationControllerDelegate {
    ...
    func navigationController(_ navigationController: UINavigationController, willShow viewController: UIViewController, animated: Bool) {
        ...
        if (viewController.hidesBottomBarWhenPushed) {
            ...
            PressureAdvertisingManager.shared.advSuperView?.isHidden = true
        //: } else {
        } else {
...
extension LetterClingReactiveCompatible: UIGestureRecognizerDelegate {
