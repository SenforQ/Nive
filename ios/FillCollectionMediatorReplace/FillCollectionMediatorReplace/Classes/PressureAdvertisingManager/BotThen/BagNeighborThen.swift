...
class BagNeighborThen: NSObject {
    //: var infoModel: TalkingVideoChatModel?          
    var infoModel: SimulationChatModel?          // 视频通话model
    //: var config = TalkingVideoChatConfig()          
    var config = ClingEarnChatConfig()          // 视频渲染配置类
    ...
    private var trtcView: DisplayReactiveCompatible?    // 预览view（使用商汤美颜）
...
extension BagNeighborThen {
    ...
    func gutter(_ frontCamera: Bool) {
        ...
        self.timeBridgeFront(front: frontCamera)
    }
    
    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func controlCamera(_ openCamera: Bool) {
    ...
    func stint() {
        ...
        DispatchQueue.main.async {
            ...
            self.window(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
...
extension BagNeighborThen {
    ...
    private func timeBridgeFront(front: Bool) {
        ...
        self.trtcView?.talkFront(isFront: front)
    }
    
    /// 是否开启摄像头
    /// - Parameter isOpen: 是否开启
    //: private func setTRTCCamera(isOpen: Bool) {
    private func hair(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard notiMaxMsg == true else {
            //: if isOpen {
            if isOpen {
        ...
        self.trtcView?.reckon(isOpen: isOpen)
...
extension BagNeighborThen {
    ...
    func remarkSignature() {
        // 监听 App 切后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(appDidEnterBackground(notif:)),
                                               selector: #selector(norOpenNotif(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(quantityeraction(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(quickTo(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(replyNotif(notif:)),
    ...
    @objc private func norOpenNotif(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func quantityeraction(notif: Notification?) {
        //: if notif == nil { 
    ...
    @objc private func quickTo(notif: Notification) {
    ...
    @objc func replyNotif(notif: Notification) {
...
extension BagNeighborThen {
    ...
    func window(smallWindow: Bool) {
        ...
        if self.trtcView == nil {
            ...
            self.trtcView = DisplayReactiveCompatible()
        }
        //: self.trtcView?.startPreView(isSmallWindow: smallWindow)
        self.trtcView?.tension(isSmallWindow: smallWindow)
        //: if smallWindow { 
