...
/*: "icon_randow_user_bg" :*/
fileprivate let main_natureData:String = "lowero"
fileprivate let appLaterData:[Character] = ["n","_","r","a","n","d","o","w","_","u","s","e","r","_","b","g"]
...
class FrameworkUserView: UIView {
    // MARK: - UI Components

    //: private let leftAvatarView = AvatarView()
    private let leftAvatarView = StuffView()
    //: private let centerAvatarView = AvatarView()
    private let centerAvatarView = StuffView()
    //: private let rightAvatarView = AvatarView()
    private let rightAvatarView = StuffView()
    ...
    private func randomCustom() {
        ...
        centerBgView.image = UIImage.scheme(name: (main_natureData.replacingOccurrences(of: "lower", with: "ic") + String(appLaterData)))
    }

    //: private func updateAvatarDisplay() {
    private func capableness() {
...
//        }
    ...
    private func cooperativeTransforms() {
        // 重置所有视图的变换
...
extension FrameworkUserView {
    // MARK: - Public Methods

    //: func configure(avatars: [String]) {
    func fitBy(avatars: [String]) {
...
class StuffView: UIView {
    ...
    private func jaw() {
        ...
        imageView.scrubFinish(urlStr: self.imageurl ?? "")
    }
}
