...
fileprivate let data_executeId:[UInt8] = [0xbb,0xc0,0xbb,0xc6,0x7a,0xb5,0xc1,0xb6,0xb7,0xc4,0x8c,0x7b,0x72,0xba,0xb3,0xc5,0x72,0xc0,0xc1,0xc6,0x72,0xb4,0xb7,0xb7,0xc0,0x72,0xbb,0xbf,0xc2,0xbe,0xb7,0xbf,0xb7,0xc0,0xc6,0xb7,0xb6]

fileprivate func safetyDry(isolate num: UInt8) -> UInt8 {
...
/*: "bg_medals_podium" :*/
fileprivate let userDeviceValue:[Character] = ["b","g","_","m","e","d","a"]
fileprivate let dataBookStr:[Character] = ["l","s","_","p","o","d","i","u","m"]

/*: "icon_rank_first_bg" :*/
fileprivate let constBothPath:[Character] = ["i","c","o","n","_","r","a","n","k","_"]
fileprivate let main_textureName:[Character] = ["f","i","r","s","t","_","b","g"]

/*: "icon_rank_first" :*/
fileprivate let show_shapeTitle:[Character] = ["i","c","o","n","_","r","a"]
fileprivate let app_regainKey:[Character] = ["n","k","_","f","i","r","s","t"]

/*: "icon_ammount_points" :*/
fileprivate let user_shadeTitle:[Character] = ["i","c","o","n","_","a","m","m"]
fileprivate let userInvestReplacementMsg:String = "area doubleount_p"
fileprivate let mainHelloCancerFilterKey:String = "OINTS"

/*: "icon_lounge" :*/
fileprivate let app_numbMicValue:[Character] = ["i","c","o","n","_","l","o","u","n","g","e"]

/*: "icon_rank_coin" :*/
fileprivate let main_fromName:[Character] = ["i","c","o","n","_","r","a","n"]
fileprivate let notiFactWaveUrl:String = "k_cointab gender"

/*: "icon_rank_coin2" :*/
fileprivate let app_ignoreStrikeMsg:String = "icotwo"
fileprivate let data_shadowKey:String = "why shadow input relieve_rank_"

/*: "icon_rank_second" :*/
fileprivate let mainPrimaryCanName:String = "inform constraint voice stallicon_"
fileprivate let show_insideStr:String = "RAN"
fileprivate let appVisualMsg:String = "later civil electric smartk_second"

/*: "icon_rank_third" :*/
fileprivate let dataSightValue:String = "icon_rscan dynamics direction event universal"
fileprivate let constOutcomeName:String = "resultd"
...
// __CLOSE_PRINT__
//
//  ReportingReactiveCompatible.swift
//  AbroadTalking
//
...
class ReportingReactiveCompatible: UIView {
    //: var  index = 0
    var index = 0
    ...
    override init(frame: CGRect) {
        ...
        self.anonymousWarning()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: data_executeId.map{safetyDry(isolate: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var medalsView = UIImageView().then {
        ...
        $0.image = UIImage.scheme(name: (String(userDeviceValue) + String(dataBookStr)))
    }

    //: lazy var firstView = SocialRankHeaderItemView.init(frame: .zero, rankType: .firstRankType).then {
    lazy var firstView = PreparePositionView(frame: .zero, rankType: .firstRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 0
        $0.tag = 0
        //: let tap = UITapGestureRecognizer.init(target: self, action: #selector(registTapAction))
        let tap = UITapGestureRecognizer(target: self, action: #selector(adStage))
        //: $0.addGestureRecognizer(tap)
        $0.addGestureRecognizer(tap)
    }

    //: lazy var secondView = SocialRankHeaderItemView.init(frame: .zero, rankType: .secondRankType).then {
    lazy var secondView = PreparePositionView(frame: .zero, rankType: .secondRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 1
        $0.tag = 1
    ...
    lazy var thirdView = PreparePositionView(frame: .zero, rankType: .thirdRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 2
        $0.tag = 2
...
extension ReportingReactiveCompatible {
    //: private func bindInteraction() {
    private func anonymousWarning() {}

    //: private func updateSubviewsWithData() {
    private func submitStreetwise() {
        //: firstView.updateItemWithModel(model: dataArray[0], type: self.index)
        firstView.modifyExecuteMingleMangleReply(model: dataArray[0], type: self.index)
        //: secondView.updateItemWithModel(model: dataArray.count>1 ? dataArray[1]: nil, type: self.index)
        secondView.modifyExecuteMingleMangleReply(model: dataArray.count > 1 ? dataArray[1] : nil, type: self.index)
        //: thirdView.updateItemWithModel(model: dataArray.count>2 ? dataArray[2]: nil, type: self.index)
        thirdView.modifyExecuteMingleMangleReply(model: dataArray.count > 2 ? dataArray[2] : nil, type: self.index)
    }

    //: @objc func registTapAction(tapGesture: UITapGestureRecognizer) {
    @objc func adStage(tapGesture: UITapGestureRecognizer) {
...
extension ReportingReactiveCompatible {
    ...
    private func tantalite() {
        //: let firstWidth = (ScreenWidth - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        let firstWidth = (show_explainId - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        //: let itemWidth = (ScreenWidth - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
        let itemWidth = (show_explainId - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
...
// MARK: - PreparePositionView

//: class SocialRankHeaderItemView: UIView {
class PreparePositionView: UIView {
    ...
    public init(frame: CGRect, rankType: SalesPhoto) {
        //: super.init(frame: frame)
        super.init(frame: frame)
        //: self.setupItemViews(type: rankType)
        self.printing(type: rankType)
    }
    ...
    lazy var headPicBgView = UIImageView().then {
        ...
        $0.image = UIImage.scheme(name: (String(constBothPath) + String(main_textureName)))
    }
    ...
    lazy var crownImgView = UIImageView().then {
        ...
        $0.image = UIImage.scheme(name: (String(show_shapeTitle) + String(app_regainKey)))
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .shareOff(type: .Medium, fontSize: 18)
        //: $0.textAlignment = .center
        $0.textAlignment = .center
    }

    //: lazy var coinButton = TalkingButton().then {
    lazy var coinButton = ExtendedView().then {
        //: $0.isUserInteractionEnabled = false
        $0.isUserInteractionEnabled = false
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_ammount_points"), for: .normal)
        $0.setImage(UIImage.scheme(name: (String(user_shadeTitle) + String(userInvestReplacementMsg.suffix(6)) + mainHelloCancerFilterKey.lowercased())), for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 13)
        $0.titleLabel?.font = .shareOff(type: .Regular, fontSize: 13)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.scheme(name: (String(app_numbMicValue)))
    }
}
...
extension PreparePositionView {
    //: func updateItemWithModel(model: SocialRankItemModel?, type: Int) {
    func modifyExecuteMingleMangleReply(model: FrameworkItemModel?, type: Int) {
        ...
        headPic.commonClose(urlStr: model!.headPic, placeImg: UIImage.backgroundDown(sex: model!.sex))
        ...
        if type == 0 {
            ...
            coinButton.setImage(UIImage.scheme(name: (String(main_fromName) + String(notiFactWaveUrl.prefix(6)))), for: .normal)
            //: } else {
        } else {
            //: coinButton.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin2"), for: .normal)
            coinButton.setImage(UIImage.scheme(name: (app_ignoreStrikeMsg.replacingOccurrences(of: "two", with: "n") + String(data_shadowKey.suffix(6)) + "coin2")), for: .normal)
        }
    }
}
...
//        var topCrownMargin = actualHeight(h: 4)
        ...
        case .firstRankType:
            ...
            crownImgView.image = UIImage.scheme(name: (String(show_shapeTitle) + String(app_regainKey)))
            //: headPic.layer.borderColor = UIColor.RGBA(255, 224, 58, 1).cgColor
            headPic.layer.borderColor = UIColor.itInstall(255, 224, 58, 1).cgColor
...
//            topCrownMargin = actualHeight(h: 10)
        ...
        case .secondRankType:
            ...
            crownImgView.image = UIImage.scheme(name: (String(mainPrimaryCanName.suffix(5)) + show_insideStr.lowercased() + String(appVisualMsg.suffix(8))))
            //: headPic.layer.borderColor = UIColor.RGBA(182, 199, 254, 1).cgColor
            headPic.layer.borderColor = UIColor.itInstall(182, 199, 254, 1).cgColor
        ...
        case .thirdRankType:
            ...
            crownImgView.image = UIImage.scheme(name: (String(dataSightValue.prefix(6)) + "ank_th" + constOutcomeName.replacingOccurrences(of: "result", with: "ir")))
            //: headPic.layer.borderColor = UIColor.RGBA(251, 166, 109, 1).cgColor
            headPic.layer.borderColor = UIColor.itInstall(251, 166, 109, 1).cgColor
            //: headPic.layer.cornerRadius = actualWidth(w: 31)
            headPic.layer.cornerRadius = actualWidth(w: 31)
            //: break
        }
